		...
		(SELECT 1 FROM rept040, rept019
		 ...
		 SELECT 2 FROM talt050, talt023
			WHERE t50_compania    = b12_compania
			  ...
			  AND t23_cont_cred   = "C"
		 UNION ALL
		 SELECT 3 FROM cxct040
			WHERE z40_compania   = b12_compania
			  AND z40_localidad  = 1
			  AND z40_tipo_comp  = b12_tipo_comp
			  AND z40_num_comp   = b12_num_comp
		 UNION ALL
		 SELECT 4 FROM cajt010
...
SELECT (SELECT LPAD(g02_localidad, 2, 0) || " " || TRIM(g02_abreviacion)
	...
	WHERE b12_compania      = 1
	  AND b12_estado        = "M"
	  AND EXISTS
		(SELECT 1 FROM rept040, acero_gc:rept019
			WHERE r40_compania  = b12_compania
			  AND r40_tipo_comp = b12_tipo_comp
			  AND r40_num_comp  = b12_num_comp
			  AND r40_localidad = 2
			  ...
			  AND r19_cont_cred = "C"
		 UNION ALL
		 SELECT 2 FROM cxct040
			WHERE z40_compania   = b12_compania
			  AND z40_localidad  = 2
			  AND z40_tipo_comp  = b12_tipo_comp
			  AND z40_num_comp   = b12_num_comp
		 UNION ALL
		 SELECT 3 FROM cajt010
			WHERE j10_compania     = b12_compania
			  AND j10_localidad    = 2
			  AND j10_tip_contable = b12_tipo_comp
			  AND j10_num_contable = b12_num_comp
		 UNION ALL
		 SELECT 4 FROM acero_gc:cajt010
			WHERE j10_compania     = b12_compania
			  AND j10_localidad    = 2
			  AND j10_tip_contable = b12_tipo_comp
			  AND j10_num_contable = b12_num_comp)
