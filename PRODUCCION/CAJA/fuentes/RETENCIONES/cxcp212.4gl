...
FOREACH q_di_aplic INTO r_z20.*
	...
	WHILE intentar
		...
		IF STATUS <> 0 THEN
			LET int_flag = 0
			CALL fl_hacer_pregunta('Se esta generando primero una transacci�n que esta actualizando el saldo del documento. Si desea intentar generar el AJ nuevamente presione SI, de lo contrario presione NO.', 'Yes')
				RETURNING resp
			IF resp <> 'Yes' THEN
				ROLLBACK WORK
				WHENEVER ERROR STOP
	...
	WHILE intentar
		...
		IF STATUS <> 0 THEN
			LET int_flag = 0
			CALL fl_hacer_pregunta('Al momento otra transacci�n esta actualizando el saldo del documento. Si desea intentar generar el AJ nuevamente presione SI, de lo contrario presione NO.', 'Yes')
				RETURNING resp
			IF resp <> 'Yes' THEN
				ROLLBACK WORK
				WHENEVER ERROR STOP
