...
WHILE TRUE
	...
	FOREACH	q_deto INTO r_det.*, r_r19.r19_compania, r_r19.r19_localidad,
		...
		IF r_det.num_sri IS NULL THEN
			CALL obtener_num_sri(r_det.areaneg, r_r19.r19_cod_tran,
						r_r19.r19_num_tran)
		...
		IF rm_par.r01_codigo IS NOT NULL THEN
			IF r_r19.r19_cod_tran IS NULL THEN
				CONTINUE FOREACH
			END IF
			CALL fl_lee_cabecera_transaccion_rep(r_r19.r19_compania,
							r_r19.r19_localidad,
							r_r19.r19_cod_tran,
							r_r19.r19_num_tran)
				RETURNING r_r19.*
		...
		IF resp = 'Yes' THEN
			...
			CALL fl_lee_cabecera_transaccion_rep(r_r19.r19_compania,
							...
							r_r19.r19_num_tran)
				RETURNING r_r19.*
			CALL fl_lee_vendedor_rep(vg_codcia, r_r19.r19_vendedor)
				RETURNING r_r01.*
			ELSE
			INITIALIZE r_r01.*, r_r19.* TO NULL
			END IF
			LET registro = fl_justifica_titulo('D',r_det.areaneg,3),
				'|', fl_justifica_titulo('D', z_cobro, 5)
				CLIPPED, '|',
...
PAGE HEADER
	...
	PRINT COLUMN 001, "AREA",
	      COLUMN 007, "ZONA",
	      COLUMN 014, "CLIENTE",
	      COLUMN 054, "DOCUMENTO",
	      COLUMN 073, "NUMERO SRI",
	      COLUMN 089, "FECHA EMI.",
	      COLUMN 100, "FECHA VCTO.",
	      COLUMN 111, fl_justifica_titulo('D', "DIAS", 5),
...
ON EVERY ROW
	...
	PRINT COLUMN 001, fl_justifica_titulo('D', areaneg, 3),
	      ...
	      COLUMN 021, nomcli[1,31] CLIPPED,
	      COLUMN 054, tipo_doc, '-', 
	      		  fl_justifica_titulo('I', num_doc, 11) CLIPPED, '-', 
	      		  fl_justifica_titulo('I', dividendo, 3) USING "&&&",
	      COLUMN 073, num_sri CLIPPED,
	      COLUMN 089, fecha_emi USING "dd-mm-yyyy",
	      COLUMN 100, fecha_vcto USING "dd-mm-yyyy",
	      COLUMN 111, antiguedad USING "-,--&",
...
FUNCTION obtener_num_sri(areaneg, cod_tran, num_tran)
...
END CASE
LET num_sri = NULL
SELECT UNIQUE r38_num_sri INTO num_sri
	FROM rept038
