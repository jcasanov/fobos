...
WHILE TRUE
	...
	DISPLAY ARRAY rm_dcli TO rm_dcli.*
		...
		ON KEY(F5)
			LET i = arr_curr()
			SELECT area_n, cod_tran, num_tran
				INTO r_aux.area_n, r_aux.cod_tran,r_aux.num_tran
				FROM tempo_doc 
				WHERE ROWID = rm_rowid[i]
			CALL ver_factura_devolucion(i, r_aux.area_n,
							r_aux.cod_tran,
							r_aux.num_tran, 'D')
			LET int_flag = 0
		ON KEY(F6)
		...
		ON KEY(F8)
			...
			SELECT a.area_n
				...
				WHERE a.codcli   = codcli
				  ...
				  AND a.tipo_doc = 'F'
			CALL ver_factura_devolucion(i, r_aux.area,
							r_aux[i].cod_tran,
							r_aux[i].num_tran, 'F')
			LET int_flag = 0
		ON KEY(F15)
