...
DEFINE vm_num_guia	SMALLINT
DEFINE vm_max_guia	SMALLINT
DEFINE rm_hoja		ARRAY[600] OF RECORD
				r114_guia_remision LIKE rept114.r114_guia_remision,
				--r114_cod_tran	LIKE rept114.r114_cod_tran,
				--r114_num_tran	LIKE rept114.r114_num_tran,
...
DEFINE rm_codhoj	ARRAY[600] OF RECORD
			...
			END RECORD
DEFINE rm_guias		ARRAY[100] OF RECORD
				num_guia	LIKE rept114.r114_guia_remision,
				num_guia_sri	LIKE rept095.r95_num_sri
			END RECORD
...
--DISPLAY "Zona"		TO tit_col4
--DISPLAY "SubZona"		TO tit_col5
...
CALL muestra_estado()
CALL leer_cabecera('I')
IF int_flag THEN
	RETURN
END IF
--CALL generar_det_ing()
CALL lee_detalle()
IF int_flag THEN
	CALL muestra_salir()
	RETURN
...
DEFINE r_r111, r_r111_2	RECORD LIKE rept111.*
...
IF num_args() = 4 THEN
	...
	CONSTRUCT BY NAME expr_sql ON r113_num_hojrut, r113_fecha, r113_estado,
		r113_cod_trans, r113_cod_chofer, --r113_cod_ayudan,
		r113_observacion, r113_km_ini, r113_km_fin, r113_usuario,
	...
	ON KEY(F2)
		...
		END IF
		{--
		IF INFIELD(r113_cod_ayudan) AND
		   r_r110.r110_cod_trans IS NOT NULL
		THEN
			CALL fl_ayuda_chofer(vg_codcia, vg_codloc,
						r_r110.r110_cod_trans, "A", "T")
				RETURNING r_r111_2.r111_cod_chofer,
					  r_r111_2.r111_nombre
		      	IF r_r111_2.r111_cod_chofer IS NOT NULL THEN
				DISPLAY r_r111_2.r111_cod_chofer TO
					r113_cod_ayudan
				DISPLAY r_r111_2.r111_nombre TO tit_ayudan
		      	END IF
		END IF
		--}
		LET int_flag = 0
	AFTER FIELD r113_cod_trans
		...
		ELSE
			INITIALIZE r_r110.*, r_r111.*, r_r111_2.*,
					rm_r113.r113_cod_trans,
					rm_r113.r113_cod_chofer
					--rm_r113.r113_cod_ayudan
				TO NULL
		END IF
		DISPLAY BY NAME rm_r113.r113_cod_trans, r_r110.r110_descripcion,
				r_r110.r110_placa, rm_r113.r113_cod_chofer,
				r_r111.r111_nombre--, rm_r113.r113_cod_ayudan
		--CLEAR tit_ayudan
	AFTER FIELD r113_cod_chofer
		...
		IF rm_r113.r113_cod_chofer IS NOT NULL THEN
			...
			END IF
			{--
			IF r_r111.r111_tipo <> "C" THEN
				CALL fl_mostrar_mensaje('Esta persona no esta configurado como chofer.', 'exclamation')
				NEXT FIELD r113_cod_chofer
			END IF
			--}
		...
		ELSE
			INITIALIZE r_r111.*, rm_r113.r113_cod_chofer TO NULL
		END IF
		DISPLAY BY NAME rm_r113.r113_cod_chofer, r_r111.r111_nombre
	{--
	AFTER FIELD r113_cod_ayudan
		LET rm_r113.r113_cod_trans = GET_FLDBUF(r113_cod_trans)
		IF rm_r113.r113_cod_trans IS NULL THEN
			INITIALIZE r_r111_2.*, rm_r113.r113_cod_ayudan TO NULL
			DISPLAY BY NAME rm_r113.r113_cod_ayudan
			DISPLAY r_r111_2.r111_nombre TO tit_ayudan
			CONTINUE CONSTRUCT
		END IF
		LET rm_r113.r113_cod_ayudan = GET_FLDBUF(r113_cod_ayudan)
		IF rm_r113.r113_cod_ayudan IS NOT NULL THEN
			CALL fl_lee_chofer(vg_codcia, vg_codloc,
						rm_r113.r113_cod_trans,
						rm_r113.r113_cod_ayudan)
				RETURNING r_r111_2.*
			IF r_r111_2.r111_compania IS NULL THEN
				CALL fl_mostrar_mensaje('Este ayudante no existe en la compañía o no esta asociado a éste transporte.', 'exclamation')
				NEXT FIELD r113_cod_ayudan
			END IF
			IF r_r111_2.r111_tipo <> "A" THEN
				CALL fl_mostrar_mensaje('Esta persona no esta configurado como ayudante.', 'exclamation')
				NEXT FIELD r113_cod_ayudan
			END IF
			LET rm_r113.r113_cod_ayudan = r_r111_2.r111_cod_chofer
		ELSE
			INITIALIZE r_r111_2.*, rm_r113.r113_cod_ayudan TO NULL
		END IF
		DISPLAY BY NAME rm_r113.r113_cod_ayudan
...
DISPLAY ARRAY rm_hoja TO rm_hoja.*
	...
	ON KEY(F6)
		CALL fl_ver_transaccion_rep(vg_codcia, vg_codloc,
						rm_hoja[i].r114_cod_tran,
						rm_hoja[i].r114_num_tran)
		LET int_flag = 0
	--}
	ON KEY(F6)
		CALL control_imprimir()
	...
	--#BEFORE ROW
		--#LET i = arr_curr()
		--#LET j = scr_line()
		--#CALL muestra_etiquetas_det(i, vm_ind_arr)
	--#AFTER DISPLAY
		--#CONTINUE DISPLAY
END DISPLAY
CALL muestra_etiquetas_det(0, vm_ind_arr)
...
FOR i = 1 TO vm_ind_arr
	--OUTPUT TO REPORT reporte_hoja_ruta(i, rm_hoja[i].r114_guia_remision)
...
--REPORT reporte_hoja_ruta(i, guia_remision)
REPORT reporte_hoja_ruta(i, codcli, nomcli)
DEFINE i		SMALLINT
DEFINE codcli		LIKE rept019.r19_codcli
DEFINE nomcli		LIKE rept019.r19_nomcli
--DEFINE guia_remision	LIKE rept095.r95_guia_remision
...
PAGE HEADER
	...
	{--
	CALL fl_lee_chofer(rm_r113.r113_compania, rm_r113.r113_localidad,
				rm_r113.r113_cod_trans, rm_r113.r113_cod_ayudan)
		RETURNING r_r111.*
	--}
	PRINT COLUMN 001, "AYUDANTE   : ",-- rm_r113.r113_cod_ayudan USING "&&&", " ", r_r111.r111_nombre CLIPPED,
	      COLUMN 067, "KM FINAL        : ",
...
BEFORE GROUP OF guia_remision
	...
	CALL fl_lee_cabecera_transaccion_rep(vg_codcia, vg_codloc,
		...
		RETURNING r_r19.*
	--
	IF r_r19.r19_nomcli IS NULL THEN
		LET r_r19.r19_nomcli = r_r19.r19_referencia
	END IF
	CALL fl_lee_guias_remision(vg_codcia, vg_codloc, guia_remision)
		RETURNING r_r95.*
	PRINT COLUMN 001, rm_codhoj[i].r108_descripcion[1, 15] CLIPPED,
	      COLUMN 017, r_r19.r19_nomcli[1, 25] CLIPPED,
	      COLUMN 043, r_r95.r95_num_sri CLIPPED
	      --COLUMN 062, r_r95.r95_punto_lleg[1, 60] CLIPPED
	--SKIP 1 LINES
--}
...
ON EVERY ROW
	...
	{ OJO
	NEED 4 LINES
	PRINT COLUMN 062, rm_hoja[i].r114_cod_tran, "-",
			rm_hoja[i].r114_num_tran USING "<<<<<<<&",
	}
	LET expr_cli = ")"
...
FUNCTION leer_cabecera(flag)
DEFINE flag		CHAR(1)

{--
IF flag = 'I' THEN
	FOR vm_num_guia = 1 TO vm_max_guia
		INITIALIZE rm_guias[vm_num_guia].* TO NULL
	END FOR
	LET vm_num_guia = 0
END IF
--}
...
INPUT BY NAME rm_r113.r113_cod_trans, rm_r113.r113_cod_chofer,
	...
	ON KEY(F2)
		...
		THEN
			CALL fl_ayuda_chofer(vg_codcia, vg_codloc,
						rm_r113.r113_cod_trans, "A","A")
				RETURNING r_r111_2.r111_cod_chofer,
					  r_r111_2.r111_nombre
		      	IF r_r111_2.r111_cod_chofer IS NOT NULL THEN
				LET rm_r113.r113_cod_ayudan =
							r_r111_2.r111_cod_chofer
				DISPLAY BY NAME rm_r113.r113_cod_ayudan
				DISPLAY r_r111_2.r111_nombre TO tit_ayudan
		      	END IF
		END IF
	...
	AFTER FIELD r113_cod_ayudan
		...
		IF rm_r113.r113_cod_ayudan IS NOT NULL THEN
			...
			IF r_r111_2.r111_tipo <> "A" THEN
				...
				NEXT FIELD r113_cod_ayudan
			END IF
			IF r_r111_2.r111_estado = "B" THEN
...
{--
FUNCTION lee_guias(flag)
DEFINE flag		CHAR(1)
DEFINE r_guia		ARRAY[100] OF RECORD
				num_guia	LIKE rept114.r114_guia_remision,
				num_guia_sri	LIKE rept095.r95_num_sri
			END RECORD
DEFINE r_r95		RECORD LIKE rept095.*
...
IF vg_gui = 0 THEN        
	...
	LET num_cols = 34 
END IF                  
OPEN WINDOW w_repp252_2 AT row_ini, 26 WITH num_rows ROWS, num_cols COLUMNS
	ATTRIBUTE(FORM LINE FIRST, COMMENT LINE LAST, MENU LINE lin_menu,
		  MESSAGE LINE LAST, BORDER) 
IF vg_gui = 1 THEN
	OPEN FORM f_repf252_2 FROM '../forms/repf252_2'
ELSE
	OPEN FORM f_repf252_2 FROM '../forms/repf252_2c'
END IF
DISPLAY FORM f_repf252_2
DISPLAY "Numero"	TO tit_col1
DISPLAY "No. Guía SRI"	TO tit_col2
LET num_row  = 0
FOR i = 1 TO vm_num_guia
	LET r_guia[i].* = rm_guias[i].*
END FOR
LET n_hojrut = 0
...
END IF
LET int_flag = 0
CALL set_count(vm_num_guia)
INPUT ARRAY rm_guias WITHOUT DEFAULTS FROM rm_guias.*
	ON KEY(INTERRUPT)
		LET int_flag = 0
		CALL fl_mensaje_abandonar_proceso() RETURNING resp      
		IF resp = 'Yes' THEN
			LET int_flag = 1
			FOR i = 1 TO vm_num_guia
				LET rm_guias[i].* = r_guia[i].*
			END FOR
			DELETE FROM tmp_guia WHERE 1 = 1
			LET vm_cabecera = 1
			EXIT INPUT
		END IF
	ON KEY(F2)
		IF INFIELD(num_guia) THEN
			CALL fl_ayuda_guias_remision(vg_codcia, vg_codloc, "R")
				RETURNING n_guia
			IF n_guia IS NOT NULL THEN
				CALL fl_lee_guias_remision(vg_codcia, vg_codloc,
								n_guia)
					RETURNING r_r95.*
				LET rm_guias[i].num_guia     = n_guia
				LET rm_guias[i].num_guia_sri = r_r95.r95_num_sri
				DISPLAY rm_guias[i].* TO rm_guias[j].*
			END IF
		END IF
		LET int_flag = 0
	ON KEY(F5)
		LET vm_cabecera = 1
		IF flag = 'I' THEN
			LET vm_num_guia = num_row
		END IF
		EXIT INPUT
	ON KEY(F6)
		CALL ver_guia(rm_guias[i].num_guia)
		LET int_flag = 0
	BEFORE DELETE
		IF flag = 'I' THEN
			CONTINUE INPUT
		...
		DELETE FROM tmp_detalle
			WHERE r114_guia_remision = n_guia
	BEFORE ROW
		LET i       = arr_curr()
		LET j       = scr_line()
		LET n_guia  = rm_guias[i].num_guia
	...
	AFTER FIELD num_guia
		IF rm_guias[i].num_guia IS NOT NULL THEN
			CALL fl_lee_guias_remision(vg_codcia, vg_codloc,
							rm_guias[i].num_guia)
				RETURNING r_r95.*
			...
			SELECT UNIQUE r114_num_hojrut
				...
				WHERE r113_compania       = vg_codcia
				  ...
				  AND r114_guia_remision  = rm_guias[i].num_guia
				  AND r114_estado         = "N"
			IF n_hojrut_o IS NOT NULL THEN
				LET mens = "La guía de remisión No. ",
					rm_guias[i].num_guia_sri CLIPPED,
					" esta asignada al control de ruta No.",
					" ", n_hojrut_o USING "<<<<<<&", "."
				CALL fl_mostrar_mensaje(mens, 'exclamation')
				NEXT FIELD num_guia
			END IF
		ELSE
			INITIALIZE rm_guias[i].* TO NULL
		END IF
		DISPLAY rm_guias[i].* TO rm_guias[j].*
	AFTER INPUT
		FOR i = 1 TO num_row - 1
			FOR j = i + 1 TO num_row
				IF rm_guias[i].num_guia = rm_guias[j].num_guia
				THEN
					LET mens = "El No. Guía de Remisión ",
					rm_guias[i].num_guia USING "<<<<<<&",
						" esta repetido. "
...
END INPUT
IF vm_cabecera THEN
	CLOSE WINDOW w_repp252_2
	RETURN
END IF
DELETE FROM tmp_guia WHERE 1 = 1
LET vm_num_guia = num_row
FOR i = 1 TO vm_num_guia
	INSERT INTO tmp_guia VALUES (rm_guias[i].*)
	IF flag = 'I' THEN
		CONTINUE FOR
	END IF
	SELECT COUNT(*)
		INTO ctos
		FROM tmp_detalle
		WHERE r114_guia_remision = rm_guias[i].num_guia
	IF ctos > 0 THEN
		CONTINUE FOR
	END IF
	INSERT INTO tmp_detalle
		(r114_guia_remision, r114_cod_tran, r114_num_tran,
		 r108_descripcion, r109_descripcion, r114_hora_lleg,
		 r114_hora_sali, r112_descripcion, r114_recibido_por,
		 r114_cod_zona, r114_cod_subzona, r114_cod_obser)
		SELECT r95_guia_remision, r97_cod_tran, r97_num_tran,
		...
		FROM rept095, rept097
		WHERE r95_compania      = vg_codcia
		  AND r95_localidad     = vg_codloc
		  AND r95_guia_remision = rm_guias[i].num_guia
...
{--
FUNCTION generar_det_ing()
DEFINE r_hoja		RECORD
				r114_guia_remision LIKE rept114.r114_guia_remision,
				r114_cod_tran	LIKE rept114.r114_cod_tran,
				r114_num_tran	LIKE rept114.r114_num_tran,
				r108_descripcion LIKE rept108.r108_descripcion,
	...
	SELECT r95_guia_remision, r97_cod_tran, r97_num_tran,
			...
			(SELECT r109_descripcion
				...
				WHERE r109_compania    = r95_compania
				  ...
				  AND r109_cod_subzona = r95_cod_subzona),
			r95_cod_zona, r95_cod_subzona
		FROM rept095, rept097, tmp_guia
		WHERE r95_compania      = vg_codcia
		  AND r95_localidad     = vg_codloc
		  AND r97_compania      = r95_compania
		  AND r97_localidad     = r95_localidad
		  AND r97_guia_remision = r95_guia_remision
		  AND n_guia            = r97_guia_remision
LET vm_ind_arr = 1
FOREACH q_gen_det INTO r_hoja.*
	INITIALIZE rm_hoja[vm_ind_arr].*, rm_codhoj[vm_ind_arr].* TO NULL
	LET rm_hoja[vm_ind_arr].r114_guia_remision = r_hoja.r114_guia_remision
	LET rm_hoja[vm_ind_arr].r114_cod_tran      = r_hoja.r114_cod_tran
	LET rm_hoja[vm_ind_arr].r114_num_tran      = r_hoja.r114_num_tran
	LET rm_hoja[vm_ind_arr].r108_descripcion   = r_hoja.r108_descripcion
	LET rm_hoja[vm_ind_arr].r109_descripcion   = r_hoja.r109_descripcion
	LET rm_codhoj[vm_ind_arr].r114_cod_zona    = r_hoja.r114_cod_zona
	LET rm_codhoj[vm_ind_arr].r114_cod_subzona = r_hoja.r114_cod_subzona
	LET rm_codhoj[vm_ind_arr].r114_cod_obser   = r_hoja.r114_cod_obser
	LET vm_ind_arr = vm_ind_arr + 1
	IF vm_ind_arr > vm_elementos THEN
...
INPUT ARRAY rm_hoja WITHOUT DEFAULTS FROM rm_hoja.*
	ON KEY(INTERRUPT)
		...
		END IF
	ON KEY(F2)
		IF INFIELD(r114_guia_remision) AND rm_r113.r113_estado <> "P"
		THEN
			CALL fl_ayuda_guias_remision(vg_codcia, vg_codloc, "R")
				RETURNING n_guia
			IF n_guia IS NOT NULL THEN
	...
	AFTER INPUT
		...
		FOR i = 1 TO num_row
			...
			END IF
		END FOR
		IF rm_hoja[num_row].r114_guia_remision IS NULL THEN
...
FOR i = 1 TO vm_ind_arr
	...
	--LET rm_r114.r114_cod_tran      = rm_hoja[i].r114_cod_tran
	--LET rm_r114.r114_num_tran      = rm_hoja[i].r114_num_tran
...
DISPLAY BY NAME rm_r113.r113_num_hojrut, rm_r113.r113_estado,
		...
		rm_r113.r113_cod_trans, rm_r113.r113_cod_chofer,
		--rm_r113.r113_cod_ayudan,
		rm_r113.r113_usu_cierre,
		rm_r113.r113_fec_cierre, rm_r113.r113_usu_elim,
		rm_r113.r113_fec_elim, rm_r113.r113_usuario, rm_r113.r113_fecing
CALL fl_lee_transporte(rm_r113.r113_compania, rm_r113.r113_localidad,
			rm_r113.r113_cod_trans)
	RETURNING r_r110.*
CALL fl_lee_chofer(rm_r113.r113_compania, rm_r113.r113_localidad,
...
DECLARE q_t1 CURSOR FOR
	SELECT r114_guia_remision, r95_num_sri
		FROM rept095, tmp_detalle
		WHERE r95_compania      = vg_codcia
		  AND r95_localidad     = vg_codloc
...
LET vm_num_guia = 1
DELETE FROM tmp_guia WHERE 1 = 1
FOREACH q_t1 INTO rm_guias[vm_num_guia].*
	INSERT INTO tmp_guia VALUES (rm_guias[vm_num_guia].*)
	LET vm_num_guia = vm_num_guia + 1
        IF vm_num_guia > vm_max_guia THEN
		EXIT FOREACH
	END IF	
END FOREACH 
LET vm_num_guia = vm_num_guia - 1
--}
SELECT COUNT(*) INTO vm_ind_arr FROM tmp_detalle
DROP TABLE tmp_detalle
CALL muestra_etiquetas_det(0, vm_ind_arr)
...
END IF
{--
SELECT r114_guia_remision, r114_cod_tran, r114_num_tran,
		r108_descripcion, r109_descripcion, r114_hora_lleg,
		r114_hora_sali, r114_recibido_por, r112_descripcion,
		r114_cod_zona, r114_cod_subzona, r114_cod_obser
--}
...
LET vm_ind_arr = 1
FOREACH q_hoja INTO rm_hoja[vm_ind_arr].*, rm_codhoj[vm_ind_arr].*
	LET vm_ind_arr = vm_ind_arr + 1
        IF vm_ind_arr > vm_elementos THEN
		EXIT FOREACH
	END IF	
