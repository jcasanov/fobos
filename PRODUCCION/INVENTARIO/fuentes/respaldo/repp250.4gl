--------------------------------------------------------------------------------
-- Titulo           : repp250.4gl - Inventario Físico por Bodega
-- Elaboracion      : 10-Dic-2010
-- Autor            : NPC
-- Formato Ejecucion: fglrun repp250 base módulo compañía localidad
-- Ultima Correccion: 
-- Motivo Correccion: 
--------------------------------------------------------------------------------
...
DEFINE total_bueno	DECIMAL(12,2)
DEFINE total_incompleto	DECIMAL(12,2)
DEFINE total_suma	DECIMAL(12,2)
...
DEFER INTERRUPT
CLEAR SCREEN
CALL startlog('../logs/repp250.err')
--#CALL fgl_init4js()
...
IF vg_gui = 0 THEN
	...
	LET num_cols = 78
END IF
OPEN WINDOW w_repp250 AT row_ini, 2 WITH num_rows ROWS, num_cols COLUMNS
	ATTRIBUTE(FORM LINE FIRST, COMMENT LINE LAST, MENU LINE lin_menu,
		  MESSAGE LINE LAST - 1, BORDER) 
IF vg_gui = 1 THEN
	OPEN FORM f_repf250 FROM "../forms/repf250_1"
ELSE
	OPEN FORM f_repf250 FROM "../forms/repf250_1c"
END IF
DISPLAY FORM f_repf250
...
DECLARE q_deto CURSOR FOR
	SELECT bodega, item, descrip, vendible, no_vend, sto_act, total,
...
FOREACH q_deto INTO rm_inventario[vm_num_det].*, rm_periodo[vm_num_det].*,
	...
	--}
		...
		LET rm_inventario[vm_num_det].mens_dif       = NULL
		IF rm_inventario[vm_num_det].stock_act = 0 THEN
			LET rm_inventario[vm_num_det].stock_act = NULL
		END IF
	END IF
	IF rm_periodo[vm_num_det].diferencia = 0 THEN
		LET rm_periodo[vm_num_det].diferencia     = NULL
	END IF
...
DEFINE i, j, posi	SMALLINT
DEFINE pos_pan		SMALLINT
DEFINE encontro		SMALLINT
DEFINE mensaje		VARCHAR(250)
DEFINE resp		CHAR(6)

IF vm_posicionar = 'S' THEN
	LET pos_pan  = 1
	LET encontro = 0
	FOR i = 1 TO vm_num_det
		IF rm_inventario[i].r89_item = vm_item THEN
			LET posi     = i
			LET encontro = 1
			EXIT FOR
		END IF
		IF pos_pan > fgl_scr_size('rm_inventario') THEN
			LET pos_pan = 1
		ELSE
			LET pos_pan = pos_pan + 1
		END IF
	END FOR
	IF NOT encontro THEN
		LET pos_pan = 1
		LET posi    = 1
		CALL fl_mostrar_mensaje('El ítem en esta bodega no se encuentra en el detalle para posicionarlo.', 'info')
	END IF
END IF
CALL muestra_tot_reg()
LET int_flag = 0
CALL set_count(vm_num_det)
INPUT ARRAY rm_inventario WITHOUT DEFAULTS FROM rm_inventario.*
	ON KEY(INTERRUPT)
		LET int_flag = 1
	...
	BEFORE INPUT 
		...
		--#IF vm_posicionar = 'S' THEN
			--#CALL dialog.setcurrline(pos_pan, posi)
			--#CALL muestra_contadores_det(posi, vm_num_det)
		--#END IF
		IF (rm_g05.g05_tipo <> 'UF' OR rm_vend.r01_tipo = 'J' OR
		    rm_vend.r01_tipo = 'G')
		THEN
	...
	BEFORE ROW
		LET i = arr_curr()
		LET j = scr_line()
		CALL muestra_etiquetas(rm_inventario[i].r89_item, i, j,
					vm_num_det)
		CALL obtener_totales(i, j)
	AFTER ROW
		IF (rm_inventario[i].r89_bueno IS NULL AND
		    rm_inventario[i].r89_incompleto IS NOT NULL) OR
		   (rm_inventario[i].r89_bueno IS NOT NULL AND
		    rm_inventario[i].r89_incompleto IS NULL)
		THEN
			CALL fl_mostrar_mensaje('Debe digitar el valor vendible y/o no vendible de este ítem', 'exclamation')
			CONTINUE INPUT
		END IF
		CALL obtener_totales(i, j)
	AFTER FIELD r89_bueno, r89_incompleto
		LET i = arr_curr()
		LET j = scr_line()
		IF (rm_inventario[i].r89_bueno IS NULL AND
		    rm_inventario[i].r89_incompleto IS NOT NULL)
		THEN
			LET rm_inventario[i].r89_bueno = 0
		END IF
		IF (rm_inventario[i].r89_bueno IS NOT NULL AND
		    rm_inventario[i].r89_incompleto IS NULL)
		THEN
	...
	AFTER INPUT
		...
		IF total_suma = 0 AND vm_num_det > 1 THEN
			CALL fl_mostrar_mensaje('Al menos debe digitar el Inventario de un ítem.', 'exclamation')
			CONTINUE INPUT
		END IF
		--
		CALL obtener_totales(i, j)
		IF vm_posicionar = 'S' THEN
			IF vm_item IS NULL THEN
				CALL fl_mostrar_mensaje('Digite el item desde el cual quiere posicionarse en el detalle.', 'exclamation')
				NEXT FIELD vm_item
			END IF
		END IF
END INPUT
...
FOR i = 1 TO vm_num_det
	...
	IF STATUS = NOTFOUND THEN
		...
		LET r_r89.r89_bodega     = rm_inventario[i].r89_bodega
		LET r_r89.r89_item       = rm_inventario[i].r89_item
		LET r_r89.r89_usuario    = rm_r89.r89_usuario
		LET r_r89.r89_anio       = rm_periodo[i].r89_anio
		LET r_r89.r89_mes        = rm_periodo[i].r89_mes
...
LET int_flag = 0
INPUT BY NAME vm_bodega, vm_item, vm_posicionar, rm_r89.r89_usuario,
	...
	AFTER INPUT
		...
		END IF
		{
		IF vm_bodega IS NULL AND vm_item IS NULL THEN
			CALL fl_mostrar_mensaje('Al menos debe ingresar la Bodega ó un ítem para registrar el Inventario.', 'exclamation')
			CONTINUE INPUT
		END IF
		IF vm_bodega IS NOT NULL AND vm_item IS NOT NULL THEN
			CALL fl_mostrar_mensaje('Debe solo ingresar la Bodega ó un ítem que se esta haciendo el Inventario.', 'exclamation')
			CONTINUE INPUT
		END IF
		}
END INPUT
...
LET expr_bod2 = NULL
IF vm_bodega IS NOT NULL THEN
	LET expr_bod  = '   AND r11_bodega    = "', vm_bodega, '"'
	LET expr_bod2 = '   AND r89_bodega    = "', vm_bodega, '"'
END IF
LET expr_ite  = NULL
LET expr_ite2 = NULL
IF vm_item IS NOT NULL AND vm_posicionar = 'N' THEN
	LET expr_ite  = '   AND r11_item      = "', vm_item CLIPPED, '"'
...
--}
	LET query = 'SELECT * FROM resp_exis ',
		...
		expr_bod CLIPPED, 
		expr_ite CLIPPED, 
		'   AND r11_bodega    IN (SELECT bod_loc FROM tmp_bod) ',
		'   AND DATE(r11_fec_corte) BETWEEN "', vm_fecha_ini,
					     '" AND "', vm_fecha_fin, '"',
		'   AND r11_stock_act <> 0 ',
		' INTO TEMP t_e '
	PREPARE exec_t_e FROM query
	EXECUTE exec_t_e
	LET query = 'SELECT r11_bodega bodega, r11_item item, ',
			'r10_nombre descrip, 0.00 vendible, 0.00 no_vend, ',
			'0.00 total, "NO DIGITAD" mens_dife, ',
			'YEAR(r11_fec_corte) anio, ',
			'MONTH(r11_fec_corte) mes, r10_marca marca, ',
			'r72_desc_clase clase, r11_stock_act sto_act, ',
			'"', vg_usuario CLIPPED, '" usuario, "" usu_mod ',
		' FROM t_e, rept010, rept072 ',
...
LET query = 'SELECT bodega, item, descrip, vendible, no_vend, total, ',
			...
			'     WHEN mens_dife IS NOT NULL ',
				'THEN mens_dife ',
				'ELSE "" ',
			'END mens_dife, (sto_act - total) diferencia, ',
...
FOR l = 1 TO vm_num_det
	...
	IF rm_inventario[l].r89_suma IS NOT NULL THEN
		...
		LET total_suma = total_suma + rm_inventario[l].stock_act
	END IF
	{--
	IF rm_periodo[l].diferencia IS NOT NULL THEN
		LET total_dif = total_dif + rm_periodo[l].diferencia
	END IF
	--}
	IF rm_inventario[l].r89_suma IS NOT NULL THEN
		LET total_dif = total_dif + rm_inventario[l].r89_suma
	END IF
END FOR
...
LET query = 'SELECT (SELECT g02_abreviacion ',
			...
			'NVL(bodega,r89_bodega) bodega, r02_nombre, marca, ',
			'clase, descrip, item, ',
			'CASE WHEN sto_act < total THEN "SOBRANTE" ',
