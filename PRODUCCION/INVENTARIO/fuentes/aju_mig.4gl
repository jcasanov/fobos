...
INPUT BY NAME rm_r19.r19_referencia, rm_r19.r19_vendedor, rm_r19.r19_bodega_ori,
	...
	ON KEY(F2)
		...
		IF INFIELD(r19_bodega_ori) THEN
		     CALL fl_ayuda_bodegas_rep(vg_codcia, vg_codloc, 'A', 'T', 'A', 'T')
	    	...
	    	IF rm_aj_exist[i].r20_item IS NOT NULL THEN
				...
				CALL fl_mostrar_mensaje('El item no pertenece a la División ingresada.','exclamation')
                	...
                	IF rm_r10.r10_costo_mb = 0 THEN
                       		--CALL fgl_winmessage(vg_producto,'El item no tiene costo, haga un ajuste de costo.','exclamation')
				CALL fl_mostrar_mensaje('El item no tiene costo, haga un ajuste de costo.','exclamation')
                       		NEXT FIELD r20_item
                	END IF
			CALL muestra_etiquetas_det(i, max_row, i)
...
DECLARE q_caca CURSOR FOR
	SELECT * FROM rept011
		...
		ORDER BY r11_item
LET vm_ind_arr = 1
FOREACH q_caca INTO r_r11.*
	CALL fl_lee_item(r_r11.r11_compania, r_r11.r11_item) RETURNING r_r10.*
	IF r_r10.r10_linea <> linea THEN
		CONTINUE FOREACH
	END IF
	DECLARE q_caca2 CURSOR FOR
		SELECT r20_item
			FROM rept019, rept020
			WHERE r19_compania   = r_r11.r11_compania
			  AND r19_localidad  = vg_codloc
			  AND r19_cod_tran   IN ('A+', 'A-')
			  AND r19_bodega_ori = r_r11.r11_bodega
			  AND r19_referencia MATCHES
					'CARGA INICIAL POR MIGRACION BD*'
			  AND r20_compania   = r19_compania
			  AND r20_localidad  = r19_localidad
			  AND r20_cod_tran   = r19_cod_tran
			  AND r20_num_tran   = r19_num_tran
			  AND r20_bodega     = r19_bodega_ori
			  AND r20_item       = r_r11.r11_item
			ORDER BY r20_orden
	OPEN q_caca2
	FETCH q_caca2 INTO item
	IF STATUS <> NOTFOUND THEN
		CLOSE q_caca2
		FREE q_caca2
		CONTINUE FOREACH
	...
	CLOSE q_caca2
	FREE q_caca2
	IF r_r10.r10_comentarios <> 'MIGRACION' THEN
		CONTINUE FOREACH
	END IF
	LET rm_aj_exist[vm_ind_arr].r20_cant_ven  = r_r11.r11_stock_act
	LET rm_aj_exist[vm_ind_arr].r20_stock_ant = 0
	LET rm_aj_exist[vm_ind_arr].r20_item      = r_r11.r11_item
	LET rm_aj_exist[vm_ind_arr].r10_nombre    = r_r10.r10_nombre
	LET rm_aj_exist[vm_ind_arr].r10_costo_mb  = r_r10.r10_costo_mb
	LET rm_aj_exist[vm_ind_arr].total         = r_r11.r11_stock_act *
							r_r10.r10_costo_mb
