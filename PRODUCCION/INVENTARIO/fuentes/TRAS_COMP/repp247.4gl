		...
		IF rm_detalle[i].r44_item_tra IS NOT NULL THEN
			...
			IF r_r10.r10_costo_mb <= 0.01 AND
			   fl_item_tiene_movimientos(r_r10.r10_compania,
							r_r10.r10_codigo)
			THEN
				LET rm_detalle[i].r44_costo_tra =
							r_r10.r10_costo_mb
				DISPLAY BY NAME rm_detalle[i].r44_costo_tra
			...
			CALL fl_lee_item(vg_codcia, rm_detalle[i].r44_item_ori)
				RETURNING r_r10_2.*
			IF r_r10_2.r10_costo_mb <= 0.01 AND
			   fl_item_tiene_movimientos(r_r10_2.r10_compania,
							r_r10_2.r10_codigo)
			THEN
				CALL fl_mostrar_mensaje('Debe estar configurado correctamente el costo del item origen y NO con costo menor igual a 0.01.', 'exclamation')
				NEXT FIELD r44_item_tra
...
DEFINE costo_ori	LIKE rept010.r10_costo_mb
DEFINE sto_tra		LIKE rept011.r11_stock_act
...
{--
	...
	IF r_r10_o.r10_cantpaq < r_r10_t.r10_cantpaq THEN
		LET costo_ori = rm_detalle[i].r44_costo_ori *
				r_r10_t.r10_cantpaq
	ELSE
		LET costo_ori = rm_detalle[i].r44_costo_ori /
				r_r10_t.r10_cantpaq
...
END IF
--}
IF rm_detalle[i].r44_costo_tra <= 0.01 AND
   fl_item_tiene_movimientos(vg_codcia, rm_detalle[i].r44_item_tra)
THEN
...
FOREACH q_det2 INTO item, cantidad, item_t
	...
	END IF
	IF cod_tran <> 'A+' THEN
		CALL fl_lee_item(vg_codcia, item_cons) RETURNING r_r10_o.*
		IF cod_tran = 'A-' THEN
			CALL fl_lee_item(vg_codcia, item_t)
				RETURNING r_r10_t.*
		ELSE
...
{
FUNCTION conversion_unidades(i)
DEFINE i		SMALLINT
DEFINE r_r10_o, r_r10_t	RECORD LIKE rept010.*

CALL fl_lee_item(vg_codcia, rm_detalle[i].r44_item_tra) RETURNING r_r10_t.*
CALL fl_lee_item(vg_codcia, rm_detalle[i].r44_item_ori) RETURNING r_r10_o.*
IF r_r10_o.r10_cantpaq < r_r10_t.r10_cantpaq THEN
	LET cant = (rm_detalle[i].r44_cant_tra / r_r10_t.r10_cantpaq)
ELSE
	LET cant = (rm_detalle[i].r44_cant_tra * r_r10_t.r10_cantpaq)
END IF
