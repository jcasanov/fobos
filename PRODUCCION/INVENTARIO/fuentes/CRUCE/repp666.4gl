			...
			IF resp = 'Yes' THEN
				CALL control_cargar_transferencia(vg_codloc,
					...
					RETURNING rm_tran[i].r90_codtra_fin,
				                  rm_tran[i].r90_numtra_fin
			END IF
			--#IF rm_tran[i].r90_codtra_fin IS NOT NULL AND resp = 'Yes' THEN
				--#CALL dialog.keysetlabel("F6","Destino")
				--#CALL dialog.keysetlabel("F7","")
				--#CALL dialog.keysetlabel("F8","")
			--# END IF
...
DEFINE base_rem1	CHAR(50)
DEFINE base_rem2	CHAR(50)
...
SELECT * INTO r_r02.*
	FROM rept002
	WHERE r02_compania = r_r19.r19_compania
	  AND r02_codigo   = r_r19.r19_bodega_ori
...
SELECT * INTO r_r02.*
	FROM rept002
	WHERE r02_compania = r_r19.r19_compania
	  AND r02_codigo   = r_r19.r19_bodega_dest
IF r_r02.r02_localidad != vg_codloc THEN
...
CASE vg_codloc
	{--
	WHEN 1	LET base_rem1 = 'acero_qm@idsuio01:'
		LET base_rem2 = 'acero_qs@idsuio01:'
	WHEN 3	LET base_rem1 = 'acero_gm@idsgye01:'
		LET base_rem2 = 'acero_qs@idsuio01:'
	WHEN 4	LET base_rem1 = 'acero_qm@idsuio01:'
		LET base_rem2 = 'acero_gm@idsgye01:'
	WHEN 1	LET base_rem1 = 'acero_qm@acgyede:'
		LET base_rem2 = 'acero_qs@acgyede:'
	WHEN 3	LET base_rem1 = 'aceros@acgyede:'
		LET base_rem2 = 'acero_qs@acgyede:'
	WHEN 4	LET base_rem1 = 'aceros@acgyede:'
		LET base_rem2 = 'acero_gm@acgyede:'
	--}
	--
	WHEN 1	LET base_rem1 = 'acero_qm@idsuio01:'
		LET base_rem2 = 'acero_qs@idsuio02:'
	WHEN 3	LET base_rem1 = 'acero_gm@idsgye01:'
		LET base_rem2 = 'acero_qs@idsuio02:'
	WHEN 4	LET base_rem1 = 'acero_qm@idsuio01:'
		LET base_rem2 = 'acero_gm@idsgye01:'
...
IF NOT flag THEN
	--IF vg_codloc = 1 THEN
		...
		END IF
		IF vm_cruce THEN
			CALL proceso_cruce_de_bodegas(codloc, cod_tran,num_tran,
							r_r19.*, flag)
		END IF
	--END IF
...
FOREACH qu_dtr INTO r_r20.*
	...
	LET query = 'SELECT * FROM ', base_rem1 CLIPPED, 'rept068 ',
			'WHERE r68_compania = ', vg_codcia,
			'  AND r68_loc_tr   = ', r_r20.r20_localidad,
			'  AND r68_cod_tr   = "', r_r20.r20_cod_tran, '"',
			'  AND r68_num_tr   = ', r_r20.r20_num_tran,
			' UNION ',
			'SELECT * FROM ', base_rem2 CLIPPED, 'rept068 ',
			'WHERE r68_compania = ', vg_codcia,
			'  AND r68_loc_tr   = ', r_r20.r20_localidad,
			'  AND r68_cod_tr   = "', r_r20.r20_cod_tran, '"',
			'  AND r68_num_tr   = ', r_r20.r20_num_tran,
	...
	EXECUTE exec_t1
	DECLARE q_r68 CURSOR FOR SELECT * FROM t1
	OPEN q_r68
	FETCH q_r68 INTO r_r68.*
	CLOSE q_r68
	FREE q_r68
	DROP TABLE t1
	IF r_r68.r68_compania IS NOT NULL THEN
		LET r_r20.r20_cant_ven = r_r68.r68_cantidad
		--LET r_r20.r20_cant_ped = r_r68.r68_cantidad
	END IF
	IF NOT flag THEN
...
IF flag THEN
	UPDATE rept090
		...
		WHERE r90_compania  = r_r90.r90_compania
		  ...
		  AND r90_num_tran  = r_r90.r90_num_tran
	--IF vg_codloc = 1 THEN
		CALL proceso_cruce_de_bodegas(r_r90.r90_localidad,
						r_r90.r90_cod_tran,
						r_r90.r90_num_tran,
						r_r19.*, flag)
	--END IF
	LET query = 'SELECT r90_compania cia, r90_localidad loc, ',
				'r90_cod_tran tp, r90_num_tran num ',
			' FROM ', base_rem1 CLIPPED, 'rept042, ',
				base_rem1 CLIPPED, 'rept090 ',
			' WHERE r42_compania   = ', vg_codcia,
			'   AND r42_localidad  = ', r_r90.r90_localidad,
			'   AND r42_cod_tr_re  = "', r_r90.r90_cod_tran, '"',
			'   AND r42_num_tr_re  = ', r_r90.r90_num_tran,
			'   AND r90_compania   = r42_compania ',
			'   AND r90_locali_fin = r42_localidad ',
			'   AND r90_codtra_fin = r42_cod_tran ',
			'   AND r90_numtra_fin = r42_num_tran ',
			' UNION',
			' SELECT r90_compania cia, r90_localidad loc, ',
				'r90_cod_tran tp, r90_num_tran num ',
			' FROM ', base_rem2 CLIPPED, 'rept042, ',
				base_rem2 CLIPPED, 'rept090 ',
			' WHERE r42_compania   = ', vg_codcia,
			'   AND r42_localidad  = ', r_r90.r90_localidad,
			'   AND r42_cod_tr_re  = "', r_r90.r90_cod_tran, '"',
			'   AND r42_num_tr_re  = ', r_r90.r90_num_tran,
	...
	INSERT INTO rept068
		(r68_compania, r68_localidad, r68_cod_tran, r68_num_tran,
		 r68_loc_tr, r68_cod_tr, r68_num_tr, r68_bodega, r68_item,
		 r68_secuencia, r68_cantidad, r68_usuario, r68_fecing)
		SELECT a.r19_compania,
			(SELECT e.r68_localidad
				FROM rept068 e
				WHERE e.r68_compania = c.cia
				  AND e.r68_loc_tr   = c.loc
				  AND e.r68_cod_tr   = c.tp
				  AND e.r68_num_tr   = c.num),
			(SELECT e.r68_cod_tran
				FROM rept068 e
				WHERE e.r68_compania = c.cia
				  AND e.r68_loc_tr   = c.loc
				  AND e.r68_cod_tr   = c.tp
				  AND e.r68_num_tr   = c.num),
			(SELECT e.r68_num_tran
				FROM rept068 e
				WHERE e.r68_compania = c.cia
				  AND e.r68_loc_tr   = c.loc
				  AND e.r68_cod_tr   = c.tp
				  AND e.r68_num_tr   = c.num),
			a.r19_localidad, a.r19_cod_tran, a.r19_num_tran,
			b.r20_bodega, b.r20_item, b.r20_orden,
			b.r20_cant_ven * (-1), a.r19_usuario, b.r20_fecing
			FROM t2 c, rept019 a, rept020 b
			WHERE a.r19_compania  = c.cia
			  AND a.r19_localidad = c.loc
			  AND a.r19_cod_tran  = r_r20.r20_cod_tran
			  AND a.r19_num_tran  = r_r20.r20_num_tran
			  ...
			  AND EXISTS
				(SELECT 1 FROM rept019 d
					WHERE d.r19_compania  = c.cia
					  AND d.r19_localidad = c.loc
					  AND d.r19_cod_tran  = c.tp
					  AND d.r19_num_tran  = c.num)
	INSERT INTO rept069
		(r69_compania, r69_localidad, r69_cod_tran, r69_num_tran,
		 r69_loc_tr, r69_cod_tr, r69_num_tr, r69_fecing)
		SELECT a.r19_compania,
			(SELECT e.r68_localidad
				FROM rept068 e
				WHERE e.r68_compania = c.cia
...
FUNCTION proceso_cruce_de_bodegas(codloc, cod_tran, num_tran, r_r19, flag)
DEFINE codloc		LIKE rept091.r91_localidad
DEFINE cod_tran		LIKE rept091.r91_cod_tran
DEFINE num_tran		LIKE rept091.r91_num_tran
DEFINE r_r19		RECORD LIKE rept019.*
...
DEFINE r_fact		RECORD
			...
			END RECORD
DEFINE r_r19_f		RECORD LIKE rept019.*
...
IF NOT verificar_item_bodega_sin_stock(codloc, cod_tran, num_tran, r_r19.*) THEN
	RETURN
END IF
...
FUNCTION verificar_item_bodega_sin_stock(codloc1, cod_tran, num_tran, r_r19)
DEFINE codloc1		LIKE rept091.r91_localidad
DEFINE cod_tran		LIKE rept091.r91_cod_tran
DEFINE num_tran		LIKE rept091.r91_num_tran
DEFINE r_r19		RECORD LIKE rept019.*
...
IF cuantos = 0 THEN
	...
	RETURN 0
END IF
LET vm_stock_pend = obtener_stock_pendiente(codloc1, cod_tran, num_tran,r_r19.*)
...
FUNCTION obtener_stock_pendiente(codloc, cod_tran, num_tran, r_r19)
DEFINE codloc		LIKE rept091.r91_localidad
DEFINE cod_tran		LIKE rept091.r91_cod_tran
DEFINE num_tran		LIKE rept091.r91_num_tran
DEFINE r_r19		RECORD LIKE rept019.*
...
DEFINE expr_sql1	VARCHAR(200)
DEFINE expr_sql2	VARCHAR(200)

INITIALIZE r_r69.*, expr_sql1, expr_sql2 TO NULL
DECLARE q_r69 CURSOR FOR
	SELECT * FROM rept069
		WHERE r69_compania = vg_codcia
		  AND r69_loc_tr   = codloc
		  AND r69_cod_tr   = cod_tran
		  AND r69_num_tr   = num_tran
OPEN q_r69
FETCH q_r69 INTO r_r69.*
CLOSE q_r69
FREE q_r69
CALL obtener_bod_sin_stock()
IF r_r69.r69_compania IS NOT NULL THEN
	LET expr_sql1 = '   AND r20_cod_tran = "', r_r69.r69_cod_tran, '"',
			'   AND r20_num_tran = ', r_r69.r69_num_tran
END IF
...
EXECUTE exec_t_r20
IF r_r69.r69_compania IS NOT NULL THEN
	LET expr_sql1 = '   AND r19_cod_tran = "', r_r69.r69_cod_tran, '"',
			'   AND r19_num_tran = ', r_r69.r69_num_tran
	LET expr_sql2 = '   AND r19_tipo_dev = "', r_r69.r69_cod_tran, '"',
			'   AND r19_num_dev  = ', r_r69.r69_num_tran
END IF
