...
IF vg_gui = 0 THEN
	...
	LET num_cols = 78
END IF
OPEN WINDOW w_repf216_1 AT row_ini, 2 WITH num_rows ROWS, num_cols COLUMNS
	ATTRIBUTE(FORM LINE FIRST + 1, COMMENT LINE LAST, MENU LINE lin_menu,
		  BORDER, MESSAGE LINE LAST - 1) 
IF vg_gui = 1 THEN
	OPEN FORM f_repf216_1 FROM '../forms/repf216_1'
ELSE
	OPEN FORM f_repf216_1 FROM '../forms/repf216_1c'
END IF
DISPLAY FORM f_repf216_1
...
DISPLAY ARRAY r_detalle TO r_detalle.*
	...
	ON KEY(F9)
		...
		--#ELSE
			--#CALL dialog.keysetlabel("F9","Guía Remisión")
		--#END IF
	--#BEFORE ROW
		--#LET i = arr_curr()
		--#LET j = scr_line()
...
FOR j = 1 TO vm_num_detalles
	...
	IF localidad_bodega(rm_r19.r19_bodega_ori) = vg_codloc THEN
		...
		CALL fl_lee_bodega_rep(r_r11.r11_compania, r_r11.r11_bodega)
			RETURNING r_r02.*
		IF r_r11.r11_stock_act < 0 AND r_r02.r02_tipo <> 'S' THEN
			WHENEVER ERROR STOP
			ROLLBACK WORK
...
INPUT BY NAME rm_r19.r19_vendedor,    rm_r19.r19_bodega_ori,
	...
	AFTER INPUT
		...
		THEN
			...
			NEXT FIELD r19_bodega_ori
		END IF
		IF rm_r19.r19_bodega_dest = vm_bod_tal AND vg_usuario <> "FOBOS"
		THEN
			CALL fl_mostrar_mensaje('No puede poner como destino la bodega logica del taller.','exclamation')
			NEXT FIELD r19_bodega_dest
		END IF
		CALL fl_lee_bodega_rep(vg_codcia, rm_r19.r19_bodega_ori)
			RETURNING rm_r02.*
		...
		THEN
			...
			NEXT FIELD r19_bodega_ori
		END IF
		--IF rm_vend.r01_tipo <> 'G' THEN COMENTADO EL 27/02/2014
			IF local_ori <> vg_codloc THEN
...
IF (localidad_bodega(bodega_ori) = 1 AND localidad_bodega(bodega_dest) = 3) OR
   ...
   (localidad_bodega(bodega_ori) = 7 AND localidad_bodega(bodega_dest) = 6) THEN
	IF (TODAY >= MDY(12, 28, YEAR(TODAY))) AND 
...
IF r_r97.r97_compania IS NOT NULL THEN
	LET comando  = 'cd ..', vg_separador, '..', vg_separador, 'REPUESTOS',
			...
			rm_r19.r19_cod_tran, '"'
	RUN comando
	CALL generar_doc_elec(r_r97.r97_guia_remision)
END IF
