...
display ' caca ', r_r36.r36_bodega, ' ', r_detalle[i].r20_bodega
	...
	END IF
display '  cant_dev ', cant_dev, ' ', r_r37.r37_cant_ent, ' bd ',
	r_r36.r36_bodega_real
			...
			IF cantidad > cant_tra THEN
				...
				SELECT NVL(SUM(r37_cant_ent), 0)
					...
					WHERE r34_compania    = vg_codcia
					  ...
					  AND r37_bodega      = r36_bodega
					  AND r37_num_entrega = r36_num_entrega
					  AND r37_item        = r_item.item
				{--
				IF cant_ent < cant_tra THEN
					LET cantidad = cantidad - cant_tra
				ELSE
					LET cantidad = cantidad - cant_ent
				END IF
				--}
				LET cantidad = cantidad - cant_tra
			ELSE
				LET cantidad = cant_tra
display 'nuevo ', cantidad
			END IF
		END IF
		IF cantidad <= 0 THEN
			CONTINUE FOREACH
