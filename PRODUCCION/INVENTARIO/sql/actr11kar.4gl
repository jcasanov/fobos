...
FOREACH q_items INTO bodega, item, stock
	...
	WHENEVER ERROR CONTINUE
	UPDATE rept011
		SET r11_stock_act = saldo
		WHERE r11_compania = vg_codcia
		  AND r11_bodega   = bodega
		  AND r11_item     = item
	IF STATUS <> 0 THEN
		ROLLBACK WORK
		DISPLAY '    ERROR AL ACTUALIZAR STOCK DEL ITEM: ',item CLIPPED,
			'. PROCESADO ABORTADO OK.'
		WHENEVER ERROR STOP
		EXIT PROGRAM
	END IF
	WHENEVER ERROR STOP
	DISPLAY '  ACTUALIZADO STOCK ITEM: ', item CLIPPED, ' Bod. ',
		bodega USING "&&", ' con saldo: ', saldo USING "---,--&.##",
		'. OK'
	DISPLAY ' '
	LET i = i + 1
	LET j = j + 1
