...
select r19_tipo_dev, r19_num_dev, r20_cod_tran cod_tran, r20_num_tran num_tran,
	...
	into temp t2;
drop table t_bd;
select unique r19_tipo_dev, r19_num_dev, cod_tran, num_tran
	from t2
	where r19_tipo_dev = 'DF'
	into temp t_cont_df;
select r19_tipo_dev, r19_num_dev, count(*) hay_df_mas_1tr
	from t_cont_df
	group by 1, 2
	having count(*) > 1
	order by 3 desc;
drop table t_cont_df;
...
drop table t4;
drop table t5;
select r20_num_tran, count(*) tot_df from t6 group by 1 into temp t_tot;
