--FALTA INCLUIR LA ACTUALIZACION DEL CODIGO DE PROVEEDOR
DATABASE aceros
...
MAIN
	...
	IF num_args() <> 6 AND num_args() <> 7 AND num_args() <> 8 THEN
...
IF orden > 0 THEN
	...
	IF cod_prov IS NOT NULL THEN
		UPDATE ordt010
			SET c10_codprov = cod_prov
			WHERE c10_compania  = codcia
			  AND c10_localidad = codloc
			  AND c10_numero_oc = orden
	END IF
