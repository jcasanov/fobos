...
CALL eliminar_trans_activo_fijo() RETURNING tip_comp, num_comp
IF NOT eliminar_oc_composicion() THEN
	ROLLBACK WORK
	EXIT PROGRAM
END IF
COMMIT WORK
	...
	AFTER FIELD c13_numero_oc
		IF rm_c13.c13_numero_oc IS NOT NULL THEN
			...
			IF r_c01.c01_modulo = 'CI' THEN
				SELECT COUNT(*)
					INTO cuantos
					FROM rept049, rept048
					WHERE r49_compania    = vg_codcia
					  AND r49_localidad   = vg_codloc
					  AND r49_numero_oc   =
							rm_c13.c13_numero_oc
					  AND r48_compania    = r49_compania
					  AND r48_localidad   = r49_localidad
					  AND r48_composicion = r49_composicion
					  AND r48_item_comp   = r49_item_comp
					  AND r48_sec_carga   = r49_sec_carga
					  AND r48_estado      = 'C'
				IF cuantos > 0 THEN
					CALL fl_mostrar_mensaje('No puede anular esta orden de compra, debido a que pertenece a una composición de ítem que ha sido ingresada al INVENTARIO.', 'exclamation')
                       			NEXT FIELD c13_numero_oc
				END IF
			END IF
			...
			IF r_c10.c10_ord_trabajo IS NOT NULL THEN
				...
				IF r_t23.t23_estado <> 'A' THEN
					CALL fl_mostrar_mensaje('La orden de compra esta asociada a una orden de trabajo que no esta activa.','exclamation')
                       			NEXT FIELD c13_numero_oc
				END IF
			END IF
...
IF r_c01.c01_modulo IS NULL THEN
...
FUNCTION eliminar_oc_composicion()
DEFINE cuantos		INTEGER
...
SELECT COUNT(*)
	...
	WHERE r49_compania  = vg_codcia
	  AND r49_localidad = vg_codloc
	  AND r49_numero_oc = rm_c13.c13_numero_oc
IF cuantos = 0 THEN
	RETURN 1
END IF
WHENEVER ERROR CONTINUE
...
IF STATUS <> 0 THEN
	WHENEVER ERROR STOP
	CALL fl_mostrar_mensaje('Ha ocurrido un error al intentar borrar la lista de ordenes de compra de la composición de ítems. Por favor llame al ADMINISTRADOR.', 'stop')
	RETURN 0
END IF
WHENEVER ERROR STOP
RETURN 1
