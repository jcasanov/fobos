--------------------------------------------------------------------------------
-- Titulo           : ordp103.4gl - Cambio de precios masivo
-- Elaboracion      : 26-Jul-2017
-- Autor            : NPC
-- Formato Ejecucion: fglrun ordp103 base modulo compania [codigo] [flag]
-- Ultima Correccion: 
-- Motivo Correccion: 
--------------------------------------------------------------------------------
...
ELSE
	OPEN FORM f_ordf103_1 FROM '../forms/ordf103_1c'
END IF
...
IF vg_gui = 0 THEN
	...
	LET num_cols = 78
END IF
OPEN WINDOW w_ordf103_3 AT row_ini, 04 WITH num_rows ROWS, num_cols COLUMNS
	ATTRIBUTE(FORM LINE FIRST, COMMENT LINE LAST, MENU LINE lin_menu,
		  BORDER)
IF vg_gui = 1 THEN
	OPEN FORM f_ordf103_3 FROM '../forms/ordf103_3'
ELSE
	OPEN FORM f_ordf103_3 FROM '../forms/ordf103_3c'
END IF
DISPLAY FORM f_ordf103_3
LET vm_num_det = 0
...
WHILE TRUE
	...
	INPUT ARRAY r_det_arch WITHOUT DEFAULTS FROM r_det_arch.*
		...
		AFTER INPUT
			IF NOT valida_item_comp(i) THEN
				CONTINUE INPUT
			END IF
			LET salir = 1
	END INPUT
...
FOR i = 1 TO lim
	...
	IF r_r48.r48_compania IS NOT NULL THEN
		IF (r_r10.r10_costo_mb + r_r48.r48_costo_mo) >=
		    r_det_arch[i].r86_precio_nue
		THEN
			LET mensaje = 'El Precio Nuevo del ítem ',
					r_r10.r10_codigo CLIPPED,
					' esta por debajo, del costo promedio',
					' mas la mano de obra.'
			CALL fl_mostrar_mensaje(mensaje, 'exclamation')
			LET resul = 0
			EXIT FOR
...
OPEN WINDOW w_detalle AT row_ini, 03 WITH num_rows ROWS, num_cols COLUMNS
	ATTRIBUTE(FORM LINE FIRST, COMMENT LINE LAST, MENU LINE lin_menu,
		  BORDER)
IF vg_gui = 1 THEN
	OPEN FORM f_ordf103_2 FROM '../forms/ordf103_2'
ELSE
	OPEN FORM f_ordf103_2 FROM '../forms/ordf103_2c'
END IF
DISPLAY FORM f_ordf103_2
...
IF vg_gui = 0 THEN
	...
	LET num_cols = 78
END IF
OPEN WINDOW w_ordf103_4 AT row_ini, 04 WITH num_rows ROWS, num_cols COLUMNS
	ATTRIBUTE(FORM LINE FIRST, COMMENT LINE LAST, MENU LINE lin_menu,
		  BORDER)
IF vg_gui = 1 THEN
	OPEN FORM f_ordf103_4 FROM '../forms/ordf103_4'
ELSE
	OPEN FORM f_ordf103_4 FROM '../forms/ordf103_4c'
END IF
DISPLAY FORM f_ordf103_4
LET vm_num_det = 0
CALL mostrar_botones_detalle3()
DISPLAY BY NAME novedades
CALL muestra_detalle_nov()
LET int_flag = 0
CLOSE WINDOW w_ordf103_4
RETURN
