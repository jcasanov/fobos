...
IF diferencia <= tope THEN
	...
	IF vm_tipo_doc = 'RT' THEN
		SELECT * INTO r_p04.* FROM cxpt004
			WHERE p04_tipo_doc = vm_tipo_doc
	...
	DISPLAY	r_g02.g02_abreviacion CLIPPED, ' - ', r_z04.z04_nombre CLIPPED,
		...
		'   SERIE: ', num_ini CLIPPED, ' AL ', num_fin CLIPPED
END IF
...
CASE vm_tipo_doc
	WHEN 'FA'
		CALL ejecuta_query_principal('TA', 1)
		CALL ejecuta_query_principal('RE', 1)
	WHEN 'NV'
		CALL ejecuta_query_principal('TA', 2)
		CALL ejecuta_query_principal('RE', 2)
	WHEN 'NC'
		CALL ejecuta_query_principal('XX', 0)
	WHEN 'ND'
		CALL ejecuta_query_principal('XX', 0)
	WHEN 'RT'
		CALL ejecuta_query_principal('XX', 0)
...
CASE modulo
	...
	WHEN 'RE'
		...
		LET expr_fact = '   AND ', prefijo, '_cod_tran  = "FA"',
				'   AND LENGTH(', prefijo, '_cedruc) = ', num
END CASE
CASE vm_tipo_doc
	WHEN 'NC'
		LET prefijo   = 'z21'
