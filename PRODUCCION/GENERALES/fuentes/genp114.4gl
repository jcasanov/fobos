...
-- Autor                : GVA
-- Formato de Ejecución : fglrun  Genp114 base GE 
...
OPEN WINDOW w_pra AT 3,2 WITH 14 ROWS, 80 COLUMNS
...
CONSTRUCT BY NAME expr_sql ON g16_partida, g16_nombre, g16_porcentaje, 
			      g16_usuario, g16_fecing
	ON KEY(F2)
		IF INFIELD(g16_partida) THEN
			CALL fl_ayuda_partidas()RETURNING codigo
			IF codigo IS NOT NULL THEN
				LET rm_pra.g16_partida = codigo
...
IF int_flag THEN
	...
	RETURN
END IF
LET query = 'SELECT *, ROWID FROM gent016 WHERE ', expr_sql CLIPPED, 
		' ORDER BY 2'
...
BEGIN WORK
DECLARE q_up CURSOR FOR SELECT * FROM gent016 WHERE ROWID = vm_r_rows[vm_row_current]
	FOR UPDATE
OPEN q_up
...
OPTIONS INPUT WRAP
LET int_flag = 0
INPUT BY NAME rm_pra.g16_partida, rm_pra.g16_nombre, rm_pra.g16_porcentaje
	      WITHOUT DEFAULTS
	ON KEY(INTERRUPT)
        	 IF field_touched(rm_pra.g16_partida, rm_pra.g16_nombre, 
				  rm_pra.g16_porcentaje)
                 THEN
                        LET int_flag = 0
	...
	AFTER FIELD g16_partida
		...
		IF rm_pra.g16_partida IS NOT NULL THEN
		      CALL fl_lee_partida(rm_pra.g16_partida)RETURNING rm_pra2.*
		      IF rm_pra2.g16_partida IS NOT NULL THEN
                           CALL fgl_winmessage(vg_producto,'Ya existe la partida arancelaria','exclamation')
...
IF vm_num_rows <= 0 THEN
	RETURN
END IF
SELECT * INTO rm_pra.* FROM gent016 WHERE ROWID = num_row
