...
INPUT BY NAME rm_r19.r19_cod_tran, rm_r19.r19_tipo_dev, rm_r19.r19_num_dev, 
	...
	AFTER FIELD r19_num_dev
		...
		IF items_alterados(r_r19.*) THEN
			--CALL fgl_winmessage(vg_producto,'Algunos items de esta compra local han sido afectados por otra transacción y no se podrá realizar la devolución.','exclamation')
			CALL fl_mostrar_mensaje('Algunos items de esta compra local tienen stock insuficiente o su costo promedio ha variado por otra transacción y no se podrá realizar la devolución.','exclamation')
			INITIALIZE r_r19.* TO NULL
			CALL muestra_etiquetas(r_r19.*)
			NEXT FIELD r19_num_dev
		END IF
