...
DEFINE rm_cxp4		RECORD LIKE cxpt020.*
...
INITIALIZE rm_cxp.*, rm_cxp2.*, rm_cxp3, rm_cxp4.* TO NULL
FOR indice = 1 TO 10
	LET rm_orden[indice] = '' 
END FOR
LET vm_num_rows    = 0
...
INITIALIZE rm_cxp.*, rm_cxp2.*, rm_cxp3, rm_cxp4.*, r_cxp.*, r_cxp3.*,
...
LET rm_cxp.p22_tipo_trn   = vm_tipo_trn
...
IF NOT int_flag THEN
	...
	DECLARE q_aplic CURSOR FOR SELECT * FROM cxpt021
		WHERE p21_compania  = vg_codcia
		  ...
		  AND p21_codprov   = rm_cxp.p22_codprov
		  AND p21_tipo_doc  = rm_cxp3.p21_tipo_doc
		  AND p21_num_doc   = rm_cxp3.p21_num_doc
		FOR UPDATE
	OPEN q_aplic
	...
	IF NOT int_flag AND vm_num_elm > 0 THEN
		...
		FOR i = 1 TO vm_num_elm
			...
			END IF
			IF valor_aux >
			rm_cxp4.p20_saldo_cap + rm_cxp4.p20_saldo_int THEN
				ROLLBACK WORK
			...
			INSERT INTO cxpt023 VALUES(rm_cxp2.p23_compania,
				...
				rm_cxp3.p21_tipo_doc, rm_cxp3.p21_num_doc,
				rm_cxp2.p23_valor_cap, rm_cxp2.p23_valor_int,
				rm_cxp2.p23_valor_mora, rm_cxp4.p20_saldo_cap,
				rm_cxp4.p20_saldo_int)
			UPDATE cxpt020
		...
		FREE q_up2
		WHENEVER ERROR CONTINUE
		DECLARE q_up3 CURSOR FOR SELECT * FROM cxpt021
		 	    WHERE p21_compania  = vg_codcia
			      AND p21_localidad = vg_codloc
			      AND p21_codprov   = rm_cxp.p22_codprov
			      AND p21_tipo_doc  = rm_cxp3.p21_tipo_doc
		...
		END IF
		WHENEVER ERROR STOP
		UPDATE cxpt021 SET p21_saldo = rm_cxp3.p21_saldo
...
LET int_flag = 0
INPUT BY NAME rm_cxp.p22_codprov, rm_cxp.p22_moneda, rm_cxp3.p21_tipo_doc,
	rm_cxp3.p21_num_doc, rm_cxp.p22_referencia
	WITHOUT DEFAULTS
	ON KEY(INTERRUPT)
	        IF field_touched(rm_cxp.p22_codprov, rm_cxp.p22_moneda,
			rm_cxp3.p21_tipo_doc, rm_cxp3.p21_num_doc,
	...
	ON KEY(F2)
		...
		IF infield(p21_tipo_doc) THEN
                        ...
                        LET int_flag = 0
                        IF codt_aux IS NOT NULL THEN
                                LET rm_cxp3.p21_tipo_doc = codt_aux
                                DISPLAY BY NAME rm_cxp3.p21_tipo_doc
                                DISPLAY nomt_aux TO tit_tipo_doc
                        END IF
                END IF
		IF infield(p21_num_doc) THEN
			CALL fl_ayuda_doc_favor_tes(vg_codcia, vg_codloc,
					rm_cxp.p22_codprov,rm_cxp3.p21_tipo_doc)
				RETURNING nom_aux, r_cxp.p21_tipo_doc,
					r_cxp.p21_num_doc, r_cxp.p21_saldo,
					r_cxp.p21_moneda
			LET int_flag = 0
			IF r_cxp.p21_num_doc IS NOT NULL THEN
				LET rm_cxp3.p21_num_doc = r_cxp.p21_num_doc
				LET rm_cxp3.p21_saldo = r_cxp.p21_saldo
				DISPLAY BY NAME rm_cxp3.p21_num_doc
				DISPLAY r_cxp.p21_saldo TO p21_saldo
			END IF 
		END IF
	...
	AFTER FIELD p21_tipo_doc 
		IF rm_cxp3.p21_tipo_doc IS NOT NULL THEN
			CALL fl_lee_tipo_doc_tesoreria(rm_cxp3.p21_tipo_doc)
				RETURNING r_tip.* 
	...
	INPUT ARRAY rm_aju WITHOUT DEFAULTS FROM rm_aju.*
		...
		AFTER INPUT
			...
			IF valor_aux > vm_saldo THEN
				...
				NEXT FIELD tit_valor_apl
			END IF
			IF rm_cxp3.p21_saldo > 0 THEN
				LET rm_cxp3.p21_saldo = rm_cxp3.p21_saldo
							- valor_aux
			END IF
			LET salir = 1
	END INPUT
...
FOR i = 1 TO vm_num_elm
	IF NOT check THEN
		IF rm_cxp3.p21_saldo > 0 THEN
			IF rm_cxp3.p21_saldo > rm_aju[i].tit_saldo_act THEN
				LET rm_cxp3.p21_saldo = rm_cxp3.p21_saldo
						+ rm_aju[i].tit_saldo_act * (-1)
				LET rm_aju[i].tit_valor_apl =
						rm_aju[i].tit_saldo_act * (-1)
			ELSE
				LET rm_aju[i].tit_valor_apl = rm_cxp3.p21_saldo
								* (-1)
				LET rm_cxp3.p21_saldo = 0
			END IF
		...
		END IF
	ELSE
		LET rm_cxp3.p21_saldo       = vm_saldo
		LET rm_aju[i].tit_valor_apl = 0
...
IF vm_num_rows > 0 THEN
	DECLARE q_dt CURSOR FOR SELECT cxpt022.*, cxpt023.*, cxpt021.*
		...
		WHERE cxpt022.ROWID = num_registro
                  ...
                  AND p21_num_doc   = p23_doc_favor
        OPEN q_dt
        FETCH q_dt INTO rm_cxp.*, rm_cxp2.*, rm_cxp3.*
	IF STATUS = NOTFOUND THEN
		CALL fgl_winmessage (vg_producto,'No existe registro con índice: ' || vm_row_current,'exclamation')
		RETURN
	END IF
	DISPLAY BY NAME	rm_cxp.p22_tipo_trn, rm_cxp.p22_num_trn,
			rm_cxp.p22_codprov, rm_cxp.p22_moneda,
			rm_cxp.p22_referencia, rm_cxp3.p21_tipo_doc,
			rm_cxp3.p21_num_doc, rm_cxp.p22_usuario,
			rm_cxp.p22_fecing, rm_cxp3.p21_saldo
	CALL fl_lee_proveedor(rm_cxp.p22_codprov) RETURNING r_pro_gen.*
	DISPLAY r_pro_gen.p01_nomprov TO tit_nombre_pro
	CALL fl_lee_tipo_doc_tesoreria(rm_cxp3.p21_tipo_doc) RETURNING r_tip.* 
	DISPLAY r_tip.p04_nombre TO tit_tipo_doc
	CALL fl_lee_moneda(rm_cxp.p22_moneda) RETURNING r_mon.* 
	DISPLAY r_mon.g13_nombre TO tit_mon_bas
