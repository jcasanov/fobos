...
DEFER INTERRUPT
CLEAR SCREEN
CALL startlog('../logs/cxpp206.error')
CALL fgl_init4js()
...
DEFINE benef_che	LIKE ctbt012.b12_benef_che
...
LET int_flag = 0
CALL lee_beneficiario(rm_prov.p01_nomprov) RETURNING benef_che
IF int_flag THEN
	ROLLBACK WORK
	RETURN
END IF	
...
LET rm_ccomp.b12_benef_che   = benef_che 
...
IF int_flag THEN
	...
	RETURN
END IF
CALL fl_lee_actualiza_cheque_cta_cte(rm_ordp.p24_compania, 
									 rm_ordp.p24_banco,
									 rm_ordp.p24_numero_cta)
	RETURNING rm_ccomp.b12_num_cheque								 
IF rm_ccomp.b12_num_cheque = -1 THEN
	CLEAR FORM
	ROLLBACK WORK
	CALL muestra_titulos()
	RETURN
...
FUNCTION inserta_cheque(valor)
DEFINE valor			LIKE ctbt013.b13_valor_base
DEFINE r_p100			RECORD LIKE cxpt100.*

INITIALIZE r_p100.* TO NULL
LET r_p100.p100_compania   = vg_codcia
LET r_p100.p100_tipo_comp  = rm_ccomp.b12_tipo_comp
LET r_p100.p100_num_comp   = rm_ccomp.b12_num_comp
LET r_p100.p100_banco      = rm_ordp.p24_banco
LET r_p100.p100_numero_cta = rm_ordp.p24_numero_cta
LET r_p100.p100_num_cheque = rm_ccomp.b12_num_cheque
LET r_p100.p100_benef_che  = rm_ccomp.b12_benef_che
LET r_p100.p100_moneda     = rm_ccomp.b12_moneda   
LET r_p100.p100_paridad    = rm_ccomp.b12_paridad   
LET r_p100.p100_valor      = valor 
LET r_p100.p100_usuario    = vg_usuario
LET r_p100.p100_fecing	   = CURRENT

INSERT INTO cxpt100 VALUES (r_p100.*)
...
FUNCTION lee_beneficiario(b12_benef_che)
DEFINE b12_benef_che		LIKE ctbt012.b12_benef_che

LET int_flag = 0
OPTIONS INPUT NO WRAP
INPUT BY NAME b12_benef_che WITHOUT DEFAULTS

RETURN b12_benef_che
...
IF rm_ordp.p24_total_cap + rm_ordp.p24_total_int + rm_ordp.p24_total_ret +
	...
	RETURN 0
END IF
IF rm_ordp.p24_medio_pago <> 'C' THEN
	CALL fgl_winmessage(vg_producto, 'La orden de pago no se cancela con cheque.', icono)
	RETURN 0
END IF
...
LET comando = 'cd ..', vg_separador, '..', vg_separador,
	      'TESORERIA', vg_separador, 'fuentes', 
	      vg_separador, '; fglrun cxpp404 ', vg_base, ' ',
	      'TE', vg_codcia, ' ', vg_codloc, ' ', rm_ccomp.b12_tipo_comp, 
	      ' ', rm_ccomp.b12_num_comp
