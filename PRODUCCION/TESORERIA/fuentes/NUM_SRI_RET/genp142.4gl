...
LET query = 'SELECT *, ROWID FROM gent037 ',
		...
		' ORDER BY g37_tipo_doc, g37_secuencia, g37_fecha_emi'
...
INPUT BY NAME rm_g37.g37_localidad, rm_g37.g37_tipo_doc, rm_g37.g37_cont_cred,
	...
	AFTER INPUT
		...
		IF rm_g37.g37_cont_cred <> 'N' THEN
			IF rm_g37.g37_tipo_doc <> 'FA' THEN
				CALL fl_lee_tipo_doc('FA') RETURNING r_z04.* 
				CALL fl_mostrar_mensaje('La Forma de Pago solo es utilizado en '|| r_z04.z04_nombre CLIPPED || '.', 'info')
				LET rm_g37.g37_cont_cred = 'N'
...
SELECT NVL(MAX(g37_secuencia), 0) + 1 INTO secuencia
	FROM gent037
...
IF r_g37_a.g37_compania IS NOT NULL THEN
	IF rm_g37.g37_fecha_emi <= r_g37_a.g37_fecha_exp THEN
		...
		END IF
		CALL fl_mostrar_mensaje('La Fecha de Emision no puede ser menor o igual que la fecha de expiracion del ultimo registro de ' || r_z04.z04_nombre CLIPPED || '.', 'exclamation')
