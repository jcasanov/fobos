...
INPUT BY NAME rm_par.*
	...
	AFTER FIELD fecha_emi
		IF rm_par.fecha_emi IS NOT NULL THEN
			...
			END IF
		END IF
	AFTER FIELD fecha_emi1
		IF rm_par.fecha_emi1 IS NOT NULL THEN
			IF rm_par.fecha_emi1 <= vm_fecha_ini THEN
				CALL fl_mostrar_mensaje('La Fecha de Emision Inicial no puede ser menor a la Fecha de Inicio de las TESORERIA en el FOBOS.', 'exclamation')
				NEXT FIELD fecha_emi1
			END IF
		END IF
	AFTER FIELD fecha_emi2
		IF rm_par.fecha_emi2 IS NOT NULL THEN
			IF rm_par.fecha_emi2 <= vm_fecha_ini THEN
				CALL fl_mostrar_mensaje('La Fecha de Emision Inicial no puede ser menor a la Fecha de Inicio de las TESORERIA en el FOBOS.', 'exclamation')
				NEXT FIELD fecha_emi2
			END IF
		END IF
	...
	AFTER INPUT 
		...
		IF rm_par.fecha_emi IS NOT NULL THEN
			...
			END IF
		END IF
		IF rm_par.fecha_emi1 IS NULL AND rm_par.fecha_emi2 IS NOT NULL
		THEN
			CALL fl_mostrar_mensaje('Digite fecha de emision inicial.', 'exclamation')
			NEXT FIELD fecha_emi1
		END IF
		IF rm_par.fecha_emi1 IS NOT NULL AND rm_par.fecha_emi2 IS NULL
		THEN
			CALL fl_mostrar_mensaje('Digite fecha de emision final.', 'exclamation')
			NEXT FIELD fecha_emi2
		END IF
		...
		END IF
		IF rm_par.fecha_emi1 IS NOT NULL THEN
			IF rm_par.fecha_emi1 >= rm_par.fecha_emi2 THEN
				CALL fl_mostrar_mensaje('La fecha de emision inicial debe ser menor que la fecha de emision final.', 'exclamation')
				NEXT FIELD fecha_emi1
			END IF
		END IF
		IF rm_par.fecha_vcto1 IS NOT NULL THEN
		...
		IF rm_par.fecha_emi IS NOT NULL THEN
			IF rm_par.fecha_emi >= rm_par.fecha_vcto1 THEN
				CALL fl_mostrar_mensaje('La fecha de emision general debe ser menor que la fecha de vencimiento inicial.', 'exclamation')
				NEXT FIELD fecha_emi
			END IF
			IF rm_par.fecha_emi1 IS NOT NULL THEN
				CALL fl_mostrar_mensaje('No se puede poner rango de fecha de emision, cuando el campo "Cartera Del" tiene una fecha de emision inicial.', 'info')
				LET rm_par.fecha_emi1 = NULL
				LET rm_par.fecha_emi2 = NULL
				DISPLAY BY NAME rm_par.fecha_emi1,
						rm_par.fecha_emi2
			END IF
		END IF
		IF rm_par.ind_doc = 'F' THEN
...
LET query = 'INSERT INTO tempo_doc ',
		'SELECT p20_tipo_doc, p20_num_doc, p20_dividendo,',
			...
			' END) saldo_mov, ',
			' p20_numero_oc, p20_localidad, p04_tipo ',
...
IF rm_par.fecha_emi1 IS NOT NULL THEN
	LET query = 'SELECT * FROM tempo_doc ',
			' WHERE fecha_emi BETWEEN "', rm_par.fecha_emi1,
					   '" AND "', rm_par.fecha_emi2, '"',
			' INTO TEMP t1'
	PREPARE cons_emi FROM query
	EXECUTE cons_emi
...
WHILE TRUE
	...
	FOREACH q_cond INTO rm_doc[i].*, r_aux[i].* 
		...
		IF r_aux[i].tipo = 'D' THEN
			LET lim              = LENGTH(rm_doc[i].numdoc)
			LET ini              = 2
			IF lim = 1 THEN
				LET ini = 1
			END IF
			LET rm_doc[i].numdoc =rm_doc[i].numdoc[ini,lim] CLIPPED,
						'-',
...
RUN "mv ../../../tmp/cxpp315.unl $HOME/tmp/"
LET mensaje = FGL_GETENV("HOME"), '/tmp/cxpp315.unl'
...
PAGE HEADER
	...
	IF rm_par.fecha_emi1 IS NOT NULL THEN
		PRINT COLUMN 015, "** RANGO DE EMISION  : ",
			rm_par.fecha_emi1 USING 'dd-mm-yyyy', ' - ',
			rm_par.fecha_emi2 USING 'dd-mm-yyyy'
	END IF
	IF rm_par.fecha_vcto1 IS NOT NULL THEN
