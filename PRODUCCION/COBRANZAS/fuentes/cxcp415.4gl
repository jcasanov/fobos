{*
 * Titulo           : cxcp415.4gl - Valores de soliciud de cobro por recaudar
 * Elaboracion      : 24-sep-2008
 * Autor            : JCM
...
DEFER INTERRUPT
CLEAR SCREEN
CALL startlog('../logs/cxcp415.error')
CALL fgl_init4js()
...
OPTIONS INPUT WRAP,
	ACCEPT KEY	F12
OPEN FORM f_rep FROM "../forms/cxcf415_1"
DISPLAY FORM f_rep
...
WHILE TRUE
	...
	IF rm_par.f_pago IS NOT NULL THEN
		LET expr_pago = ' AND z101_codigo_pago = "', rm_par.f_pago, '"'
	END IF
	...
	LET query = 'SELECT DATE(j10_fecha_pro), g03_nombre, ',
		          ...
		          ' j10_nomcli, j10_tipo_destino, j10_num_destino,',
                          ' j10_moneda, j10_valor, z101_codigo_pago, ',
		          ' z101_cod_bco_tarj, z101_num_ch_aut, z101_num_cta_tarj, ',
		          ' z101_moneda, z101_valor ',
		      ' FROM cajt010, cxct101, OUTER gent003 ',
		      ' WHERE j10_compania    = ', vg_codcia, 
		        ...
		        ' AND z101_compania    = j10_compania ',
		        ' AND z101_localidad   = j10_localidad ',
		        ' AND z101_numero_sol  = j10_num_fuente ', 
				expr_pago CLIPPED,
		        ' ORDER BY 2, 1, 4, 5'
...
PAGE HEADER
	...
	CALL fl_justifica_titulo('I', 'VALORES POR RECAUDAR EN CAJA', 80)
		RETURNING titulo
