--------------------------------------------------------------------------------
-- Titulo           : cxcp403.4gl - Impresion Nota de Credito
-- Elaboracion      : 07-oct-2004
-- Autor            : JCM
-- Formato Ejecucion: fglrun cxcp403 BD MODULO COMPANIA LOCALIDAD NUMDEV
-- Ultima Correccion: 
-- Motivo Correccion:
--------------------------------------------------------------------------------
...
IF rm_r19.r19_num_tran IS NULL THEN	
	CALL FGL_WINMESSAGE(vg_producto,'No existe devolucion.','stop')
	EXIT PROGRAM
END IF
...
--	print '&k2S' 		-- Letra condensada
	...
	PRINT COLUMN 12, fl_justifica_titulo('I', '(' || rm_r19.r19_num_dev || ')' CLIPPED, 15),
	      COLUMN 70, DATE(rm_r19.r19_fecing) USING 'dd-mm-yyyy', 1 SPACES, TIME 
	PRINT COLUMN 12, rm_r19.r19_nomcli CLIPPED,
	      COLUMN 70, rm_r19.r19_cedruc 
	PRINT COLUMN 12, rm_r19.r19_dircli CLIPPED
	PRINT COLUMN 12, rm_r19.r19_telcli CLIPPED,
	      COLUMN 70, rm_r19.r19_vendedor
	PRINT COLUMN 12, forma_pago CLIPPED
	PRINT COLUMN 12, rm_r19.r19_oc_externa CLIPPED

	SKIP 4 LINES

ON EVERY ROW
	PRINT COLUMN 5,   cod CLIPPED, 
	      COLUMN 22,  descrip CLIPPED,
	      COLUMN 56,  cant     USING '####',
	      COLUMN 62,  precio   USING "###,###,##&.##",
	      COLUMN 76,  subtotal USING "###,###,##&.##"
...
ON LAST ROW
	...
	PRINT COLUMN 78, rm_r19.r19_tot_bruto USING '###,###,##&.##'
	SKIP 1 LINES
	PRINT COLUMN 78, rm_r19.r19_tot_dscto USING '###,###,##&.##'
	PRINT COLUMN 35,  vm_vendedor CLIPPED,
	      COLUMN 78, impuesto             USING '###,###,##&.##'
	IF rm_r19.r19_ped_cliente IS NOT NULL THEN
		PRINT COLUMN 10,  'Nota: ', fl_justifica_titulo('I', rm_r19.r19_ped_cliente, 10) CLIPPED,
		      COLUMN 78, rm_r19.r19_flete     USING '###,###,##&.##'
	ELSE
		PRINT COLUMN 78, rm_r19.r19_flete     USING '###,###,##&.##'
	END IF
	PRINT COLUMN 78, rm_r19.r19_tot_neto  USING '###,###,##&.##'--, 'E'
