--------------------------------------------------------------------------------
-- Titulo           : cxcp666.4gl - Proceso Digitacion Retenciones de Facturas
-- Elaboracion      : 24-Oct-2008
-- Autor            : NPC
-- Formato Ejecucion: fglrun cxcp666 base modulo compañía localidad
-- Ultima Correccion: 
-- Motivo Correccion: 
--------------------------------------------------------------------------------
...
DEFER INTERRUPT
CLEAR SCREEN
CALL startlog('../logs/cxcp666.err')
--#CALL fgl_init4js()
...
IF vg_gui = 0 THEN
	...
	LET num_cols = 78
END IF
OPEN WINDOW w_cxcf666_1 AT row_ini, 02 WITH num_rows ROWS, num_cols COLUMNS
	ATTRIBUTE(FORM LINE FIRST, COMMENT LINE LAST, MENU LINE lin_menu,
		  BORDER, MESSAGE LINE LAST) 
IF vg_gui = 1 THEN
	OPEN FORM f_cxcf666_1 FROM '../forms/cxcf666_1'
ELSE
	OPEN FORM f_cxcf666_1 FROM '../forms/cxcf666_1c'
END IF
DISPLAY FORM f_cxcf666_1
LET vm_cod_tran = 'FA'
LET vm_max_rows = 20000
LET dias_tope   = 592
LET vm_fec_ini  = MDY(01, 01, 2007)
LET vm_fec_fin  = MDY(08, 15, 2008)
...
LET rm_par.fecha_ini  = MDY(01, 01, YEAR(rm_par.fecha_fin))
...
INPUT BY NAME rm_par.*
	...
	BEFORE FIELD fecha_ini
		LET fec_i = rm_par.fecha_ini
	BEFORE FIELD fecha_fin
		LET fec_f = rm_par.fecha_fin
	...
	AFTER FIELD fecha_ini
		IF rm_par.fecha_ini IS NULL THEN
			LET rm_par.fecha_ini = fec_i
			DISPLAY BY NAME rm_par.fecha_ini
		END IF
		IF rm_par.fecha_ini < vm_fec_ini AND
		   rm_par.fecha_ini > vm_fec_fin
		THEN
			LET mensaje = 'La fecha inicial tiene que ser una ',
					'fecha entre: ',
					vm_fec_ini USING "dd-mm-yyyy", ' y ',
					vm_fec_fin USING "dd-mm-yyyy", '.'
			CALL fl_mostrar_mensaje(mensaje, 'exclamation')
			NEXT FIELD fecha_ini
		END IF
	AFTER FIELD fecha_fin
		IF rm_par.fecha_fin IS NULL THEN
			LET rm_par.fecha_fin = fec_f
			DISPLAY BY NAME rm_par.fecha_fin
		END IF
		IF rm_par.fecha_fin < vm_fec_ini AND
		   rm_par.fecha_fin > vm_fec_fin
		THEN
			LET mensaje = 'La fecha final tiene que ser una ',
					'fecha entre: ',
					vm_fec_ini USING "dd-mm-yyyy", ' y ',
					vm_fec_fin USING "dd-mm-yyyy", '.'
			CALL fl_mostrar_mensaje(mensaje, 'exclamation')
...
IF rm_j02.j02_solicitudes = 'N' THEN
	LET rm_par.tipo_venta = 'C'
END IF
IF (rm_j02.j02_aux_cont IS NULL) AND (vg_codloc = 2 OR vg_codloc = 4) THEN
	LET rm_par.tipo_venta = 'C'
END IF
--}
DISPLAY BY NAME rm_par.tipo_venta
...
LET query = 'SELECT * FROM cxct020 ',
		...
		'   AND EXTEND(z20_fecha_emi, YEAR TO MONTH) BETWEEN ',
					'DATE("', rm_par.fecha_ini, '")',
...
IF rm_par.tipo_venta = 'R' OR rm_par.tipo_venta = 'T' THEN
	LET query = ' INSERT INTO tmp_det ',
		...
		' FROM tmp_z20, ', retorna_base_loc() CLIPPED, 'rept038 ',
...
END IF
DROP TABLE tmp_z20
LET query = 'SELECT * FROM cajt010, cajt011 ',
		...
		'   AND EXTEND(j10_fecha_pro, YEAR TO MONTH) BETWEEN ',
					'DATE("', rm_par.fecha_ini, '")',
					' AND DATE("', rm_par.fecha_fin, '")',
...
LET query = 'INSERT INTO tmp_det ',
	'SELECT UNIQUE j10_localidad z20_localidad, "FA" z20_tipo_doc, ',
		...
		' FROM tmp_j10, rept038 ',
		' WHERE r38_compania        = j10_compania ',
...
IF registros_retenciones(numero_ret, i) = 0 THEN
	...
	LET query = 'SELECT "", "", "", j91_codigo_pago, z08_tipo_ret,',
			' z08_porcentaje, "", "",',
			' CASE WHEN "', tipo_f, '" = "PR" THEN',
...
INPUT BY NAME rm_j14.j14_num_ret_sri, rm_j14.j14_autorizacion,
	...
	AFTER FIELD j14_fecha_emi
		...
		IF NOT cambi THEN
			...
			CONTINUE INPUT
		END IF
		LET fecha_min  = vm_fec_ini
		LET fecha_tope = vm_fec_fin
		...
		IF (MDY(MONTH(fecha_min), 01, YEAR(fecha_min)) + 1 UNITS MONTH
			- 1 UNITS DAY) < (fecha_tope - (dias_tope+ 1) UNITS DAY)
		THEN
...
IF total <= 0 THEN
	...
	RETURN
END IF
SELECT * FROM tmp_ret INTO TEMP t1
{--
SELECT * FROM tmp_ret
	WHERE EXISTS
...
BEGIN WORK
	...
	FOREACH q_ret_doc INTO num_ret_s
		IF rm_par.devuelve = 'S' AND tipo = 'A' THEN
			IF NOT genera_documento_deudor(num_ret_s) THEN
				DROP TABLE tmp_doc
				WHENEVER ERROR STOP
				ROLLBACK WORK
				RETURN 0
			END IF
		END IF
		IF NOT genera_solicitud_cobro(tipo, num_ret_s) THEN
			DROP TABLE tmp_doc
			WHENEVER ERROR STOP
			ROLLBACK WORK
			RETURN 0
		END IF
		IF NOT genera_forma_pago(num_ret_s) THEN
			DROP TABLE tmp_doc
			WHENEVER ERROR STOP
			ROLLBACK WORK
			RETURN 0
		...
		CALL inserta_fecha_pro(num_ret_s)
		IF rm_par.devuelve = 'S' AND tipo = 'A' THEN
			CALL generar_aplicacion_documentos(num_ret_s)
			...
			{--
			--IF vg_codloc < 3 OR vg_codloc > 5 THEN
				CALL generar_egreso_efectivo_caja(num_ret_s)
...
FUNCTION genera_solicitud_cobro(tipo, num_r)
DEFINE tipo		LIKE cxct024.z24_tipo
DEFINE num_r		VARCHAR(16)
DEFINE r_z24		RECORD LIKE cxct024.*

CALL generar_cabecera_solicitud_cobro(tipo, num_r) RETURNING r_z24.*
IF r_z24.z24_compania IS NULL THEN
	RETURN 0
END IF
IF tipo = 'P' THEN
	IF NOT genera_detalle_solicitud_cobro(r_z24.*, num_r) THEN
		RETURN 0
	END IF
END IF
...
FUNCTION generar_cabecera_solicitud_cobro(tipo, num_r)
DEFINE tipo		LIKE cxct024.z24_tipo
DEFINE num_r		VARCHAR(16)
...
FUNCTION genera_detalle_solicitud_cobro(r_z24, num_r)
DEFINE r_z24		RECORD LIKE cxct024.*
DEFINE num_r		VARCHAR(16)
...
DECLARE q_ret_det_sol CURSOR FOR
	SELECT cod_tr, num_tr
		FROM tmp_ret
		WHERE numero_sol  = r_z24.z24_numero_sol
		  AND num_ret_sri = num_r
LET j = 0
FOREACH q_ret_det_sol INTO cod_tran, num_tran
	FOR i = j + 1 TO vm_num_rows
		IF rm_detalle[i].valor_ret <= 0 OR rm_detalle[i].chequear = 'N'
		THEN
			CONTINUE FOR
		END IF
...
FUNCTION genera_forma_pago(num_ret_s)
DEFINE num_ret_s	CHAR(16)
DEFINE r_j10		RECORD LIKE cajt010.*

CALL graba_detalle_forma_pago(num_ret_s) RETURNING r_j10.*
IF r_j10.j10_compania IS NULL THEN
	RETURN 0
END IF
IF NOT generar_trans_pago(r_j10.*) THEN
	RETURN 0
END IF
RETURN 1
...
FUNCTION graba_detalle_forma_pago(num_ret_s)
DEFINE num_ret_s	CHAR(16)
DEFINE r_j10		RECORD LIKE cajt010.*
...
FOREACH q_for INTO r_j11.j11_num_ch_aut, r_j11.j11_codigo_pago, r_j11.j11_valor
	...
	CALL grabar_detalle_retencion(r_j10.*, r_j11.j11_secuencia,
				r_j11.j11_codigo_pago, r_j11.j11_num_ch_aut)
END FOREACH
RETURN r_j10.*
...
FUNCTION grabar_detalle_retencion(r_j10, secuencia, codigo_pago, num_ret)
DEFINE r_j10		RECORD LIKE cajt010.*
DEFINE secuencia	LIKE cajt011.j11_secuencia
DEFINE codigo_pago	LIKE cajt001.j01_codigo_pago
...
FUNCTION generar_trans_pago(r_j10)
DEFINE r_j10		RECORD LIKE cajt010.*
...
CASE r_z24.z24_tipo
	WHEN 'P'
		CALL genera_ingreso_caja_pg(r_j10.*, r_z24.*) RETURNING r_z22.*
...
FUNCTION genera_ingreso_caja_pg(r_j10, r_z24)
DEFINE r_j10		RECORD LIKE cajt010.*
DEFINE r_z24		RECORD LIKE cxct024.*
DEFINE r_z20		RECORD LIKE cxct020.*
DEFINE r_z22		RECORD LIKE cxct022.*
...
FOREACH q_ddoc INTO r_z25.*
	...
	IF r_z25.z25_valor_int > r_z20.z20_saldo_int THEN
		ROLLBACK WORK
		CALL fl_mostrar_mensaje('Valor interés a pagar mayor que saldo del documento: ' || r_z25.z25_tipo_doc || ' ' || r_z25.z25_num_doc || ' ' || r_z25.z25_dividendo, 'stop')
		EXIT PROGRAM
	END IF
	LET i = i + 1
...
FOREACH q_j14_cr INTO r_j14.*
	CALL lee_factura_inv(r_j14.j14_compania, r_j14.j14_localidad,
				r_j14.j14_cod_tran, r_j14.j14_num_tran)
		RETURNING r_r19.*
	IF r_r19.r19_cont_cred = 'R' THEN
		CONTINUE FOREACH
	END IF
	UPDATE cajt014
		SET j14_cont_cred = r_r19.r19_cont_cred
...
FUNCTION genera_documento_deudor(num_ret_s)
DEFINE num_ret_s	CHAR(16)
DEFINE r_ret		RECORD
				num_ret_sri		CHAR(16),
				autorizacion		VARCHAR(15,10),
...
FOREACH q_di INTO r_ret.*
	...
	LET r_z20.z20_referencia = 'DOC.RT ', num_ret_s CLIPPED,
					' P/FA-SIN SALDO'
	...
	LET r_z20.z20_num_sri    = num_ret_s
...
FUNCTION generar_aplicacion_documentos(num_ret_s)
DEFINE num_ret_s	CHAR(16)
DEFINE r_j10		RECORD LIKE cajt010.*
