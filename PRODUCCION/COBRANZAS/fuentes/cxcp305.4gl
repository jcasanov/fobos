...
--		      fglrun cxcp305 base módulo compañía localidad cliente
--				     moneda
-- Ultima Correccion: 
-- Motivo Correccion: 
------------------------------------------------------------------------------
...
DEFINE rm_par  RECORD
	...
	valor		DECIMAL(12,2)
	END RECORD
DEFINE rm_rows	ARRAY [1000] OF LIKE cxct001.z01_codcli
DEFINE rm_dcli 		ARRAY [1000] OF RECORD
			tit_area	CHAR(15),
...
DEFER INTERRUPT
CLEAR SCREEN
CALL startlog('../logs/cxcp305.error')
CALL fgl_init4js()
...
MENU 'OPCIONES'
	...
	COMMAND KEY('M') 'Movimientos' 'Ver detalle de pagos.'
		IF vm_num_rows > 0 THEN
			CALL mostrar_movimientos_cliente(vg_codcia, vg_codloc,
	...
	COMMAND KEY('T') 'Datos'
		IF vm_row_current > 0 THEN
			LET comando = 'fglrun cxcp101 ', vg_base, ' ',
				vg_modulo, ' ', vg_codcia, ' ', 
	...
	COMMAND KEY('P') 'Ch. Protestados'
		IF vm_row_current > 0 THEN
			CALL mostrar_cheque_protestados(vg_codcia, vg_codloc,
				rm_rows[vm_row_current], rm_par.moneda)
		END IF
	COMMAND KEY('F') 'Ch. Postfechados'
		IF vm_row_current > 0 THEN
			CALL mostrar_cheque_postfechados(vg_codcia, vg_codloc,
				rm_rows[vm_row_current], rm_par.moneda)
		END IF
...
LET int_flag = 0
IF num_args() = 4 THEN
	OPEN WINDOW w_par AT 9,8 WITH FORM "../forms/cxcf305_1"
	...
	INPUT BY NAME rm_par.* WITHOUT DEFAULTS
		...
		AFTER FIELD codcli
			IF rm_par.codcli IS NOT NULL THEN
				...
				CALL fl_lee_cliente_localidad(vg_codcia, vg_codloc, rm_cligen.z01_codcli)
					RETURNING rm_clicia.*
...
IF rm_par.codcli IS NULL THEN
	CASE rm_par.tipo_saldo 
		...
		WHEN 'V'
			...
			LET orden      = '4 DESC'
		WHEN 'T'
			LET expr_valor = 'SUM(z30_saldo_xvenc) + ' ||
...
WHILE TRUE
	...
	DISPLAY ARRAY rm_dcli TO rm_dcli.*
		...
		ON KEY(F5)
			...
			IF r_an.g03_modulo = 'TA' THEN
				LET comando = 'cd ' || vg_dir_fobos CLIPPED || 
				      vg_separador || 'TALLER' || 
		...
		ON KEY(F6)
			LET i = arr_curr()
			SELECT z20_codcli, z20_num_doc, z20_dividendo
				INTO codcli, num_doc, dividendo
				FROM temp_doc 
				WHERE ROWID = rm_rowid[i]
			CALL muestra_movimientos_documento_cxc(vg_codcia, 
				vg_codloc, areaneg, codcli, rm_dcli[i].z20_tipo_doc, 
				num_doc, dividendo)
		ON KEY(F7)
			LET i = arr_curr()
	...
	END IF
	LET query = 'SELECT tit_area, z20_tipo_doc, num_doc, z20_fecha_vcto,',
			'tit_estado, dias, saldo, ROWID ',
			'FROM temp_doc ',
			'ORDER BY ',
...
DISPLAY 'Tip'           TO tit_col2
DISPLAY 'No. Documento' TO tit_col3
...
IF vm_num_rows > 0 THEN
	...
	SELECT  SUM(z30_saldo_favor), SUM(z30_saldo_xvenc), SUM(z30_saldo_venc)
		...
		WHERE z30_compania  = vg_codcia AND 
		      ...
		      z30_codcli    = rm_cligen.z01_codcli AND 
		      z30_moneda    = rm_par.moneda
	...
	SELECT SUM(z26_valor) INTO tot_postfec FROM cxct026
		WHERE z26_compania  = vg_codcia AND 
		      z26_localidad = vg_codloc AND 
		      z26_codcli    = rm_cligen.z01_codcli AND 
		      z26_estado    = 'A'
	IF tot_postfec IS NULL THEN
		LET tot_postfec = 0
	END IF
...
DECLARE q_doc CURSOR FOR
	SELECT cxct020.*, g03_abreviacion
...
FOREACH q_doc INTO r_doc.*, tit_area
	...
	INSERT INTO temp_doc VALUES (tit_area, r_doc.z20_tipo_doc, numdoc,
		r_doc.z20_fecha_vcto, tit_estado, dias, valor_aux, 
		aux,
	        r_doc.z20_areaneg, r_doc.z20_num_doc, r_doc.z20_dividendo,
		r_doc.z20_codcli, r_doc.z20_cod_tran, r_doc.z20_num_tran)
...
FUNCTION muestra_movimientos_documento_cxc(codcia, codloc, areaneg, codcli, tipo_doc, 
					   num_doc, dividendo)
...
DEFINE r_pdoc	ARRAY[1000] OF RECORD
...
LET max_rows = 1000
OPEN WINDOW w_mdoc AT 8,3 WITH FORM "../forms/cxcf305_3"
...
WHILE TRUE
	...
	LET i = 1
	LET tot_pago = 0
	OPEN q_dpgc USING codcia, codloc, codcli, tipo_doc, num_doc, dividendo
	WHILE TRUE
	...
	DISPLAY ARRAY r_pdoc TO r_pdoc.*
		...
		ON KEY(INTERRUPT)
			EXIT DISPLAY
		ON KEY(F5)
			CALL fl_muestra_forma_pago_caja(codcia, codloc, areaneg, codcli, r_pdoc[i].z23_tipo_trn, r_pdoc[i].z23_num_trn) 
		ON KEY(F6)
			LET i = arr_curr()
			LET comando = 'fglrun cxcp202 ' || vg_base || ' ' ||
			      vg_modulo || ' ' ||
			      codcia || ' ' || 
			      codloc || ' ' ||
			      codcli    || ' ' ||
...
LET max_rows = 1000
OPEN WINDOW w_ftrn AT 8,3 WITH FORM "../forms/cxcf305_5"
	ATTRIBUTE(FORM LINE FIRST, COMMENT LINE LAST, MESSAGE LINE LAST, BORDER)
DISPLAY 'Tp'                  TO tit_col1 
DISPLAY 'Número'              TO tit_col2 
...
WHILE TRUE
	...
	LET i = 1
	LET tot_pago = 0
	OPEN q_dtf USING codcia, codloc, codcli, tipo_doc, num_doc
	WHILE TRUE
...
LET max_rows = 800
OPEN WINDOW w_dmcli AT 8,3 WITH FORM "../forms/cxcf305_4"
	ATTRIBUTE(FORM LINE FIRST, COMMENT LINE LAST, MESSAGE LINE LAST, BORDER)
DISPLAY 'Tp'                  TO tit_col1 
DISPLAY 'Número'              TO tit_col2 
...
WHILE TRUE
	...
	LET i = 1
	LET tot_pago = 0
	OPEN q_dmcli USING codcia, codloc, codcli, moneda
	WHILE TRUE
		FETCH q_dmcli INTO r_movc[i].*, dividendo, r_arn[i]
		IF status = NOTFOUND THEN
			EXIT WHILE
		END IF
	...
	DISPLAY ARRAY r_movc TO r_movc.*
		...
		ON KEY(INTERRUPT)
			EXIT DISPLAY
		ON KEY(F5)
			CALL fl_muestra_forma_pago_caja(codcia, codloc, r_arn[i], codcli, r_movc[i].z23_tipo_trn, r_movc[i].z23_num_trn) 
		ON KEY(F6)
			LET i = arr_curr()
			LET comando = 'fglrun cxcp202 ' || vg_base || ' ' ||
			      vg_modulo || ' ' ||
...
LET max_rows = 500
OPEN WINDOW w_dda AT 6,11 WITH FORM "../forms/cxcf305_7"
	ATTRIBUTE(FORM LINE FIRST, COMMENT LINE LAST, MESSAGE LINE LAST, BORDER)
...
LET columna_2 = 2
WHILE TRUE
	LET query = 'SELECT z21_tipo_doc, z21_num_doc, z21_fecha_emi, ' ||
			' z21_valor, z21_saldo, z21_areaneg ' ||
	        	' FROM cxct021 ' ||
			' WHERE z21_compania  = ? AND ' || 
		              ' z21_localidad = ? AND ' ||
		      	      ' z21_codcli    = ? AND ' ||
		      	      ' z21_moneda    = ? ' ||
			' ORDER BY ', columna_1, ' ',
	...
	LET tot_valor = 0
	LET tot_saldo = 0
	OPEN q_dda USING codcia, codloc, codcli, moneda
	WHILE TRUE
		FETCH q_dda INTO r_dda[i].*, r_arn[i]
		IF status = NOTFOUND THEN
			EXIT WHILE
		END IF
	...
	DISPLAY ARRAY r_dda TO r_dda.*
		...
		ON KEY(INTERRUPT)
			EXIT DISPLAY
		ON KEY(F5)
			CALL fl_muestra_forma_pago_caja(codcia, codloc, r_arn[i], codcli, r_dda[i].z21_tipo_doc, r_dda[i].z21_num_doc) 
		ON KEY(F6)
			LET i = arr_curr()
		...
		ON KEY(F7)
			...
			codcli, r_dda[i].z21_tipo_doc, r_dda[i].z21_num_doc) 
		ON KEY(F15)
			LET i = 1
			LET int_flag = 2
			EXIT DISPLAY
...
DEFINE v_nuevoprog	VARCHAR(400)
...
LET v_max_chpr = 1000
OPEN WINDOW w_chpr AT 04, 15
        WITH FORM '../forms/cxcf305_8'
	...
	DISPLAY ARRAY r_chpr TO r_chpr.*
		...
		ON KEY(F5)
			LET v_nuevoprog = 'cd ..', vg_separador, '..',
					...
					vg_separador, 'fuentes',
					vg_separador, '; fglrun cxcp207
					', vg_base, ' ', vg_modulo,
						' ', vg_codcia, ' ',
					vg_codloc, ' ', r_ch_pr[i].j12_banco,
...
LET v_max_chpf = 1000
OPEN WINDOW w_chpf AT 04, 15
        WITH FORM '../forms/cxcf305_9'
        ATTRIBUTE(FORM LINE FIRST, COMMENT LINE LAST, MESSAGE LINE LAST,
                   MENU LINE 0, BORDER)
CALL mostrar_botones_chques()
CALL fl_lee_cliente_general(codcli) RETURNING r_cli.*
	...
	DISPLAY ARRAY r_chpf TO r_chpf.*
		...
		ON KEY(F5)
			LET v_nuevoprog = 'cd ..', vg_separador, '..',
					vg_separador, 'COBRANZAS',
					vg_separador, 'fuentes',
					vg_separador, '; fglrun cxcp206
					', vg_base, ' ', vg_modulo,
						' ', vg_codcia, ' ',
					vg_codloc, ' ', r_ch_pf[i].z26_codcli,
					' ', r_ch_pf[i].z26_banco, ' ', '"',
...
CALL fl_genera_saldos_cliente(vg_codcia, vg_codloc, rm_cligen.z01_codcli) 
CALL mostrar_registro(rm_rows[vm_row_current])
