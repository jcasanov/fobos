...
-- Autor            : NPC
-- Formato Ejecucion: fglrun cxcp202 base módulo compañía localidad
--			[cliente tipo-transacción número-transacción]
-- Ultima Correccion: 
-- Motivo Correccion: 
------------------------------------------------------------------------------
...
DEFINE rm_cxc3		RECORD LIKE cxct020.*
...
INITIALIZE rm_cxc.*, rm_cxc2.*, rm_cxc3.* TO NULL
FOR indice = 1 TO 10
	LET rm_orden[indice] = '' 
END FOR
LET vm_num_rows    = 0
...
INITIALIZE rm_cxc.*, rm_cxc2.*, rm_cxc3, r_cxc.*, r_mon.* TO NULL
...
IF NOT int_flag THEN
	...
	IF NOT int_flag AND vm_num_elm > 0 THEN
		...
		FOR i = 1 TO vm_num_elm
			...
			END IF
			IF valor_aux >
			rm_cxc3.z20_saldo_cap + rm_cxc3.z20_saldo_int THEN
				ROLLBACK WORK
			...
			IF valor_aux <= rm_cxc3.z20_saldo_int THEN
		                LET rm_cxc2.z23_valor_int =
						rm_aju[i].tit_valor_apl
		                LET rm_cxc2.z23_valor_cap = 0
		        ELSE
           		     	LET rm_cxc2.z23_valor_int =
					rm_cxc3.z20_saldo_int
                		IF rm_aju[i].tit_valor_apl < 0 THEN
           		     		LET rm_cxc2.z23_valor_int =
						rm_cxc2.z23_valor_int * (-1)
				END IF
			...
			INSERT INTO cxct023 VALUES(rm_cxc2.z23_compania,
				...
				rm_cxc2.z23_valor_cap, rm_cxc2.z23_valor_int,
				rm_cxc2.z23_valor_mora, rm_cxc3.z20_saldo_cap,
				rm_cxc3.z20_saldo_int)
			UPDATE cxct020
...
LET int_flag = 0
IF num_args() = 4 THEN
	CONSTRUCT BY NAME expr_sql ON z22_tipo_trn, z22_num_trn, z22_subtipo,
	z22_codcli, z22_areaneg, z22_moneda, z22_cobrador, z22_referencia
	ON KEY(F2)
...
LET int_flag = 0
INPUT BY NAME rm_cxc.z22_tipo_trn, rm_cxc.z22_subtipo, rm_cxc.z22_codcli,
	rm_cxc.z22_areaneg, rm_cxc.z22_moneda, rm_cxc.z22_cobrador,
	rm_cxc.z22_referencia
	WITHOUT DEFAULTS
	ON KEY(INTERRUPT)
	        IF field_touched(rm_cxc.z22_tipo_trn, rm_cxc.z22_subtipo,
			rm_cxc.z22_codcli, rm_cxc.z22_areaneg,
			rm_cxc.z22_moneda, rm_cxc.z22_cobrador,
			rm_cxc.z22_referencia)
	        THEN
	               	LET int_flag = 0
	...
	ON KEY(F2)
		IF infield(z22_tipo_trn) THEN
			...
			LET int_flag = 0
			IF codt_aux IS NOT NULL THEN
				LET rm_cxc.z22_tipo_trn = codt_aux
				DISPLAY BY NAME rm_cxc.z22_tipo_trn
				DISPLAY nomt_aux TO tit_tipo_trn
			END IF 
		END IF
		IF infield(z22_subtipo) THEN
			CALL fl_ayuda_subtipo_entidad(rm_cxc.z22_tipo_trn)
				RETURNING codte_aux, codst_aux, nomte_aux,
					nomst_aux
			LET int_flag = 0
			IF codte_aux IS NOT NULL THEN
				LET rm_cxc.z22_subtipo = codst_aux
				DISPLAY BY NAME rm_cxc.z22_subtipo
				DISPLAY nomte_aux TO tit_subtipo
			END IF 
		END IF
		IF infield(z22_codcli) THEN
		...
		IF infield(z22_cobrador) THEN
			...
			LET int_flag = 0
			IF codc_aux IS NOT NULL THEN
				LET rm_cxc.z22_cobrador = codc_aux
				DISPLAY BY NAME rm_cxc.z22_cobrador
				DISPLAY nomc_aux TO tit_cobrador
			END IF 
		END IF
	AFTER FIELD z22_tipo_trn 
		IF rm_cxc.z22_tipo_trn IS NOT NULL THEN
			CALL fl_lee_tipo_doc(rm_cxc.z22_tipo_trn)
				RETURNING r_tip.* 
	...
	AFTER FIELD z22_subtipo
		IF rm_cxc.z22_subtipo IS NOT NULL THEN
			CALL fl_lee_subtipo_entidad(rm_cxc.z22_tipo_trn,
							rm_cxc.z22_subtipo)
				RETURNING r_sub.*
	...
	AFTER FIELD z22_cobrador
		IF rm_cxc.z22_cobrador IS NOT NULL THEN
			CALL fl_lee_cobrador_cxc(vg_codcia,rm_cxc.z22_cobrador)
				RETURNING r_cob.*
	...
	AFTER INPUT
		CALL fl_lee_transaccion_cxc(vg_codcia, vg_codloc,
				rm_cxc.z22_codcli, rm_cxc.z22_tipo_trn,
				rm_cxc.z22_num_trn)
			RETURNING r_cxc_aux.*
...
IF vm_num_rows > 0 THEN
	DECLARE q_dt CURSOR FOR SELECT * FROM cxct022 WHERE ROWID = num_registro
        OPEN q_dt
        FETCH q_dt INTO rm_cxc.*
	IF STATUS = NOTFOUND THEN
		CALL fgl_winmessage (vg_producto,'No existe registro con índice: ' || vm_row_current,'exclamation')
		RETURN
	END IF
	DISPLAY BY NAME	rm_cxc.z22_num_trn, rm_cxc.z22_codcli,
			rm_cxc.z22_tipo_trn, rm_cxc.z22_subtipo,
			rm_cxc.z22_areaneg, rm_cxc.z22_moneda,
			rm_cxc.z22_cobrador, rm_cxc.z22_referencia,
                	rm_cxc.z22_usuario, rm_cxc.z22_fecing
	CALL fl_lee_cliente_general(rm_cxc.z22_codcli) RETURNING r_cli_gen.*
	DISPLAY r_cli_gen.z01_nomcli TO tit_nombre_cli
	CALL fl_lee_tipo_doc(rm_cxc.z22_tipo_trn) RETURNING r_tip.* 
	DISPLAY r_tip.z04_nombre TO tit_tipo_trn
	CALL fl_lee_subtipo_entidad(rm_cxc.z22_tipo_trn,rm_cxc.z22_subtipo)
		RETURNING r_sub.*
	DISPLAY r_sub.g12_nombre TO tit_subtipo
