...
DEFINE rm_det		ARRAY[10000] OF RECORD
			...
			END RECORD
DEFINE rm_cliente	ARRAY[10000] OF LIKE cxct021.z21_codcli
...
DEFER INTERRUPT
CLEAR SCREEN
CALL startlog('../logs/cxcp210.err')
--#CALL fgl_init4js()
...
IF valida_sri(i) THEN
	CALL fl_validacion_num_sri(vg_codcia, vg_codloc, rm_det[i].z21_tipo_doc,
					'N', rm_det[i].z21_num_sri)
...
CALL fl_lee_documento_favor_cxc(vg_codcia, vg_codloc, rm_cliente[i],
				rm_det[i].z21_tipo_doc, rm_det[i].z21_num_doc)
	RETURNING r_z21.*
	  ...
	  AND g37_secuencia IN
		(SELECT MAX(g37_secuencia)
			...
			WHERE g37_compania  = vg_codcia
			  AND g37_localidad = vg_codloc
			  AND g37_tipo_doc  = 'NC')
IF r_z21.z21_fecha_emi >= r_g37.g37_fecha_emi AND
   r_z21.z21_fecha_emi <= r_g37.g37_fecha_exp
THEN
	RETURN 1
ELSE
	RETURN 0
...
LET comando = 'cd ..', vg_separador, '..', vg_separador, 'COBRANZAS',
	...
	rm_cliente[i], ' "', rm_det[i].z21_tipo_doc, '" ', rm_det[i].z21_num_doc
RUN comando
