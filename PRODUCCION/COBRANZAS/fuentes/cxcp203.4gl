...
DEFER INTERRUPT
CLEAR SCREEN
CALL startlog('../logs/cxcp203.err')
--#CALL fgl_init4js()
...
--display 'cxcp203: fecha en pantalla ', rm_z22.z22_fecing
	BEGIN WORK
...
--display ' '
	...
	END IF
	WHENEVER ERROR STOP
	UPDATE cxct021 SET z21_saldo = rm_z21.z21_saldo WHERE CURRENT OF q_up3
	CLOSE q_up3
	FREE q_up3
...
LET int_flag = 0
IF num_args() = 4 THEN
	CONSTRUCT BY NAME expr_sql ON z22_tipo_trn, z22_num_trn, z22_fecing,
	z22_codcli, z22_areaneg, z22_moneda, z21_tipo_doc, z21_num_doc,
	...
	AFTER FIELD z22_codcli
		IF rm_z22.z22_codcli IS NOT NULL THEN
			...
			IF r_cli.z02_compania IS NULL THEN
				--CALL fgl_winmessage(vg_producto,'Cliente no esta activado para la compañía.','exclamation')
...
WHILE NOT salir
	...
	INPUT ARRAY rm_aju WITHOUT DEFAULTS FROM rm_aju.*
		...
		AFTER FIELD tit_valor_apl
			...
			END IF
			CALL sacar_total()
			IF check AND rm_z21.z21_saldo = 0 AND
		...
		AFTER INPUT
			...
			IF valor_aux > vm_saldo THEN
				...
				NEXT FIELD tit_valor_apl
			END IF
			--IF NOT check AND rm_z21.z21_saldo > 0 THEN
			IF rm_z21.z21_saldo > 0 AND
			   rm_z21.z21_saldo >= valor_aux
			THEN
...
FOREACH q_cons1 INTO r_cxc.*
	...
	IF STATUS < 0 THEN
		...
		CALL fl_mostrar_mensaje('El documento ' || documento || ' del proveedor esta siendo modificado por otro usuario.','exclamation')
		LET vm_num_elm = 0
		WHENEVER ERROR STOP
		RETURN
	END IF
