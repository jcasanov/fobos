...
DEFER INTERRUPT
CLEAR SCREEN
CALL startlog('../logs/cxcp311.err')
--#CALL fgl_init4js()
...
CASE rm_par.tipo_detalle 
	WHEN 'A'
		...
		LET expr_sql = ' WHERE g03_areaneg  = a.', campo CLIPPED
	WHEN "C"
		LET campo    = 'tipo_cli '
		LET tipo     = 'CL'
	WHEN "R"
	...
	WHEN "P"
		...
		LET tabla    = ' cxct001 '
		LET expr_whe = NULL
		LET expr_sql = ' WHERE z01_codcli = a.', campo CLIPPED
END CASE
IF rm_par.tipo_detalle = "C" OR rm_par.tipo_detalle = "R" THEN
	...
	LET expr_sql = ' WHERE g12_subtipo = a.', campo CLIPPED
END IF
...
LET query = 'INSERT INTO tempo_acum ',
		 'SELECT a.localidad, a.', campo CLIPPED, ', descri, ',
			subquery_pven_venc('b.', campo, 0, 0), ', ',
			subquery_pven_venc('c.', campo, rm_par.rango1_i,
					rm_par.rango1_f), ', ',
			subquery_pven_venc('d.', campo, rm_par.rango2_i,
					rm_par.rango2_f), ', ',
			subquery_pven_venc('e.', campo, rm_par.rango3_i,
					rm_par.rango3_f), ', ',
			subquery_pven_venc('f.', campo, rm_par.rango4_i,
					rm_par.rango4_f), ', ',
			subquery_pven_venc('g.', campo, rm_par.rango5_i, 0),
			' FROM tempo_doc a, tmp_des ',
				expr_sql CLIPPED,
			' GROUP BY 1, 2, 3'
...
FUNCTION subquery_pven_venc(prefijo, campo, rango_i, rango_f)
DEFINE prefijo		VARCHAR(2)
DEFINE campo		VARCHAR(20)
...
IF rm_par.ind_venc = 'V' THEN
	LET sig1 = '>='
	LET sig2 = '<'
	LET cfe1 = prefijo CLIPPED, "fecha"
	LET cfe2 = '"', rm_par.fecha_cart, '"'
ELSE
	LET sig1 = '<'
	LET sig2 = '>='
	LET cfe1 = '"', rm_par.fecha_cart, '"'
	LET cfe2 = prefijo CLIPPED, "fecha"
END IF
LET expr_sql = NULL
LET maxmen   = sig1
...
END IF
LET subquery = 'NVL((SELECT NVL(SUM(', prefijo CLIPPED, 'valor), 0) ',
		' FROM tempo_doc ', prefijo[1, 1] CLIPPED,
		' WHERE ', prefijo CLIPPED, campo CLIPPED, '  = a.',
								campo CLIPPED,
		'   AND ', prefijo CLIPPED, 'localidad  = a.localidad ',
		'   AND ', prefijo CLIPPED, 'fecha     ', maxmen CLIPPED,
			' "', rm_par.fecha_cart, '"',
		expr_sql CLIPPED,
		' HAVING SUM(', prefijo CLIPPED, 'valor) > 0), 0) '
RETURN subquery CLIPPED
