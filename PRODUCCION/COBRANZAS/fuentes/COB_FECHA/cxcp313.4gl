...
END INPUT
OPTIONS INPUT WRAP
CONSTRUCT BY NAME expr_sql ON j10_codcli, j10_nomcli, r25_num_tran,
	r25_valor_cred
	ON KEY(INTERRUPT)
		LET int_flag = 2
		--RETURN NULL
...
WHILE TRUE
	LET query = 'SELECT j10_codcli, j10_nomcli, DATE(j10_fecing), ',
				' r25_cod_tran, r25_num_tran, r25_valor_cred, ',
				' j10_usuario ',
	...
	LET vm_num_det = 1
	LET total_cre  = 0
	FOREACH q_cre INTO rm_detalle[vm_num_det].*
...
FOR i = 1 TO fgl_scr_size('rm_detalle')
	CLEAR rm_detalle[i].*
END FOR
CLEAR num_rows, max_rows, total_cre
...
ON EVERY ROW
	...
	PRINT COLUMN 001, rm_detalle[i].j10_codcli	USING "<<&&&&",
	      COLUMN 008, rm_detalle[i].j10_nomcli[1, 27] CLIPPED,
	      COLUMN 036, rm_detalle[i].j10_fecing	USING "dd-mm-yyyy",
