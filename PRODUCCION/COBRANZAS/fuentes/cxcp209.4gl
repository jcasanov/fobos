--------------------------------------------------------------------------------
-- Titulo           : cxcp209.4gl - Modificación del No. SRI en Notas Debito
-- Elaboracion      : 14-Ene-2003
-- Autor            : NPC
-- Formato Ejecucion: fglrun cxcp209 base módulo compañía localidad
-- Ultima Correccion: 
-- Motivo Correccion: 
--------------------------------------------------------------------------------
...
DEFINE rm_det		ARRAY [500] OF RECORD
				...
				z20_fecing	DATE,
				z20_num_sri	LIKE cxct020.z20_num_sri
			END RECORD
DEFINE rm_cliente	ARRAY [500] OF RECORD
...
DEFER INTERRUPT
CLEAR SCREEN
CALL startlog('../logs/cxcp209.err')
--#CALL fgl_init4js()
...
OPEN WINDOW w_mas AT row_ini, 2 WITH num_rows ROWS, num_cols COLUMNS
    ATTRIBUTE(FORM LINE FIRST + 1, COMMENT LINE LAST, MENU LINE lin_menu,BORDER,
	      MESSAGE LINE LAST - 1)
IF vg_gui = 1 THEN
        OPEN FORM f_cxc FROM '../forms/cxcf209_1'
ELSE
        OPEN FORM f_cxc FROM '../forms/cxcf209_1c'
END IF
DISPLAY FORM f_cxc
...
WHILE TRUE
	...
	LET query = 'INSERT INTO tmp_sri ',
		'SELECT z20_tipo_doc, z20_num_doc, z01_nomcli, ',
			'DATE(z20_fecing), z20_num_sri, z20_codcli, ',
			'z20_dividendo ',
		'FROM cxct020, cxct001 ',
		'WHERE z20_compania    = ', vg_codcia,
		'  AND z20_localidad   = ', vg_codloc,
		'  AND z20_tipo_doc    = "ND"',
		'  AND DATE(z20_fecing) BETWEEN "', vm_fecha_ini, '"',
					'   AND "', vm_fecha_fin, '"',
...
OPTIONS INPUT WRAP
LET int_flag = 0
CONSTRUCT BY NAME expr_sql ON z01_nomcli, z20_num_sri
	ON KEY(INTERRUPT)
		LET int_flag = 2
		EXIT CONSTRUCT
        ON KEY(F1,CONTROL-W)
...
BEGIN WORK
	...
	DECLARE q_modsri2 CURSOR FOR
		SELECT * FROM cxct020
			WHERE z20_compania  = vg_codcia
			  AND z20_localidad = vg_codloc
			  AND z20_codcli    = rm_cliente[i].z20_codcli
			  AND z20_tipo_doc  = rm_det[i].z20_tipo_doc
			  AND z20_num_doc   = rm_det[i].z20_num_doc
			  AND z20_dividendo = rm_cliente[i].z20_dividendo
		FOR UPDATE
	WHENEVER ERROR CONTINUE
	OPEN q_modsri2
	FETCH q_modsri2 INTO r_z20.*
	IF STATUS < 0 THEN
		ROLLBACK WORK
	...
	DECLARE q_sri CURSOR FOR
		SELECT * FROM gent037
			WHERE g37_compania  =  vg_codcia
			  ...
			  AND g37_tipo_doc  =  rm_det[i].z20_tipo_doc
			  AND g37_cont_cred =  'N'
			{--
	...
	OPTIONS INPUT WRAP
	LET int_flag = 0
	INPUT rm_det[i].z20_num_sri WITHOUT DEFAULTS FROM rm_det[j].z20_num_sri
		ON KEY(INTERRUPT)
			LET rm_det[i].z20_num_sri = aux_sri
			DISPLAY rm_det[i].z20_num_sri TO rm_det[j].z20_num_sri
			LET int_flag = 1
			EXIT INPUT
	        ON KEY(F1,CONTROL-W)
		...
		BEFORE INPUT
			...
			LET aux_sri = rm_det[i].z20_num_sri
		...
		AFTER FIELD z20_num_sri
			IF rm_det[i].z20_num_sri IS NOT NULL THEN
				CALL validar_num_sri(aux_sri, i, j)
					RETURNING r_g37.*, resul
				CASE resul
					WHEN -1
			...
			ELSE
				LET rm_det[i].z20_num_sri = aux_sri
				DISPLAY rm_det[i].z20_num_sri TO
					rm_det[j].z20_num_sri
			END IF
		AFTER INPUT
			IF rm_det[i].z20_num_sri IS NOT NULL THEN
				CALL validar_num_sri(aux_sri, i, j)
	...
	LET sec_sri = rm_det[i].z20_num_sri[9, cuantos] USING "########"
	...
	UPDATE cxct020 SET z20_num_sri = rm_det[i].z20_num_sri
		WHERE CURRENT OF q_modsri2
	UPDATE tmp_sri SET z20_num_sri = rm_det[i].z20_num_sri
		WHERE z20_codcli   = rm_cliente[i].z20_codcli
		  AND z20_tipo_doc = rm_det[i].z20_tipo_doc
		  AND z20_num_doc  = rm_det[i].z20_num_doc
COMMIT WORK
...
IF valida_sri(i) THEN
	CALL fl_validacion_num_sri(vg_codcia, vg_codloc, rm_det[i].z20_tipo_doc,
					'N', rm_det[i].z20_num_sri)
		RETURNING r_g37.*, rm_det[i].z20_num_sri, flag
	CASE flag
		WHEN -1
			RETURN r_g37.*, -1
		WHEN 0
...
DISPLAY rm_det[i].z20_num_sri TO rm_det[j].z20_num_sri
{--
IF LENGTH(rm_det[i].z20_num_sri) < 15 THEN
	CALL fl_mostrar_mensaje('Digite completo el número del SRI.','exclamation')
	RETURN r_g37.*, 0
END IF
--}
IF aux_sri <> rm_det[i].z20_num_sri THEN
	SELECT COUNT(*) INTO cont FROM cxct020
		WHERE z20_compania  = vg_codcia
		  AND z20_localidad = vg_codloc
  		  AND z20_num_sri   = rm_det[i].z20_num_sri
	IF cont > 0 THEN
		CALL fl_mostrar_mensaje('La secuencia del SRI ' || rm_det[i].z20_num_sri[9,15] || ' ya existe.','exclamation')
		RETURN r_g37.*, 0
	END IF
END IF
RETURN r_g37.*, 1
...
CALL fl_lee_documento_deudor_cxc(vg_codcia, vg_codloc, rm_cliente[i].z20_codcli,
				rm_det[i].z20_tipo_doc, rm_det[i].z20_num_doc,
				rm_cliente[i].z20_dividendo)
	RETURNING r_z20.*
	  ...
	  AND g37_secuencia IN
		(SELECT MAX(g37_secuencia)
			...
			WHERE g37_compania  = vg_codcia
			  AND g37_localidad = vg_codloc
			  AND g37_tipo_doc  = 'ND')
IF r_z20.z20_fecha_emi >= r_g37.g37_fecha_emi AND
   r_z20.z20_fecha_emi <= r_g37.g37_fecha_exp
THEN
	RETURN 1
ELSE
	RETURN 0
...
LET vm_nuevoprog = 'cd ..', vg_separador, '..', vg_separador, 'COBRANZAS',
	...
	rm_cliente[i].z20_codcli, ' "', rm_det[i].z20_tipo_doc, '" ',
	rm_det[i].z20_num_doc, ' ', rm_cliente[i].z20_dividendo
RUN vm_nuevoprog
