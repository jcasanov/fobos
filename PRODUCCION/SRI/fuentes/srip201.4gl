...
MENU 'OPCIONES'
	BEFORE MENU
		...
		HIDE OPTION 'Cerrar Anexo'
		HIDE OPTION 'Reabrir Anexo'
	...
	COMMAND KEY('I') 'Ingresar' 'Ingresar nuevos registros. '
		...
		IF vm_num_rows <= 1 THEN
			SHOW OPTION 'Modificar'
			IF rm_s21.s21_estado = 'G' OR rm_s21.s21_estado = 'P'
			THEN
				SHOW OPTION 'Cerrar Anexo'
				HIDE OPTION 'Reabrir Anexo'
				HIDE OPTION 'Declarar Anexo'
			ELSE
				IF rm_s21.s21_estado = 'C' THEN
					HIDE OPTION 'Cerrar Anexo'
					SHOW OPTION 'Reabrir Anexo'
					SHOW OPTION 'Declarar Anexo'
				ELSE
					HIDE OPTION 'Cerrar Anexo'
					HIDE OPTION 'Reabrir Anexo'
					HIDE OPTION 'Declarar Anexo'
	...
	COMMAND KEY('C') 'Consultar' 'Consultar un registro. '
		...
		IF vm_row_current <= 1 THEN
                        HIDE OPTION 'Retroceder'
                END IF
        COMMAND KEY('W') 'Reabrir Anexo' 'Reabre anexo transaccional ventas.'
		CALL control_cierre_reapertura('R')
		IF rm_s21.s21_estado = 'G' OR rm_s21.s21_estado = 'P' THEN
			SHOW OPTION 'Cerrar Anexo'
			HIDE OPTION 'Reabrir Anexo'
			HIDE OPTION 'Declarar Anexo'
		ELSE
			...
			END IF
		END IF
        COMMAND KEY('P') 'Cerrar Anexo' 'Cierra anexo transaccional ventas.'
		...
		ELSE
			...
			END IF
		END IF
        COMMAND KEY('Z') 'Declarar Anexo' 'Declara anexo transaccional ventas.'
		CALL control_cierre_reapertura('D')
		IF rm_s21.s21_estado = 'G' OR rm_s21.s21_estado = 'P' THEN
			SHOW OPTION 'Cerrar Anexo'
			HIDE OPTION 'Reabrir Anexo'
			HIDE OPTION 'Declarar Anexo'
...
INPUT BY NAME rm_par.anio_fin, rm_par.mes_fin, rm_par.tipo_gye, rm_par.tipo_uio,
	...
	AFTER INPUT
		...
		IF r_s21.s21_compania IS NOT NULL THEN
			IF flag = 'I' OR flag = '1' THEN
				CALL fl_mostrar_mensaje('No puede generar el anexo de un periodo que ya ha sido CERRADO, PROCESADO o DECLARADO.', 'exclamation')
				LET rm_par.anio_fin = rm_par.anio_ini
				LET rm_par.mes_fin  = rm_par.mes_ini
...
IF int_flag THEN
	RETURN
END IF
LET rm_par.anio_ini = rm_par.anio_fin
LET rm_par.mes_ini  = rm_par.mes_fin
...
WHENEVER ERROR CONTINUE
	...
	UPDATE srit021
		...
		WHERE s21_compania  = rm_s21.s21_compania
		  ...
		  AND s21_mes       = rm_s21.s21_mes
	UPDATE srit000
		SET s00_ano_proceso = rm_s21.s21_anio,
		    s00_mes_proceso = rm_s21.s21_mes
		WHERE s00_compania = rm_s21.s21_compania
	IF vg_codloc = 3 THEN
		UPDATE acero_qs:srit000
			SET s00_ano_proceso = rm_s21.s21_anio,
			    s00_mes_proceso = rm_s21.s21_mes
			WHERE s00_compania = rm_s21.s21_compania
	END IF
