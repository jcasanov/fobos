--------------------------------------------------------------------------------
-- Titulo           : rolp207.4gl - Mantenimiento novedades decimo tercero
-- Elaboracion      : 23-ago-2003
-- Autor            : JCM
-- Formato Ejecucion: fglrun rolp207 base modulo compania [fecha_ini] 
--				[fecha_fin] [cod_trab] [opcion {G | C | F}]
-- 			G: Genera o regenera el decimo tercero para un trab.
-- 			C: Consulta el decimo tercero generado para un trab.
-- 			F: Cerrar   el decimo tercero generado por finiquito.
-- Ultima Correccion:
-- Motivo Correccion:
--------------------------------------------------------------------------------
...
DEFER INTERRUPT
CLEAR SCREEN
CALL startlog('../logs/rolp207.err')
CALL fgl_init4js()
...
OPTIONS
	INPUT WRAP,
	ACCEPT KEY F12
OPEN WINDOW w_rolf207_1 AT 3,2 WITH 22 ROWS, 80 COLUMNS
	ATTRIBUTE(FORM LINE FIRST, COMMENT LINE LAST, MENU LINE 0, BORDER,
			MESSAGE LINE LAST - 1) 
OPEN FORM f_rolf207_1 FROM '../forms/rolf207_1'
DISPLAY FORM f_rolf207_1
...
DISPLAY 'Total Ganado' 		TO bt_valor
DISPLAY 'Valor Decimo' 		TO bt_decimo
...
DISPLAY ARRAY rm_scr TO ra_scr.*
	BEFORE DISPLAY
		...
		CALL calcula_totales() RETURNING tot_valor,   tot_decimo,
					         tot_desctos, tot_neto
		DISPLAY BY NAME tot_valor, tot_decimo, tot_desctos, tot_neto
		EXIT DISPLAY
	BEFORE ROW
...
DECLARE q_trab CURSOR FOR 
	SELECT n36_cod_trab, n30_mon_sueldo, n36_cod_depto, n36_tipo_pago,  
               ...
               n36_ganado_per, n36_valor_bruto, n36_descuentos, n36_valor_neto 
...
IF rm_par.n36_estado = 'P' THEN
	CALL fl_mostrar_mensaje('Este decimo ya ha sido procesado.', 'exclamation')
	RETURN
END IF
...
DEFINE comando 		VARCHAR(255)
	...
	LET comando = 'fglrun rolp409 ', vg_base, ' ', vg_modulo,
...
INPUT ARRAY rm_scr WITHOUT DEFAULTS FROM ra_scr.*
	...
	ON KEY(F7)
        ...
        BEFORE INSERT
                LET salir = 0
                EXIT INPUT
	BEFORE FIELD n36_ganado_per
		LET valor = rm_scr[i].n36_ganado_per
        AFTER FIELD n36_ganado_per
                IF rm_scr[i].n36_ganado_per IS NULL THEN
			LET rm_scr[i].n36_ganado_per = valor
			DISPLAY rm_scr[i].* TO ra_scr[j].*
                	NEXT FIELD n36_ganado_per
		END IF
                IF rm_scr[i].n36_ganado_per IS NOT NULL THEN
                	IF rm_scr[i].n36_ganado_per = '.' THEN
				LET rm_scr[i].n36_ganado_per = valor
				DISPLAY rm_scr[i].* TO ra_scr[j].*
                		NEXT FIELD n36_ganado_per
			END IF
                        IF rm_scr[i].n36_ganado_per < 0 THEN
                                NEXT FIELD n36_ganado_per
                        END IF
                END IF

		LET rm_scr[i].n36_ganado_per  = fl_retorna_precision_valor(
				vm_cod_trab[i].n30_mon_sueldo,
				rm_scr[i].n36_ganado_per)
		LET rm_scr[i].n36_valor_bruto = fl_retorna_precision_valor(
				vm_cod_trab[i].n30_mon_sueldo,
				rm_scr[i].n36_ganado_per / r_n03.n03_valor)
		LET rm_scr[i].n36_valor_neto = 
			rm_scr[i].n36_valor_bruto - rm_scr[i].n36_descuentos 
...
FOR i = 1 TO vm_numelm
	...
	UPDATE rolt036 SET
		n36_ganado_per    = rm_scr[i].n36_ganado_per,
		n36_usuario_modif = vg_usuario,
		n36_fecha_modif   = CURRENT,
		n36_valor_bruto   = rm_scr[i].n36_valor_bruto,
...
OPEN WINDOW w_207_2 AT 4,6 WITH 20 ROWS, 70 COLUMNS
	ATTRIBUTE(FORM LINE FIRST, COMMENT LINE LAST, BORDER) 
IF vg_gui = 1 THEN
	OPEN FORM f_207_2 FROM '../forms/rolf207_2'
ELSE
	OPEN FORM f_207_2 FROM '../forms/rolf207_2'
END IF
...
INPUT ARRAY rm_desc WITHOUT DEFAULTS FROM ra_desc.*
	...
	BEFORE DELETE
		...
		CALL calcula_total_descuento(arr_count()) RETURNING tot_valor
		DISPLAY BY NAME tot_valor
		DISPLAY rm_scr[currelm].n36_valor_bruto - tot_valor
			TO n36_valor_neto
	...
	AFTER INPUT
		...
		DISPLAY rm_scr[currelm].n36_valor_bruto - tot_valor
			TO n36_valor_neto
		IF tot_valor > rm_scr[currelm].n36_valor_bruto THEN
...
LET rm_scr[currelm].n36_valor_neto = 
...
DISPLAY BY NAME vm_cod_trab[curr_elm].n36_cod_trab,
		...
		rm_scr[curr_elm].n36_valor_bruto,
		rm_scr[curr_elm].n36_valor_neto

DISPLAY rm_scr[curr_elm].n36_descuentos TO tot_valor
...
FOREACH q_prest2 INTO r_n37.*
	...
	UPDATE rolt046
		SET n46_saldo = n46_valor - r_n37.n37_valor
        	WHERE n46_compania   = vg_codcia
		  AND n46_num_prest  = r_n37.n37_num_prest
		  AND n46_cod_liqrol = vm_proceso
		  AND n46_fecha_ini  = vm_r_rows[vm_row_current].n36_fecha_ini
...
OPEN WINDOW w_207_2 AT 4,6 WITH 19 ROWS, 70 COLUMNS
	ATTRIBUTE(FORM LINE FIRST, COMMENT LINE LAST, BORDER) 
IF vg_gui = 1 THEN
	OPEN FORM f_207_2 FROM '../forms/rolf207_2'
ELSE
...
DEFINE cod_trab		LIKE rolt036.n36_cod_trab
DEFINE tot_par		LIKE rolt036.n36_ganado_real
...
SELECT NVL(SUM(n32_tot_gan), 0)
	INTO total_ganado
	FROM tmp_cabecera 
	WHERE n32_fecha_ini >= r_n30.n30_fecha_ing

IF YEAR(r_n30.n30_fecha_ing) = YEAR(rm_par.n36_fecha_fin) THEN
	IF DAY(r_n30.n30_fecha_ing) <> 1 AND DAY(r_n30.n30_fecha_ing) <> 16 THEN
		LET tot_par = 0
		SELECT NVL(SUM(n32_tot_gan), 0)
			INTO tot_par
			FROM tmp_cabecera 
			WHERE MONTH(n32_fecha_ini) = MONTH(r_n30.n30_fecha_ing)
		LET tot_par = (tot_par / 30) * ((DAY(rm_par.n36_fecha_fin) - 
				DAY(r_n30.n30_fecha_ing)) + 1)
		LET total_ganado = total_ganado + tot_par
	END IF
END IF
...
CALL fl_lee_trabajador_roles(vg_codcia, cod_trab) RETURNING r_n30.*
LET param = ' ', rm_par.n36_ano_proceso, ' ', vm_proceso, ' ', 'N'
IF r_n30.n30_cod_trab IS NOT NULL THEN
	LET param = param CLIPPED, ' ', r_n30.n30_cod_depto, ' ',
			r_n30.n30_cod_trab
END IF
CALL ejecuta_comando('NOMINA', vg_modulo, 'rolp410 ', param)
...
LET param = ' ', rm_par.n36_fecha_ini, ' ', rm_par.n36_fecha_fin
CALL ejecuta_comando('NOMINA', vg_modulo, 'rolp411 ', param)
...
UNLOAD TO "../../../tmp/rol_pag.txt" DELIMITER "	"
	SELECT * FROM t1
		ORDER BY secuencia
{--
LET archivo = "rol_pag_", rm_par.n36_fecha_fin USING "mmm", "_",
		rm_par.n36_fecha_fin USING "yyyy", ".txt"
--}
...
LET archivo = "ACRE_", rm_loc.g02_nombre[1, 3] CLIPPED, "_",
		...
		YEAR(rm_par.n36_fecha_fin) USING "####", "_"
