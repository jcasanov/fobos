...
main
	...
	if num_args() <> 4 then
		...
		display 'Son: base cod_trab fecha_ini fecha_fin.'
		exit program
	end if
	let base      = arg_val(1)
	let cod_trab  = arg_val(2)
	let fecha_ini = date(arg_val(3))
	let fecha_fin = date(arg_val(4))
	call activar_base()
...
if r_n30.n30_compania is null then
	display 'No existe el Empleado con Código: ', cod_trab using "&&&&"
	exit program
end if
if fecha_ini > today then
	display 'La fecha inicial no puede ser mayor a la actual.'
	exit program
end if
{--
if fecha_fin > today then
	display 'La fecha final no puede ser mayor a la actual.'
	exit program
end if
--}
if fecha_ini > fecha_fin then
	display 'La fecha final no puede ser menor a la fecha inicial.'
	exit program
end if
