--------------------------------------------------------------------------------
-- Titulo           : rolp411.4gl - LISTADO DETALLE DECIMO TERCERO
-- Elaboracion      : 18-Dic-2007
-- Autor            : NPC
-- Formato Ejecucion: fglrun rolp411 BD MODULO COMPANIA fecha_ini fecha_fin 
-- Ultima Correccion:
-- Motivo Correccion: 
--------------------------------------------------------------------------------
...
DEFER INTERRUPT
CLEAR SCREEN
CALL startlog('../logs/rolp411.err')
--#CALL fgl_init4js()
...
--START REPORT report_decimos TO FILE "listado.npc"
...
PAGE HEADER
        ...
        CALL fl_justifica_titulo('C', 'LIQUIDACION DE VALORES POR DECIMO TERCER SUELDO', 80)
                RETURNING titulo
        print ASCII escape;
	...
	IF vm_descuentos = 1 THEN
	        PRINT COLUMN 50, rm_cia.g01_razonsocial,
        	      COLUMN 208, "PAGINA: ", PAGENO USING "&&&"
	ELSE
	        PRINT COLUMN 50, rm_cia.g01_razonsocial,
        	      COLUMN 166, "PAGINA: ", PAGENO USING "&&&"
	END IF
	PRINT COLUMN 50, titulo CLIPPED
        SKIP 1 LINES
        PRINT COLUMN 50, "** PERIODO: ", vm_fecha_ini USING "dd-mm-yyyy",
	...
	PRINT COLUMN 02,  "COD.",
	      ...
	      COLUMN 129, fl_justifica_titulo('D', "TOTAL GANADO", 12),
	      COLUMN 143, fl_justifica_titulo('D', "VALOR DECIMO", 12),
	      COLUMN 167, "FIRMA";
	IF vm_descuentos = 1 THEN
	 	PRINT COLUMN 179, fl_justifica_titulo('D', "DESCUENTOS", 12),
	      	      COLUMN 193, fl_justifica_titulo('D', "ANTICIPOS", 12),
	              COLUMN 207, fl_justifica_titulo('D', "A RECIBIR", 12)
	ELSE
		PRINT ''
	END IF
...
ON EVERY ROW
	...
	END IF
	PRINT COLUMN 129, r_rol.ganado_real USING '#,###,##&.##',
	      COLUMN 143, r_rol.valor_bruto USING '#,###,##&.##',
	      COLUMN 157, '____________________'; 
	IF vm_descuentos = 1 THEN
		PRINT COLUMN 179, r_rol.descuentos USING '#,###,##&.##',
		      COLUMN 193, r_rol.anticipos  USING '#,###,##&.##',
		      COLUMN 207, r_rol.valor_neto USING '#,###,##&.##'
	ELSE
		PRINT ''
	END IF
...
ON LAST ROW 
	...
	PRINT COLUMN 02,  'TOTALES: ',
	      ...
	      COLUMN 129, SUM(r_rol.ganado_real) USING '#,###,##&.##',
	      COLUMN 143, SUM(r_rol.valor_bruto) USING '#,###,##&.##';
	IF vm_descuentos = 1 THEN
		PRINT COLUMN 179, SUM(r_rol.descuentos)  USING '#,###,##&.##',
		      COLUMN 193, SUM(r_rol.anticipos)   USING '#,###,##&.##',
