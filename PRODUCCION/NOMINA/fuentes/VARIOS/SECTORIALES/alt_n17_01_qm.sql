...
{------------------------------------------------------------------------------}
	...
	update rolt030
		set n30_sectorial = "0000000000"
		where n30_sectorial is null;
...
{------------------------------------------------------------------------------}
	...
	select n30_sectorial sect1, n17_sectorial sect2, "." descrip
		from rolt030, outer rolt017
	...
	delete from t3
		where sect2 is not null;

	update t3
		set sect1   = "0000000000",
		    descrip = "INACTIVO"
		where sect1 is null;
	...
	insert into rolt017
		...
		select 1 cia, 2003 anio, sect1, descrip, 264.00 valor,
			"FOBOS" usua, current fecing
			from t3;
	...
	update rolt030
		set n30_sectorial = "1918200000103"
		where n30_estado    = "J"
		  and n30_sectorial = "0000000000";
...
{------------------------------------------------------------------------------}
	...
	load from "sectorial_2009_qm.unl" insert into t1;
	...
	select secto, descrip, val_sec, count(*)
		from t2
		group by 1, 2, 3
		having count(*) > 1;
...
{------------------------------------------------------------------------------}
	...
	select n17_compania, n17_ano_sect, n17_sectorial, count(*) ctos
		from rolt017
		group by 1, 2, 3
		having count(*) > 1;
