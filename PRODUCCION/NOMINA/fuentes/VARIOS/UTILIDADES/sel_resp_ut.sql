SELECT 1 loc_c, n31_cod_trab, n31_tipo_carga, COUNT(n31_secuencia) tot_carg
	...
	WHERE n31_compania     = 1
	  ...
	  AND n31_fecha_nacim <= MDY(12, 31, 2008)
	  AND n30_compania     = n31_compania
	  AND n30_cod_trab     = n31_cod_trab
	  AND (n30_est_civil = 'C' OR n30_est_civil = 'U')
	GROUP BY 1, 2, 3
	UNION ALL
...
SELECT 1 localidad, n30_cod_trab cod_trab, n30_nombres nombres,
	...
	CASE WHEN YEAR(n30_fecha_ing) = 2008
		THEN ((MDY(12, 31, 2008) -
			CASE WHEN n30_fecha_reing IS NULL
				THEN n30_fecha_ing
				ELSE n30_fecha_reing
			END) +
			CASE WHEN (MOD(2008, 4) = 0) AND
				(EXTEND(CASE WHEN n30_fecha_reing IS NULL
		                                THEN n30_fecha_ing
        	                        	ELSE n30_fecha_reing
					END, YEAR TO MONTH) >=
					EXTEND(MDY(03, 01, 2008),YEAR TO MONTH))
				THEN 1
				ELSE 0
			END)
		ELSE (SELECT n90_dias_ano_ut FROM aceros:rolt090
	...
	WHERE n30_compania    = 1
	  AND n30_fecha_ing  <= MDY(12, 31, 2008)
	...
	SELECT 1 localidad, n30_cod_trab cod_trab, n30_nombres nombres,
		n30_fecha_ing fecha_ing, n30_fecha_reing fecha_reing,
		n30_fecha_sal fecha_sal,
		CASE WHEN YEAR(n30_fecha_ing) <> 2008 AND
			n30_fecha_reing IS NULL
			THEN (n30_fecha_sal - MDY(01, 01, 2008))
		     WHEN YEAR(n30_fecha_ing) = 2008 AND
			n30_fecha_reing IS NOT NULL
			THEN (MDY(12, 31, 2008) - n30_fecha_reing) +
			CASE WHEN (MOD(2008, 4) = 0) AND
				(EXTEND(n30_fecha_reing, YEAR TO MONTH) >=
					EXTEND(MDY(03, 01, 2008),YEAR TO MONTH))
				THEN 1
				ELSE 0
			END
		     WHEN n30_fecha_ing > MDY(01, 01, 2008) AND
				YEAR(n30_fecha_sal) > 2008
			THEN (MDY(12, 31, 2008) - n30_fecha_ing) +
			CASE WHEN (MOD(2008, 4) = 0) AND
				(EXTEND(n30_fecha_ing, YEAR TO MONTH) >=
					EXTEND(MDY(03, 01, 2008),YEAR TO MONTH))
				THEN 1
				ELSE 0
			END
		     WHEN n30_fecha_reing > MDY(01, 01, 2008) AND
				YEAR(n30_fecha_sal) > 2008
			THEN (MDY(12, 31, 2008) - n30_fecha_reing) +
			CASE WHEN (MOD(2008, 4) = 0) AND
				(EXTEND(n30_fecha_reing, YEAR TO MONTH) >=
			...
			END
		     WHEN n30_fecha_ing > MDY(01, 01, 2008) AND
			  n30_fecha_sal > MDY(01, 01, 2008) AND
			  n30_fecha_sal < MDY(12, 31, 2008)
			THEN (n30_fecha_sal - n30_fecha_ing) +
			CASE WHEN (MOD(2008, 4) = 0) AND
				(EXTEND(n30_fecha_ing, YEAR TO MONTH) >=
					EXTEND(MDY(03, 01, 2008),YEAR TO MONTH))
				THEN 1
			...
			END
		     WHEN n30_fecha_reing > MDY(01, 01, 2008) AND
			  n30_fecha_sal   > MDY(01, 01, 2008) AND
			  n30_fecha_sal   < MDY(12, 31, 2008)
			THEN (n30_fecha_sal - n30_fecha_reing) +
			CASE WHEN (MOD(2008, 4) = 0) AND
				(EXTEND(n30_fecha_reing, YEAR TO MONTH) >=
					EXTEND(MDY(03, 01, 2008),YEAR TO MONTH))
				THEN 1
		...
		WHERE n30_compania    = 1
		  AND n30_fecha_ing  <= MDY(12, 31, 2008)
		  AND n30_fecha_sal  <= MDY(12, 31, 2009)
		  AND n30_tipo_contr  = 'F'
		  AND n30_estado     <> 'J'
		  AND n30_tipo_trab   = 'N'
		  AND n30_fec_jub    IS NULL
		...
		CASE WHEN YEAR(n30_fecha_ing) = 2008
			THEN ((MDY(12, 31, 2008) -
				...
				CASE WHEN (MOD(2008, 4) = 0) AND
					...
					ELSE 0
				END)
			ELSE (SELECT n90_dias_ano_ut FROM acero_qm:rolt090
				WHERE n90_compania = n30_compania)
		END dias_trab,
		(SELECT SUM(tot_carg)
			FROM tmp_car
...
DELETE FROM te_trab
	WHERE fecha_reing > MDY(12, 31, 2008);

DELETE FROM te_trab
	WHERE  te_trab.fecha_sal < MDY(01, 01, 2008)
	  AND (fecha_reing       IS NULL
	   OR  fecha_reing       > MDY(12, 31, 2008));
