select b10_cuenta cuenta, trim(replace(b10_descripcion,
	(select trim(n30_nombres)
		...
		where n30_compania = b10_compania
		  and n30_cod_trab = 143), " ")) nomcta
	from ctbt010
	where b10_compania    = 1
	  and b10_estado      = 'A'
	  and b10_descripcion matches '*CORDOVA*RUG*'
	into temp t1;
select cuenta, trim(replace(nomcta, "CORDOVA RUGEL MANUEL ELIAS", "")) nomcta
	from t1
	into temp t2;
drop table t1;
select cuenta, trim(replace(nomcta, "CORDOVA RUGEL MANUEL ELIA", "")) nomcta
	from t2
	into temp t1;
drop table t2;
select cuenta, trim(replace(nomcta, "CORDOVA RUGEL MANUEL", "")) nomcta
	from t1
	into temp t2;
drop table t1;
select cuenta, trim(replace(nomcta, "CORDOVA RUGEL M", "")) nomcta
	from t2
	into temp tmp_cta;
drop table t2;
...
begin work;
	insert into ctbt010
		...
		select a.b10_compania, cuenta[1, 8] || '090',
			trim(nomcta) || ' ' || trim(n30_nombres), '',
			a.b10_estado, a.b10_tipo_cta, a.b10_tipo_mov,
			a.b10_nivel, '', a.b10_saldo_ma, 'PATRMOLI', current
			from tmp_cta, ctbt010 a, rolt030
			where b10_compania = 1
			  and b10_cuenta   = cuenta
			  and n30_compania = b10_compania
			  and n30_cod_trab = 185;
	insert into rolt056
	...
	insert into rolt052
		...
		select n52_compania, n52_cod_rubro, 185,
			n52_aux_cont[1, 8] || '090'
			from rolt052
			where n52_compania = 1
			  and n52_cod_trab = 143;
--rollback work;
commit work;
drop table t1;
