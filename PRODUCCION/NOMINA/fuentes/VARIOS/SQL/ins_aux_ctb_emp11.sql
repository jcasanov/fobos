select b10_cuenta cuenta, trim(replace(b10_descripcion,
	(select trim(n30_nombres)
		...
		where n30_compania = b10_compania
		  and n30_cod_trab = 17), " ")) nomcta
	from ctbt010
	where b10_compania     = 1
	  and b10_estado       = 'A'
	  and b10_cuenta      <> '51010104010'
	  and b10_descripcion matches '*CALDERON*'
	into temp t1;
select cuenta, trim(replace(nomcta, "CALDERON CARRENO MOISES ABDIAS","")) nomcta
	from t1
	into temp t2;
drop table t1;
select cuenta, trim(replace(nomcta, "CALDERON CARRENO MOISES A", "")) nomcta
	from t2
	into temp t1;
drop table t2;
select cuenta, trim(replace(nomcta, "CALDERON CARRENO MOISES", "")) nomcta
	from t1
	into temp t2;
drop table t1;
select cuenta, trim(replace(nomcta, "CALDERON CARRENO MOIS", "")) nomcta
	from t2
	into temp t1;
drop table t2;
select cuenta, trim(replace(nomcta, "CALDERON MOISES", "")) nomcta
	from t1
	into temp tmp_cta;
drop table t1;
...
select unique n56_compania cia, n56_proceso pro, n56_cod_depto dep,
	...
	into temp t2;
--select * from t2;
select a.b10_compania cia, cuenta[1, 8] || '081' cta,
...
begin work;
	...
	insert into rolt052
		...
		select n52_compania, n52_cod_rubro, 176,
			n52_aux_cont[1, 8] || '081'
			from rolt052
