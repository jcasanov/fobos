SELECT EXTEND(MDY(n32_mes_proceso,01,n32_ano_proceso),YEAR TO MONTH) AS fecha,
	...
	(SELECT g34_nombre
		...
		WHERE g34_compania  = n30_compania
		  AND g34_cod_depto = n30_cod_depto) AS dpto,
	...
	END AS net_vac,
	ROUND((NVL(CASE WHEN n30_estado = "A" OR n32_cod_trab IN (112, 170) THEN
	...
	GROUP BY 1, 2, 3, 4, 5, 6, 7, 10, 11, 12, 13, 14, 15, 16
UNION
SELECT EXTEND(n41_fecing, YEAR TO MONTH) AS fecha, n42_cod_trab AS codigo,
	...
	NVL((SELECT SUM((n39_valor_vaca + n39_valor_adic)
		...
		WHERE n39_compania      = n30_compania
		  ...
		  AND n39_cod_trab      = n30_cod_trab
		  AND EXTEND(n39_fecing, YEAR TO MONTH) =
				EXTEND(MDY(04, 01, 2013), YEAR TO MONTH)), 0.00) AS net_vac,
	...
	WHERE n41_compania        = 1
	  ...
	  AND n42_proceso         = "UT"
	  AND n42_ano             = n41_ano
          AND n42_cod_trab        NOT IN (112, 170)
...
--
UNION
SELECT EXTEND(MDY(03, 01, 2013), YEAR TO MONTH) AS fecha,
        n30_cod_trab AS codigo, n30_num_doc_id AS cedula,
        TRIM(n30_nombres) AS empleados,
        (SELECT g34_nombre
                FROM gent034
        ...
        CASE WHEN n30_estado = "I"
                THEN NVL((SELECT SUM((n39_valor_vaca + n39_valor_adic)
                        ...
                        WHERE n39_compania      = n30_compania
                          ...
                          AND n39_cod_trab      = n30_cod_trab
                          AND EXTEND(n39_fecing, YEAR TO MONTH) =
                                EXTEND(MDY(03, 01, 2013), YEAR TO MONTH)), 0.00)
        END AS net_vac,
        ROUND((NVL(CASE WHEN n30_estado = "I" THEN
                        ...
                        WHERE n56_compania    = n30_compania
                          ...
                          AND b12_estado     <> 'E'
                          AND EXTEND(b12_fec_proceso, YEAR TO MONTH) =
                                EXTEND(MDY(03, 01, 2013), YEAR TO MONTH)
                          AND b13_compania    = b12_compania
                          AND b13_tipo_comp   = b12_tipo_comp
                          AND b13_num_comp    = b12_num_comp
                          AND b13_valor_base  > 0
...
UNION
SELECT EXTEND(MDY(05, 01, 2013), YEAR TO MONTH) AS fecha,
        n30_cod_trab AS codigo, n30_num_doc_id AS cedula,
        TRIM(n30_nombres) AS empleados,
        (SELECT g34_nombre
                FROM gent034
        ...
        ROUND((NVL(CASE WHEN n30_estado = "I" THEN
                        ...
                        WHERE n56_compania    = n30_compania
                          ...
                          AND b12_estado     <> 'E'
                          AND EXTEND(b12_fec_proceso, YEAR TO MONTH) =
                                EXTEND(MDY(05, 01, 2013), YEAR TO MONTH)
                          AND b13_compania    = b12_compania
                          AND b13_tipo_comp   = b12_tipo_comp
                          AND b13_num_comp    = b12_num_comp
                          AND b13_valor_base  > 0
