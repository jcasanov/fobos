select b10_cuenta cuenta, trim(replace(b10_descripcion,
	(select trim(n30_nombres)
		...
		where n30_compania = b10_compania
		  and n30_cod_trab = 185), " ")) nomcta
	from ctbt010
	where b10_compania    = 1
	  and b10_descripcion matches '*CANTOS*'
	into temp t1;
select cuenta, trim(replace(nomcta, "CANTOS AVILA LUIS MIGUEL","")) nomcta
	from t1
	into temp t2;
drop table t1;
select cuenta, trim(replace(nomcta, "CANTOS AVILA LUIS MIG", "")) nomcta
	from t2
	into temp t1;
drop table t2;
select cuenta, trim(replace(nomcta, "CANTOS AVILA LU", "")) nomcta
	from t1
	into temp tmp_cta;
drop table t1;
select unique n56_compania cia, n56_proceso pro, n56_cod_depto dep,
	...
	where n56_compania = 1
	  and n56_cod_trab = 185
	  --and n56_estado   = "A"
	into temp t2;
select a.b10_compania cia, cuenta[1, 8] || '102' cta,
...
begin work;
	...
	insert into rolt052
		...
		select n52_compania, n52_cod_rubro, 199,
			n52_aux_cont[1, 8] || '102'
			from rolt052
