select b10_cuenta cuenta, trim(replace(b10_descripcion,
	(select trim(n30_nombres)
		...
		where n30_compania = b10_compania
		  and n30_cod_trab = 26), " ")) nomcta
	from ctbt010
	where b10_compania    = 1
	  and b10_estado      = 'A'
	  and (b10_descripcion matches '*CORDOVA*ELIA*'
	   or  b10_descripcion matches '*CORDOVA*EM*')
	into temp t1;
select cuenta, trim(replace(nomcta, "CORDOVA EMEN MANUEL ELIAS", "")) nomcta
	from t1
	into temp t2;
drop table t1;
select cuenta, trim(replace(nomcta, "CORDOVA EMEN MANUEL", "")) nomcta
	from t2
	into temp t1;
drop table t2;
select cuenta, trim(replace(nomcta, "CORDOVA ELIAS", "")) nomcta
	from t1
	into temp tmp_cta;
drop table t1;
...
begin work;
	insert into ctbt010
		...
		select a.b10_compania, cuenta[1, 8] || '069',
	...
	insert into rolt056
		...
		select cia, pro, n30_cod_depto, n30_cod_trab, 'A',
			...
			case when pro[1, 1] <> 'V' and pro <> 'UV'
				then aux2[1, 8] || '069'
				else aux2
			end,
	...
	insert into rolt052
		...
		select n52_compania, n52_cod_rubro, 143,
			n52_aux_cont[1, 8] || '069'
			from rolt052
