select b10_cuenta cuenta, trim(replace(b10_descripcion,
	(select trim(n30_nombres)
		...
		where n30_compania = b10_compania
		  and n30_cod_trab = 177), " ")) nomcta
	from ctbt010
	where b10_compania    = 1
	  and b10_estado      = 'A'
	  and b10_descripcion matches '*CASTRO*'
	into temp t1;
select cuenta, trim(replace(nomcta, "CASTRO SOQUE ALEX ISAAC","")) nomcta
	from t1
	into temp t2;
drop table t1;
select cuenta, trim(replace(nomcta, "CASTRO SOQUE ALEX ISA", "")) nomcta
	from t2
	into temp t1;
drop table t2;
select cuenta, trim(replace(nomcta, "CASTRO SOQUE AL", "")) nomcta
	from t1
	into temp tmp_cta;
drop table t1;
select unique n56_compania cia, n56_proceso pro, n56_cod_depto dep,
	...
	where n56_compania = 1
	  and n56_cod_trab = 177
	into temp t2;
select a.b10_compania cia, cuenta[1, 8] || '087' cta,
...
begin work;
	...
	insert into rolt052
		...
		select n52_compania, n52_cod_rubro, 182,
			n52_aux_cont[1, 8] || '087'
			from rolt052
			where n52_compania = 1
			  and n52_cod_trab = 177;
--rollback work;
commit work;
drop table t2;
