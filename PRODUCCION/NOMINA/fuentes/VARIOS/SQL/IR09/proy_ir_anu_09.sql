...
SELECT n30_compania cia, LPAD(n30_cod_trab, 3, 0) cod,
	...
	{--
	NVL(ROUND(SUM(n32_tot_gan / MONTH((SELECT * FROM fec_act)) * 12), 2),
	0) tot_gan,
	--}
...
SELECT cia, cod, empleado, tot_gan, val_ap, NVL(ROUND(tot_gan - val_ap, 2),
	0) val_nom,
	{--
	NVL(ROUND(tot_gan * n13_porc_trab / 100,2), 0) val_ap,
	NVL(ROUND(tot_gan - (tot_gan * n13_porc_trab / 100), 2), 0) val_nom,
	--}
	otros, bonif, tipo, d_vac
	FROM t1--, rolt013
...
SELECT cod codv, NVL(SUM(b13_valor_base), 0) val_vac, 0.00 ap_vac,
	...
	WHERE n39_compania      = cia
	  ...
	  AND n39_ano_proceso   = YEAR((SELECT * FROM fec_act))
	  AND n39_mes_proceso  <= MONTH((SELECT * FROM fec_act))
...
UNLOAD TO "proyeccion_ir_mes_lanio_p.unl"
