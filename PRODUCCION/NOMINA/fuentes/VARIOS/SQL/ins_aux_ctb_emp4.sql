select b10_cuenta cuenta, trim(replace(b10_descripcion,
	(select trim(n30_nombres)
		...
		where n30_compania = b10_compania
		  and n30_cod_trab = 105), " ")) nomcta
	from ctbt010
	where b10_compania = 1
	  and b10_estado   = 'A'
	  and b10_descripcion matches '*ESPINOZA*MU*'
	into temp t1;
select cuenta, trim(replace(nomcta, "ESPINOZA MURILLO ROSENDO MIG", "")) nomcta
	from t1
	into temp t2;
drop table t1;
select cuenta, trim(replace(nomcta, "ESPINOZA MURILLO ROSENDO", "")) nomcta
	from t2
	into temp t1;
drop table t2;
select cuenta, trim(replace(nomcta, "ESPINOZA MURILLO ROSE", "")) nomcta
	from t1
	into temp tmp_cta;
drop table t1;
...
begin work;
	insert into ctbt010
		...
		select a.b10_compania cia, cuenta[1, 8] || '073' cta,
	...
	insert into rolt052
		...
		select n52_compania, n52_cod_rubro, 165,
			case when n52_cod_rubro <> 27
				then n52_aux_cont[1, 8] || '073'
				else n52_aux_cont
			end
			from rolt052
			where n52_compania = 1
			  and n52_cod_trab = 105;
--rollback work;
commit work;
drop table t2;
