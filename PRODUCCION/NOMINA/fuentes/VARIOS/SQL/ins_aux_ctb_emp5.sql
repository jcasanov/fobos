select b10_cuenta cuenta, trim(replace(b10_descripcion,
	(select trim(n30_nombres)
		...
		where n30_compania = b10_compania
		  and n30_cod_trab = 77), " ")) nomcta
	from ctbt010
	where b10_compania = 1
	  and b10_estado   = 'A'
	  and b10_descripcion matches '*VALENCIA*'
	into temp t1;
select cuenta, trim(replace(nomcta, "VALENCIA NAZARENO HERIBERTO", "")) nomcta
	from t1
	into temp t2;
drop table t1;
select cuenta, trim(replace(nomcta, "VALENCIA NAZARENO HERIBER", "")) nomcta
	from t2
	into temp t1;
drop table t2;
select cuenta, trim(replace(nomcta, "VALENCIA NAZARENO HER", "")) nomcta
	from t1
	into temp t2;
drop table t1;
select cuenta, trim(replace(nomcta, "VALENCIA HERIBER", "")) nomcta
	from t2
	into temp tmp_cta;
drop table t2;
...
begin work;
	...
	insert into rolt052
		...
		select n52_compania, n52_cod_rubro, 166,
			case when n52_cod_rubro <> 27
				then n52_aux_cont[1, 8] || '074'
				else n52_aux_cont
			end
			from rolt052
