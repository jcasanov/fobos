select b10_cuenta cuenta, trim(replace(b10_descripcion,
	(select trim(n30_nombres)
		...
		where n30_compania = b10_compania
		  and n30_cod_trab = 187), " ")) nomcta
	from ctbt010
	where b10_compania    = 1
	  and b10_descripcion matches '*COBOS*'
	into temp t1;
select cuenta, trim(replace(nomcta, "COBOS SAENZ DE VITERI RUBEN FE","")) nomcta
	from t1
	into temp t2;
drop table t1;
select cuenta, trim(replace(nomcta, "COBOS SAENZ DE VITERI RUBEN", "")) nomcta
	from t2
	into temp t1;
drop table t2;
select cuenta, trim(replace(nomcta, "COBOS SAENZ DE VITERI RUBE", "")) nomcta
	from t1
	into temp t2;
drop table t1;
select cuenta, trim(replace(nomcta, "COBOS SAENZ DE VITERI RUB", "")) nomcta
	from t2
	into temp t1;
drop table t2;
select cuenta, trim(replace(nomcta, "COBOS SAENZ DE VITERI", "")) nomcta
	from t1
	into temp t2;
drop table t1;
select cuenta, trim(replace(nomcta, "COBOS SAENZ DE", "")) nomcta
	from t2
	into temp tmp_cta;
drop table t2;
...
begin work;
	...
	insert into rolt056
		...
		select cia, pro, n30_cod_depto, n30_cod_trab, 'A',
			...
			aux3, aux4, aux5, aux6, 'PAULPERA', current
			from t2, rolt030
			where n30_compania = cia
			  and n30_cod_trab = 213;
	insert into rolt052
		(n52_compania, n52_cod_rubro, n52_cod_trab, n52_aux_cont)
		select n52_compania, n52_cod_rubro, 213,
			n52_aux_cont[1, 8] || '116'
			from rolt052
			where n52_compania = 1
			  and n52_cod_trab = 187;
--rollback work;
commit work;
drop table t2;
