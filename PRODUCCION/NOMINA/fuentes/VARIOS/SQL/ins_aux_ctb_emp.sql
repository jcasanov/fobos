select b10_cuenta cuenta, trim(replace(b10_descripcion,
	(select trim(n30_nombres)
		...
		where n30_compania = b10_compania
		  and n30_cod_trab = 140), " ")) nomcta
	from ctbt010
	where b10_compania = 1
	  and b10_estado   = 'A'
	  and b10_descripcion matches '*AYALA*'
	into temp t1;
select cuenta, trim(replace(nomcta, "AYALA MARQUEZ WILSON ROLAN", "")) nomcta
	from t1
	into temp t2;
drop table t1;
select cuenta, trim(replace(nomcta, "AYALA MARQUEZ WILSON ROLA", "")) nomcta
	from t2
	into temp t1;
drop table t2;
select cuenta, trim(replace(nomcta, "AYALA MARQUEZ WILSON", "")) nomcta
	from t1
	into temp t2;
drop table t1;
select cuenta, trim(replace(nomcta, "AYALA MARQUEZ WI", "")) nomcta
	from t2
	into temp tmp_cta;
drop table t2;
...
begin work;
	insert into ctbt010
		...
		select a.b10_compania, cuenta[1, 8] || '068',
	...
	insert into rolt052
		...
		select n52_compania, n52_cod_rubro, 142,
			n52_aux_cont[1, 8] || '068'
			from rolt052
			where n52_compania = 1
			  and n52_cod_trab = 140;
--rollback work;
commit work;
drop table t1;
