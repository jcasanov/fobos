...
SELECT n30_cod_trab codv, NVL(SUM(b13_valor_base), 0) val_vac, 0.00 ap_vac,
	NVL(SUM(b13_valor_base), 0) net_vac
	...
	GROUP BY 1, 3
UNION
SELECT n30_cod_trab codv, NVL(SUM(n39_valor_vaca + n39_valor_adic), 0) val_vac,
	NVL(SUM(n39_descto_iess), 0) ap_vac,
	NVL(SUM((n39_valor_vaca + n39_valor_adic) - n39_descto_iess), 0) net_vac
...
SELECT LPAD(n30_cod_trab, 3, 0) cod, TRIM(n30_nombres) empleado,
	...
	NVL((SELECT val_vac
		FROM tmp_vac
		WHERE codv = n30_cod_trab), 0.00) val_vac,
	NVL((SELECT ap_vac
		FROM tmp_vac
		WHERE codv = n30_cod_trab), 0.00) ap_vac,
	NVL((SELECT net_vac
		FROM tmp_vac
