 ...
 * Autor                : GVA
 * Formato de Ejecución : fglrun repp102 base RE 1 
 *}
...
CLEAR FORM
LET int_flag = 0
CONSTRUCT BY NAME expr_sql ON r02_codigo, r02_nombre, r02_estado, r02_tipo,
			      r02_area, r02_factura, r02_localidad, r02_usuario,
			      r02_fecing
	ON KEY(F2)
...
IF int_flag THEN
	...
	RETURN
END IF
LET query = 'SELECT *, ROWID FROM rept002 WHERE ', expr_sql CLIPPED,
		' ORDER BY 3, 5'
...
BEGIN WORK
DECLARE q_up CURSOR FOR SELECT * FROM rept002 WHERE ROWID = vm_r_rows[vm_row_current]
	FOR UPDATE
OPEN q_up
...
INPUT BY NAME rm_bod.r02_codigo, rm_bod.r02_nombre, rm_bod.r02_localidad,
	      rm_bod.r02_factura, rm_bod.r02_tipo, rm_bod.r02_area
              WITHOUT DEFAULTS
        ON KEY(INTERRUPT)
        	 IF field_touched(rm_bod.r02_nombre, rm_bod.r02_factura,
	...
	AFTER FIELD r02_localidad
               IF rm_bod.r02_localidad IS NOT NULL THEN
		     CALL fl_lee_localidad(vg_codcia, rm_bod.r02_localidad)
		     RETURNING rm_loc.*
		     IF rm_loc.g02_localidad IS NULL THEN
			   CALL fgl_winmessage(vg_producto, 'No existe la localidad ','exclamation')
	...
	AFTER INPUT
                IF vm_flag_mant = 'I' THEN
                        ...
                        IF rm_bod2.r02_codigo IS NOT NULL THEN
                                CALL fgl_winmessage (vg_producto, 'La Bodega ya existe en la base de datos','exclamation')
                                NEXT FIELD r02_codigo
                        END IF
                END IF
		IF vm_flag_mant = 'I'
		OR rm_bod2.r02_nombre <> rm_bod.r02_nombre
		THEN
	      		SELECT r02_codigo INTO codigo FROM rept002
	      		WHERE r02_compania = vg_codcia
	      		AND   r02_nombre   = rm_bod.r02_nombre
	      		IF status <> NOTFOUND THEN
                 		CALL fgl_winmessage (vg_producto, 'El nombre de la bodega ya ha sido asignada al registro de codigo  '|| codigo,'exclamation')
