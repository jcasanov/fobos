...
DEFER INTERRUPT
CLEAR SCREEN
CALL startlog('../logs/repp419.err')
--#CALL fgl_init4js()
CALL fl_marca_registrada_producto()
IF num_args() <> 5 AND num_args() <> 6 THEN	-- Validar # parametros correcto
...
ELSE
	LET comando = 'prof', vm_proforma USING "<<<<<<<<<<&", '_2.txt'
	START REPORT report_proforma TO FILE comando
END IF
{--
LET cambio_precios = retorna_precio_validez_item()
IF rm_r21.r21_cod_tran IS NULL AND cambio_precios THEN
...
FOREACH q_rept022 INTO r_r22.*
	...
	IF cambio_precios THEN
		--LET r_rep.precio = r_r10.r10_precio_mb
	END IF
...
ON EVERY ROW
	...
	PRINT COLUMN 02,  r_rep.r22_item[1,7],
	      COLUMN 11,  r_rep.desc_clase[1, 44] CLIPPED,
