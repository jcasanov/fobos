--------------------------------------------------------------------------------
-- Titulo           : repp253.4gl - Ingreso Tranferencia Bodega Compras Públicas
-- Elaboracion      : 19-ene-2015
-- Autor            : NPC
-- Formato Ejecucion: fglrun repp253 base modulo compania localidad
...
DEFER INTERRUPT
CLEAR SCREEN
CALL startlog('../logs/repp253.err')
--#CALL fgl_init4js()
...
IF vg_gui = 0 THEN
	...
	LET num_cols = 78
END IF
OPEN WINDOW w_repf253_1 AT row_ini, 2 WITH num_rows ROWS, num_cols COLUMNS
	ATTRIBUTE(FORM LINE FIRST + 1, COMMENT LINE LAST, MENU LINE lin_menu,
		  BORDER, MESSAGE LINE LAST - 1) 
IF vg_gui = 1 THEN
	OPEN FORM f_repf253_1 FROM '../forms/repf253_1'
ELSE
	OPEN FORM f_repf253_1 FROM '../forms/repf253_1c'
END IF
DISPLAY FORM f_repf253_1
...
INPUT BY NAME rm_r19.r19_vendedor,    rm_r19.r19_bodega_ori,
	...
	ON KEY(F2)
		...
		IF INFIELD(r19_bodega_ori) THEN
		     	CALL fl_ayuda_bodegas_rep(vg_codcia, vg_codloc, 'A', 'F', 'R', 'T', '7')
	...
	AFTER FIELD r19_codcli
		IF rm_r19.r19_codcli IS NOT NULL THEN    
			...
			CALL fl_lee_cliente_localidad(vg_codcia, vg_codloc, 
				rm_r19.r19_codcli) RETURNING r_z02.*
			...
			IF (rm_r19.r19_cedruc IS NOT NULL AND
			    rm_r19.r19_codcli IS NULL) OR
			   (rm_r19.r19_codcli IS NOT NULL AND
			    r_z01.z01_tipo_doc_id <> 'P')
			THEN
				INITIALIZE r_g10.* TO NULL
				DECLARE q_g10 CURSOR FOR
					SELECT * FROM gent010
					WHERE g10_codcobr = rm_r19.r19_codcli
				OPEN q_g10
				...
				IF r_g10.g10_codcobr IS NOT NULL THEN
					...
					NEXT FIELD r19_codcli
				END IF
				CALL fl_validar_cedruc_dig_ver(rm_r19.r19_cedruc)
					RETURNING resul
				IF NOT resul THEN
					NEXT FIELD r19_codcli
				END IF  
			END IF
		ELSE
			LET rm_r19.r19_nomcli = NULL
			LET rm_r19.r19_dircli = NULL
			LET rm_r19.r19_telcli = NULL
			LET rm_r19.r19_cedruc = NULL
			DISPLAY BY NAME rm_r19.r19_nomcli
		END IF  
	AFTER INPUT
		IF rm_r19.r19_codcli IS NULL THEN
			CALL fl_mostrar_mensaje('Digite un código de cliente.', 'exclamation')
			NEXT FIELD r19_codcli
		END IF
		IF NOT tiene_bodega_cp() THEN
			CALL fl_mostrar_mensaje('Ya sea en la bodega origen o destino, debe digitar la BODEGA DE COMPRAS PUBLICAS para esta localidad.','exclamation')
			CONTINUE INPUT
		END IF
...
IF num_args() = 4 THEN
	...
	CONSTRUCT BY NAME expr_sql 
			  ON r19_num_tran,    r19_vendedor, r19_bodega_ori, 
			     r19_bodega_dest, r19_referencia, r19_codcli,
...
DISPLAY BY NAME rm_r19.r19_num_tran,    rm_r19.r19_cod_tran, 
		rm_r19.r19_codcli,	rm_r19.r19_vendedor, 
