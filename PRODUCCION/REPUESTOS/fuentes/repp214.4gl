...
-- CADA VEZ QUE SE REALICE UNA CONSULTA SE GUARDARAN LOS ROWID DE CADA FILA 
...
DEFINE vm_num_guia		LIKE ordt013.c13_num_guia
--
...
DEFER INTERRUPT
CLEAR SCREEN
CALL startlog('../logs/repp214.error')
CALL fgl_init4js()
...
OPTIONS
	INPUT WRAP,
	ACCEPT KEY F12
OPEN WINDOW w_214 AT 3,2 WITH 22 ROWS, 80 COLUMNS
...
DEFINE cantidad_ped	SMALLINT
DEFINE cantidad_rec	SMALLINT
...
IF INT_FLAG THEN
	...
	END IF
	IF vm_indice = 0 THEN
		UPDATE ordt010 set c10_estado = 'C' WHERE CURRENT OF q_c10
		COMMIT WORK
		RETURN
	END IF	
	ROLLBACK WORK
...
SELECT SUM(c11_cant_ped), SUM(c14_cantidad) INTO cantidad_ped, cantidad_rec
	FROM ordt011, ordt014
	WHERE c11_compania  = vg_codcia
	  AND c11_localidad = vg_codloc
	  AND c11_numero_oc = rm_r19.r19_oc_interna
	  AND c14_compania  = c11_compania
	  AND c14_localidad = c11_localidad
	  AND c14_numero_oc = c11_numero_oc
...
IF cantidad_ped <> cantidad_rec THEN
	...
	IF NOT done THEN
		CALL fgl_winquestion(vg_producto,
			'No se han recibido todos los articulos de la orden '||
			' de compra, desea poder recibirlos despues?', 'No', 
			'Yes|No', 'question', 1) RETURNING resp
		IF resp = 'Yes' THEN
			LET estado = 'P'
		END IF
	END IF
...
INITIALIZE vm_num_aut, vm_serie_comp, vm_num_guia TO NULL

LET INT_FLAG = 0
INPUT BY NAME rm_r19.r19_cod_tran, rm_r19.r19_oc_interna, vm_num_aut, 
	      vm_serie_comp, rm_r19.r19_oc_externa, vm_num_guia,
	      rm_r19.r19_vendedor,
              rm_r19.r19_usuario, rm_r19.r19_fecing 
	WITHOUT DEFAULTS
	ON KEY(INTERRUPT)
		IF NOT FIELD_TOUCHED(r19_oc_interna, r19_oc_externa, 
				     r19_vendedor
                                    ) THEN
			RETURN
		END IF
	...
	AFTER FIELD r19_oc_interna
		...
		IF r_c10.c10_estado <> 'P' THEN
			CALL fgl_winmessage(vg_producto,
				'No puede realizar una compra local de ' ||
	...
	AFTER INPUT
		...
		IF r_p20.p20_num_doc IS NOT NULL THEN
			CALL fgl_winmessage(vg_producto,
				'Esta factura ya existe para este provedor.',
				'exclamation')
			NEXT FIELD r19_oc_externa
		END IF
...
CONSTRUCT BY NAME expr_sql 
	ON r19_num_tran, r19_oc_interna, r19_oc_externa, vm_num_guia,
	   r19_vendedor, rm_r19.r19_usuario
	ON KEY(F2)
		IF INFIELD(r19_oc_interna) THEN
			CALL fl_ayuda_ordenes_compra(vg_codcia, vg_codloc, 
				0, 0, 'P', vg_modulo, 'S')
...
DECLARE q_sri CURSOR FOR
	SELECT c13_num_aut, c13_serie_comp, c13_num_guia 
	  FROM ordt013
	 WHERE c13_compania  = vg_codcia 
       AND c13_localidad = vg_codloc
	   AND c13_numero_oc = rm_r19.r19_oc_interna
...
OPEN  q_sri 
FETCH q_sri INTO vm_num_aut, vm_serie_comp, vm_num_guia
CLOSE q_sri
FREE  q_sri

DISPLAY BY NAME vm_num_aut, vm_serie_comp, vm_num_guia 
...
FOREACH q_det INTO r_r20.*, r_r10.*          
	...
	SELECT SUM(c11_cant_ped), SUM(c14_cantidad) 
	  INTO rm_compra[i].cant_ped, rm_compra[i].cant_ven
	    FROM ordt011, ordt014
	    WHERE c11_compania  = vg_codcia
	      AND c11_localidad = vg_codloc
	      AND c11_numero_oc = rm_r19.r19_oc_interna
...
DEFINE cant_rec_pro	LIKE ordt011.c11_cant_rec
...
FOREACH q_ord INTO r_c11.*, r_r10.*
	...
	SELECT SUM(c14_cantidad) INTO cant_rec_pro 
	...
	IF cant_rec_pro IS NULL THEN
		LET cant_rec_pro = 0
	END IF

	LET rm_compra[i].cant_ped   = r_c11.c11_cant_ped - r_c11.c11_cant_rec
	LET rm_compra[i].cant_ven   = cant_rec_pro
	...
	LET rm_datos[i].item        = r_c11.c11_descrip
...
LET r_r20.r20_cant_ped   = 0
LET r_r20.r20_cant_ven   = 0
...
FOR i = 1 TO vm_indice
	...
	LET costo_ing = rm_compra[i].precio - descto_unit
...
WHILE (STATUS <> NOTFOUND)
	FOR i = 1 TO vm_indice
		IF r_c11.c11_codigo = rm_compra[i].item THEN
			UPDATE ordt011 SET c11_cant_rec = rm_compra[i].cant_ven
...
LET comando = 'cd ..', vg_separador, '..', vg_separador, 'COMPRAS', 
	vg_separador, 'fuentes', vg_separador, '; fglrun ordp200 ', vg_base, 
...
INPUT BY NAME pagos, c10_interes, fecha_pago, dias_pagos WITHOUT DEFAULTS
	...
	AFTER FIELD fecha_pago
		IF fecha_pago < TODAY OR fecha_pago IS NULL THEN
			CALL fgl_winmessage(vg_producto, 
				'Debe ingresar una fecha mayor o igual a ' ||
				'la de hoy.',
				'exclamation')
	...
	INPUT ARRAY r_detalle_2 WITHOUT DEFAULTS FROM r_detalle_2.*
		...
		AFTER INPUT 
			IF tot_cap <> rm_r19.r19_tot_neto THEN
				CALL fgl_winmessage(vg_producto,
					'El valor capital no coincide ' ||
					'con el valor neto de la deuda.',
					'exclamation')
				CONTINUE INPUT
			END IF
			LET salir = 1
...
IF rm_r19.r19_cont_cred = 'C' THEN
	CALL fgl_winmessage(vg_producto,
		'Esta compra se realizó al contado',
		'exclamation')
	RETURN
END IF
...
MENU 'OPCIONES'
	...
	COMMAND KEY('F') 'Forma Pago'		'Ver forma de pago.'
		...
		ELSE
			CALL fgl_winmessage(vg_producto,
				'La compra se está realizando al contado.',
				'exclamation')
		END IF
	COMMAND KEY('R') 'Retenciones'		'Ver retenciones.'
		...
		ELSE
			CALL fgl_winmessage(vg_producto,
				'La compañía realiza las retenciones al ' || 
				'pagar la factura.',
				'exclamation')
		END IF
	COMMAND KEY('G') 'Grabar'		'Graba compra local.'
		...
		IF r_c00.c00_cuando_ret = 'C' AND tot_ret = 0 THEN
			CALL fgl_winquestion(vg_producto,
			'No se han indicado retenciones. '||
			'Seguro de generar la Compra Local sin retenciones?', 'No', 
			'Yes|No', 'question', 1) RETURNING resp
			IF resp <> 'Yes' THEN
				CONTINUE MENU
			END IF
...
LET comando = 'cd ..', vg_separador, '..', vg_separador, 'REPUESTOS', 
	vg_separador, 'fuentes', vg_separador, '; fglrun repp218 ', vg_base, 
...
IF r_p20.p20_num_doc IS NULL THEN
	CALL fgl_winmessage(vg_producto,
		'No se generaron documentos para esta compra local.',
		'exclamation')
	RETURN
END IF
...
DECLARE q_p28 CURSOR FOR
	SELECT 'S', c02_nombre, p28_codigo_sri, p28_tipo_ret, p28_valor_base, 
		...
		WHERE p28_compania   = vg_codcia
		  ...
		  AND c02_codigo_sri = p28_codigo_sri
...
LET comando = 'cd ..', vg_separador, '..', vg_separador, 'REPUESTOS', 
	vg_separador, 'fuentes', vg_separador, '; fglrun repp413 ', vg_base, 
	' ', 'RE', vg_codcia, ' ', vg_codloc, ' ', rm_r19.r19_cod_tran,
	rm_r19.r19_num_tran
