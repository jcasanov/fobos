...
DEFER INTERRUPT
CLEAR SCREEN
CALL startlog('../logs/repp430.error')
CALL fgl_init4js()
...
ON EVERY ROW
	...
	{*
	 * El valor pasado en r_rep.r19_tot_costo y total_cos no es correcto porque 
	 * refleja el costo de ingreso por unidad, por eso debo recalcular usando la
	 * rept117.
	 *}	
	-- restamos de total_cos el valor pasado de tot_costo
	LET total_cos = total_cos - r_rep.r19_tot_costo
	-- recalculamos
	SELECT SUM(r117_fob*r117_cantidad), SUM(r20_costo*r117_cantidad) 
	  INTO total_fob, r_rep.r19_tot_costo
	  FROM rept019, rept117, rept020
	 WHERE r19_compania  = r_rep.r19_compania
	   ...
	   AND r19_compania  = r117_compania
	   AND r19_localidad = r117_localidad
	   AND r20_item      = r117_item
	   AND r19_numliq    = r117_numliq 
	-- luego sumamos el valor recalculado
	LET total_cos = total_cos + r_rep.r19_tot_costo
...
ON LAST ROW
	...
	PRINT COLUMN 59,  "----------------",
	      COLUMN 77,  "----------------"
	PRINT COLUMN 48, "TOTAL ==>  ",SUM(total_fob) USING "-,---,---,--&.##",
	      COLUMN 77, SUM(r_rep.r19_tot_costo) USING "-,---,---,--&.##"
