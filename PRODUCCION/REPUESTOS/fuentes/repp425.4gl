{*
 * Titulo           : repp425.4gl - Listado Detallado de Transacciones Repuestos
 * Elaboracion      : 27-abr-2010
 * Autor            : JCM
 * Formato Ejecucion: fglrun repp425 base módulo compañía localidad
 *}
...
DEFER INTERRUPT
CLEAR SCREEN
CALL startlog('../logs/repp425.error')
CALL fgl_init4js()
...
OPTIONS INPUT WRAP,
	ACCEPT KEY	F12
OPEN FORM f_rep FROM "../forms/repf425_1"
DISPLAY FORM f_rep
...
WHILE TRUE
	...
	FOREACH q_deto INTO r_rep.*, r_r20.*
		...
		IF rm_rep.r19_cod_tran = 'AC' OR rm_rep.r19_cod_tran = 'A+'
	  	  OR rm_rep.r19_cod_tran = 'A-' THEN
			OUTPUT TO REPORT rep_ajustes (r_rep.*, r_r20.*)
		END IF
		IF int_flag THEN
			EXIT FOREACH
		END IF
...
LET int_flag = 0
INPUT BY NAME rm_rep.r19_moneda, vm_linea, rm_rep.r19_cod_tran, vm_fecha_ini, vm_fecha_fin
	WITHOUT DEFAULTS
	ON KEY(INTERRUPT)
		LET int_flag = 1
		RETURN
...
--	LET titulo = modulo, titulo 
	...
	PRINT COLUMN 27, "** Moneda        : ", rm_rep.r19_moneda,
						" ", vm_moneda_des
	IF vm_linea IS NOT NULL THEN
		PRINT COLUMN 27, "** Linea         : ", vm_linea
	ELSE
		PRINT COLUMN 27, "** Linea         : T O D A S "
	END IF
...
REPORT rep_ajustes (r_rep, r_r20)
DEFINE r_rep		RECORD LIKE rept019.*
DEFINE r_r20		RECORD LIKE rept020.*
DEFINE total_cos	DECIMAL(12,2)
DEFINE r_gen		RECORD LIKE gent021.*
...
--	LET titulo = modulo, titulo 
	...
	PRINT COLUMN 1,   "Referencia",
	      ...
	      COLUMN 104, "Numero",
	      COLUMN 112, "Cant. Ven.",
	      COLUMN 124, fl_justifica_titulo("D", "Costo", 14),
	      COLUMN 142, fl_justifica_titulo("D", "Costo Total", 14)
	PRINT "-------------------------------------------------------------------------------------------"
...
ON EVERY ROW
	...
	PRINT COLUMN 1,   referen,
	      ...
	      COLUMN 104, r_rep.r19_num_tran USING "#####&",
	      COLUMN 112, r_r20.r20_cant_ven * signo USING "--,---,--&",
	      COLUMN 124, r_r20.r20_costo * signo USING "---,---,--&.&&",
	      COLUMN 142, r_r20.r20_costo * r_r20.r20_cant_ven * signo 
		  				USING "---,---,--&.&&"
