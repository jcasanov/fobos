...
END IF
CLOSE q_deto
START REPORT transferencia TO PIPE comando
FOREACH q_deto INTO r_r20.*, n_item
	OUTPUT TO REPORT transferencia(r_r20.r20_cant_ven,
				     r_r20.r20_item,
				     n_item,
				     r_r20.r20_costo)
END FOREACH
...
REPORT transferencia(cant_ven, item, descripcion, costo)
...
PAGE HEADER
	...
	LET documento = 'TRANSFERENCIA # ' || rm_r19.r19_num_tran
	...
	PRINT COLUMN 1,  rm_cia.g01_razonsocial,
	      COLUMN 86, "Página: ", PAGENO USING "&&&"
	PRINT COLUMN 1,  titulo CLIPPED,
	      COLUMN 86, UPSHIFT(vg_proceso)

	SKIP 1 LINES
--	cabecera de la transferencia
	PRINT COLUMN 1, fl_justifica_titulo('I', 'Bodega Origen', 15), ': ', 
			fl_justifica_titulo('I', rm_r19.r19_bodega_ori, 5),
			'  ', rm_r02_ori.r02_nombre,
	      COLUMN 65, fl_justifica_titulo('I', 'Fecha Transferencia', 19), 
	      		': ', DATE(rm_r19.r19_fecing) USING 'dd-mmm-yyyy'
	      		
	PRINT COLUMN 1, fl_justifica_titulo('I', 'Bodega Destino', 15), ': ',  
			fl_justifica_titulo('I', rm_r19.r19_bodega_dest, 5),
			'  ', rm_r02_dest.r02_nombre,
	      COLUMN 65, fl_justifica_titulo('I', 'Moneda', 19), ': ',
	      		rm_g13.g13_nombre
...
--
	...
	PRINT COLUMN 1,  'Cant.',
	      ...
	      COLUMN 62, fl_justifica_titulo('D', 'Costo', 16),
	      COLUMN 80, fl_justifica_titulo('D', 'Subtotal', 16)
...
ON EVERY ROW
	...
	PRINT COLUMN 1,  cant_ven USING "####&",
	      ...
	      COLUMN 62, costo USING "#,###,###,##&.##",
	      COLUMN 80, (cant_ven * costo) USING "#,###,###,##&.##"
	
ON LAST ROW
	NEED 2 LINES
	PRINT COLUMN 80, '----------------'
	PRINT COLUMN 68, 'Costo Total ', 
