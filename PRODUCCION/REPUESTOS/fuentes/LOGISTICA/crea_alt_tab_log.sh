for i in aceros@$1 acero_gm@$1 acero_gc@$1 acero_qm@$1 acero_qs@$1
	do
		...
		if [ "$i" == "aceros@$1" ] || [ "$i" == "acero_gm@$1" ]; then
			...
			dbaccess $i car_ciu_01.sql;
		fi

		if [ "$i" == "acero_gc@$1" ]; then
			echo "Actualizando provincia en $i ...";
			dbaccess $i car_ciu_02.sql;
		fi

		if [ "$i" == "acero_qm@$1" ]; then
			echo "Actualizando provincia en $i ...";
			dbaccess $i car_ciu_03.sql;
			echo "Cargando ayudantes en $i ...";
			dbaccess $i carg_ayu_uio.sql;
		fi
