{*
 * Titulo           : repp421.4gl - Impresión recepción compra local/importacion
 * Elaboracion      : 29-ago-2008
 * Autor            : JCM
 * Formato Ejecucion: fglrun repp421 base módulo compañía localidad 
 *		              tipo_tran num_tran
 *}
...
DEFER INTERRUPT
CLEAR SCREEN
CALL startlog('../logs/repp421.error')
CALL fgl_init4js()
...
END IF
CLOSE q_deto
START REPORT recepcion TO PIPE comando
FOREACH q_deto INTO r_r20.*, n_item
	OUTPUT TO REPORT recepcion(r_r20.r20_cant_ven,
				     r_r20.r20_item,
				     n_item,
				     r_r20.r20_descuento,
				     r_r20.r20_precio,
				     r_r20.r20_val_impto)
END FOREACH
FINISH REPORT recepcion

END FUNCTION



REPORT recepcion(cant_ven, item, descripcion, descuento, precio, impto)
...
PAGE HEADER
	...
	CASE rm_r19.r19_cod_tran 
		WHEN 'IC' 
			LET documento = 'COMP. INGRESO POR COMPRA LOCAL # ' || rm_r19.r19_num_tran
		WHEN 'IX'
			LET documento = 'COMP. INGRESO POR IMPORTACION # ' || rm_r19.r19_num_tran
	END CASE
	...
	PRINT COLUMN 1, rm_r19.r19_referencia
	PRINT COLUMN 1, 'Fecha recepcion: ', 	
					DATE(rm_r19.r19_fecing) USING 'dd-mmm-yyyy'
	PRINT COLUMN 1, 'Bodega: ', rm_r02.r02_nombre
{
	INITIALIZE r_r16.* TO NULL
	SELECT rept016.* INTO r_r16.*
	  FROM rept117, rept016 
	 WHERE r117_compania  = rm_r19.r19_compania
	   AND r117_localidad = rm_r19.r19_localidad
	   AND r117_cod_tran  = rm_r19.r19_cod_tran
	   AND r117_num_tran  = rm_r19.r19_num_tran
	   AND r16_compania   = r117_compania
	   AND r16_localidad  = r117_localidad
	   AND r16_pedido     = r117_pedido

	PRINT COLUMN 1, 'Referencia pedido: ', r_r16.r16_referencia CLIPPED
}
--
