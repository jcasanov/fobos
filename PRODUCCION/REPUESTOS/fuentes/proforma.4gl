...
IF num_args() = 5 OR num_args() = 4 THEN  
	LET vm_ruta = FGL_GETENV('MAILDOC')
	DISPLAY 'Valor de variable de ambiente MAILDOC: ', vm_ruta
	IF vm_ruta IS NULL THEN
		LET vm_ruta = '/u/home/publico/doc'
	END IF
	DISPLAY 'Ruta a donde grabar la proforma      : ', vm_ruta
	DISPLAY ''
	...
	IF num_args() = 4 THEN  
		PROMPT 'Ingrese proforma: ' FOR vm_proforma 
			ON KEY (INTERRUPT)
				EXIT PROGRAM
		END PROMPT
	END IF
...
IF rm_r21.r21_numprof IS NULL THEN	
	...
	EXIT PROGRAM
END IF
SELECT COUNT(*) INTO vm_num_item FROM rept022
...
LET comando = vm_ruta CLIPPED, '/', vm_proforma USING '&&&&&&', '.txt'
START REPORT report_proforma TO comando
LET vm_num_lineas = 0
...
--	print '&k2S' 		-- Letra condensada
	...
	PRINT COLUMN 01,  "OBSERVACION     : ", rm_r21.r21_atencion,
	      COLUMN 69,  "DIRECCION      : ", rm_loc.g02_direccion
	PRINT COLUMN 01,  "ENTREGAR EN     : ", rm_r21.r21_referencia,
...
ON EVERY ROW
	...
	PRINT COLUMN 13,  r_rep.descripcion,
	      COLUMN 84,  r_rep.cantidad	USING '####&.##',
