...
LET int_flag = 0
CONSTRUCT expr_sql ON r10_codigo, r10_nombre, r10_stock_max, r10_stock_min
...
SELECT r10_sec_item r10_codigo, r10_nombre, r11_stock_act stock_pend,
	...
	INTO TEMP t_item
SELECT * FROM t_item INTO TEMP t_item_tot
...
EXECUTE pre_r11
LET query = 'INSERT INTO t_item_tot ',
		' SELECT r10_codigo, r10_nombre, stock_p1, ',
			' NVL(SUM(r11_stock_act), 0) stock_tot, stock_l1, ',
			' r10_stock_max, r10_stock_min ',
			' FROM t_r10, t_r11 ',
			' WHERE r11_bodega IN (SELECT r02_codigo FROM t_bod) ',
			'   AND r11_item    = r10_codigo ',
			' GROUP BY 1, 2, 3, 5, 6, 7'
PREPARE cit_tot FROM query
EXECUTE cit_tot
LET query = 'INSERT INTO t_item_loc ',
		' SELECT r10_codigo, NVL(SUM(r11_stock_act), 0) stock_l ',
			' FROM t_r10, t_r11 ',
			' WHERE r11_bodega IN (SELECT r02_codigo FROM t_bod ',
...
DROP TABLE t_r10
INSERT INTO t_item
	SELECT r10_codigo, r10_nombre, stock_pend, stock_tot, NVL(stock_l, 0),
			r10_stock_max, r10_stock_min
...
LET expr_item = NULL
IF rm_par.pendientes = 'S' THEN
	LET expr_item = '   AND r20_item      IN ',
				'(SELECT r10_codigo FROM t_item) '
END IF
...
SELECT r34_compania, r34_localidad, r34_bodega, r34_num_ord_des, r34_cod_tran,
	...
	INTO TEMP t_r34
LET query = 'SELECT r20_cod_tran, r20_num_tran, fecha, r20_bodega, r20_item, ',
			'r20_cant_ven, r34_num_ord_des ',
		' FROM t1, t_r34 ',
		' WHERE r34_compania  = ', vg_codcia,
		'   AND r34_localidad = ', vg_codloc,
...
DEFINE item		LIKE rept010.r10_codigo
DEFINE r_factura	ARRAY[200] OF RECORD
