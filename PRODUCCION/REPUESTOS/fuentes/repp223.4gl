...
WHILE NOT salir
	...
	INPUT ARRAY r_detalle WITHOUT DEFAULTS FROM r_detalle.*
		...
		ON KEY(F7)
			LET i = arr_curr()
			CALL transmitir_factura(r_detalle[i].num_preventa)
			LET int_flag = 0
		BEFORE INPUT
			...
			--#CALL dialog.keysetlabel("F7","Transmitir Factura")
		BEFORE ROW
	...
	FOR m = 1 TO arr_count()
		UPDATE temp_prev
			SET aprobar = r_detalle[m].aprobar
...
FOREACH q_cons INTO r_detalle_2[i].r23_nomcli,    r_detalle_2[i].r23_estado,
	...
	INSERT INTO temp_prev
		VALUES (r_detalle[i].*, r_detalle_2[i].r23_nomcli,
			r_detalle_2[i].r01_nombres)
...
FUNCTION transmitir_factura(num_prev)
DEFINE num_prev		LIKE rept023.r23_numprev
DEFINE r_r23		RECORD LIKE rept023.*
DEFINE fec_tr_fa	LIKE rept021.r21_fec_tr_fa
DEFINE mensaje		VARCHAR(250)
DEFINE resp		CHAR(6)

CALL fl_lee_preventa_rep(vg_codcia, vg_codloc, num_prev) RETURNING r_r23.*
BEGIN WORK
	WHENEVER ERROR CONTINUE
	DECLARE q_trans_fact CURSOR FOR
		SELECT * FROM rept021
			WHERE r21_compania  = r_r23.r23_compania
			  AND r21_localidad = r_r23.r23_localidad
			  AND r21_numprof   = r_r23.r23_numprof
		FOR UPDATE
	OPEN q_trans_fact 
	FETCH q_trans_fact
	IF STATUS < 0 THEN
	...
	IF STATUS = NOTFOUND THEN
		ROLLBACK WORK
		WHENEVER ERROR STOP 
		CALL fl_mostrar_mensaje('El registro de proforma no existe. Por favor llame al ADMINISTRADOR', 'stop')
		RETURN
	END IF
	LET mensaje = 'Desea que la FACTURA de ésta Pre-Venta se transmita a '
	CASE vg_codloc
		WHEN 1 LET mensaje = mensaje CLIPPED, ' Quito Matriz/Acero Sur'
		WHEN 3 LET mensaje = mensaje CLIPPED, ' Acero Guayaquil/Acero Sur'
		WHEN 4 LET mensaje = mensaje CLIPPED, ' Quito Matriz/Acero Guayaquil'
	END CASE
	LET mensaje = mensaje CLIPPED, ' para cruzar con una TRANSFERENCIA ?'
	LET int_flag = 0
	...
	UPDATE rept021
		SET r21_trans_fact = 'S',
		    ...
		    r21_fec_tr_fa  = fec_tr_fa
		WHERE CURRENT OF q_trans_fact
	IF STATUS <> 0 THEN
		ROLLBACK WORK
		WHENEVER ERROR STOP 
	...
	WHENEVER ERROR STOP
COMMIT WORK
LET mensaje = 'Cuando la Pre-Venta ', r_r23.r23_numprev USING "<<<<<<&",
		' sea facturada, esa Factura se transmitira a '
CASE vg_codloc
	WHEN 1 LET mensaje = mensaje CLIPPED, ' Quito Matriz/Acero Sur.'
	WHEN 3 LET mensaje = mensaje CLIPPED, ' Acero Guayaquil/Acero Sur.'
	WHEN 4 LET mensaje = mensaje CLIPPED, ' Quito Matriz/Acero Guayaquil.'
...
IF vg_gui = 1 THEN
	...
	DISPLAY vm_ind_arr TO max_row
END IF
DISPLAY r_detalle_2[i].r23_nomcli  TO r23_nomcli
DISPLAY r_detalle_2[i].r01_nombres TO r01_nombres
...
LET a = a + 1
DISPLAY '<F7>      Transmitir Factura'        AT a,2
