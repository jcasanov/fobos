...
DISPLAY ARRAY r_trn TO r_trn.*
	...
	ON KEY(F7)
		IF (NOT tiene_codigo_caja() OR rm_g05.g05_tipo <> 'UF') --AND
		   --(rm_g05.g05_grupo = 'GE' OR rm_g05.g05_grupo = 'SI' OR
		    --rm_g05.g05_grupo = 'OD')
		THEN
			LET comando = run_prog, 'repp231 ', vg_base, ' RE ',
					vg_codcia, ' ', vg_codloc, ' "',
					cod_tran, '" ', num_tran 
			RUN comando
		END IF
	...
	--#BEFORE ROW
		...
		--#END IF
		--#IF tiene_codigo_caja() AND rm_g05.g05_tipo = 'UF' THEN
...
OPEN WINDOW w_imp AT row_ini, 2 WITH num_rows2 ROWS, num_cols COLUMNS
	ATTRIBUTE(FORM LINE FIRST + 1, COMMENT LINE LAST, MENU LINE lin_menu,
		  MESSAGE LINE LAST - 1, BORDER) 
IF r_ctrn.r19_cod_tran = 'IM' THEN
	IF vg_gui = 1 THEN
...
ELSE
	IF vg_gui = 1 THEN
		OPEN FORM f_imp FROM '../forms/repf308_4'
	ELSE
		OPEN FORM f_imp FROM '../forms/repf308_4c'
	END IF
END IF
DISPLAY FORM f_imp
...
IF r_ctrn.r19_tipo_dev IS NULL THEN
	CLEAR r19_tipo_dev, r19_num_dev
END IF
LET i = 0
LET t_fob = 0
FOREACH q_det INTO r_dtrn.*
...
--OJO Para llegar hasta la liquidacion de la importacion
	...
	--#BEFORE DISPLAY 
		...
		--#CALL dialog.keysetlabel("F1","")
		--#CALL dialog.keysetlabel("CONTROL-W","")
		--#CALL dialog.keysetlabel("F6","Imprimir Recep.")
	--#AFTER DISPLAY 
		--#CONTINUE DISPLAY
END DISPLAY
	...
	COMMAND 'Caja'
		...
		DISPLAY ARRAY r_caj TO r_caj.*
			...
			ON KEY(F5)
				...
				SELECT * INTO r_r23.*
					FROM rept023
					WHERE r23_compania = r_ctrn.r19_compania
					  AND r23_localidad=r_ctrn.r19_localidad
					  AND r23_cod_tran = r_ctrn.r19_cod_tran
					  AND r23_num_tran = r_ctrn.r19_num_tran
				CALL ver_forma_pago(r_r23.r23_numprev)
			--#BEFORE ROW
				--#LET i = arr_curr()
				--#CALL muestra_contadores_fp(0, num_ant, i,
							--#num_caj, 0, num_cred)
	...
	COMMAND 'Crédito'
		...
		DISPLAY ARRAY r_cred TO r_cred.*
			...
			ON KEY(F5)
				...
				SELECT * INTO r_z20.*
					FROM cxct020
					WHERE z20_compania = r_ctrn.r19_compania
					  AND z20_localidad=r_ctrn.r19_localidad
					  AND z20_codcli   = r_ctrn.r19_codcli
					  AND z20_tipo_doc = r_ctrn.r19_cod_tran
					  AND z20_cod_tran = r_ctrn.r19_cod_tran
					  AND z20_num_tran = r_ctrn.r19_num_tran
					  AND z20_dividendo=
							r_cred[i].r26_dividendo
...
IF rm_g05.g05_tipo = 'UF' THEN
	...
	END IF
	LET tiene_caja = 0
	CALL fl_retorna_caja(vg_codcia, vg_codloc, rm_vend.r01_user_owner)
		RETURNING r_j02.*
	IF r_j02.j02_compania IS NOT NULL THEN
		LET tiene_caja = 1
	END IF
	IF rm_vend.r01_tipo <> 'G' AND NOT tiene_caja THEN
...
FUNCTION ver_forma_pago(numprev)
...
LET comando = 'cd ..', vg_separador, '..', vg_separador, 'CAJA', vg_separador,
		...
		' "CG" ', vg_codcia, ' ', vg_codloc, ' "PR" ', numprev
RUN comando
...
SELECT r29_pedido
	INTO pedido
	FROM rept029
	WHERE r29_compania  = r_r28.r28_compania
	  AND r29_localidad = r_r28.r28_localidad
	  AND r29_numliq    = r_r28.r28_numliq
LET param = ' "', pedido, '" "P"'
IF NOT fl_control_acceso_proceso_men(vg_usuario,vg_codcia, vg_modulo, prog) THEN
	RETURN
END IF
CALL fl_ejecuta_comando('REPUESTOS', vg_modulo, prog, param, 0)
