...
FOREACH q_notent INTO r_r34.*
	...
	DECLARE q_notent_2 CURSOR FOR
		SELECT UNIQUE r36_bodega_real FROM rept036
...
FOR i = 1 TO vm_num_repd
	...
	IF r_desp[i].r35_cant_ent <= 0 THEN
		CONTINUE FOR
	END IF
	SELECT NVL(SUM(r35_cant_ent), 0) INTO cant_ent_par
...
IF r_r19.r19_tipo_dev IS NOT NULL THEN
	IF r_r19.r19_tipo_dev = 'AF' THEN
		...
		CALL mostrar_registro_al_salir(0)
		LET vm_bodega_real = NULL
...
FUNCTION mostrar_registro_al_salir(flag)
...
END IF
IF num_args() = 7 THEN
	CALL retorna_entregar_en_refacturacion(rm_r34.r34_entregar_en)
	...
	CONSTRUCT BY NAME expr_sql ON r34_bodega, r34_num_ord_des, r34_estado,
		...
		ON KEY(F2)
			...
			IF INFIELD(r34_bodega) THEN
				CALL fl_ayuda_bodegas_rep(vg_codcia, vg_codloc, 'A', '2', 'R', 'S')
...
ELSE
	...
	END IF
END IF
LET query = 'SELECT rept034.*, ROWID FROM rept034 ',
		' WHERE r34_compania    = ', vg_codcia,
		'   AND r34_localidad   = ', vg_codloc,
...
FUNCTION retorna_entregar_en_refacturacion(entregar_en)
