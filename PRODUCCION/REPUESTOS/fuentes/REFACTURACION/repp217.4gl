...
IF done = 0 THEN
	ROLLBACK WORK
	CALL fl_mostrar_mensaje('Ha ocurrido un error en el ingreso del detalle de la devolución. No se realizará el proceso.','exclamation')
	EXIT PROGRAM
END IF
	...
	AFTER FIELD r19_num_dev
		IF rm_r19.r19_num_dev IS NOT NULL THEN
			CALL fl_lee_cabecera_transaccion_rep(vg_codcia,
				...
				RETURNING r_r19.*
                	IF r_r19.r19_num_tran IS  NULL THEN
		    		--CALL fgl_winmessage (vg_producto,'La factura no existe en la Compañía.','exclamation')
				CALL fl_mostrar_mensaje('La factura no existe en la Compañía.','exclamation')
			...
			IF TODAY > DATE(rm_r19.r19_fecing) AND 
				...
				CALL fl_mostrar_mensaje('Se va a generar NC, y el cliente es CONSUMIDOR FINAL, debe indicar un código de cliente válido.','exclamation')
				NEXT FIELD r19_codcli
			END IF
		ELSE 
			NEXT FIELD r19_num_dev
...
IF inserta_nc THEN
	...
	IF rm_r19.r19_codcli = rm_r00.r00_codcli_tal THEN
		ROLLBACK WORK
		CALL fl_mostrar_mensaje('Se va a generar NC, y el cliente es CONSUMIDOR FINAL, debe indicar un código de cliente válido.','stop')
		EXIT PROGRAM
	END IF
...
FOREACH q_retorno_t INTO r_r20.*
	...
	LET cant     = cant + r_detalle[i].r20_cant_dev
	INSERT INTO tmp_item_ret
		VALUES(num_tran, r_r20.r20_bodega, r_detalle[i].r20_bodega,
			r_detalle[i].r20_item, r_detalle[i].r20_cant_dev,
			r_detalle[i].r20_cant_dev)
	IF cant >= r_detalle[i].r20_cant_dev THEN
		EXIT FOREACH
	END IF
...
FOREACH q_tmp INTO bodega
	...
	IF r_r19.r19_num_tran = -1 THEN
		...
		SET LOCK MODE TO NOT WAIT
	END IF
	LET tipo = ' NE # ', r_item.nota_ent USING "<<<<<<<&"
