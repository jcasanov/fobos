...
OPTIONS
	INPUT WRAP,
	ACCEPT KEY F12
OPEN WINDOW w_219 AT 3,2 WITH 22 ROWS, 80 COLUMNS
...
IF vm_transaccion = vm_dev_fact THEN
	...
	IF rm_r19.r19_codcli IS NULL OR rm_r19.r19_codcli <> 
		...
		ROLLBACK WORK
		CALL fgl_winmessage(vg_producto, 'Código cliente en la factura no es del Taller', 'stop')
		EXIT PROGRAM
	END IF
...
IF NOT flag THEN
	CALL fgl_winmessage(vg_producto, 
		'Esta factura ya ha sido devuelta por completo.',
		'exclamation')
	LET INT_FLAG = 1
	RETURN
END IF
...
FOR i = 1 TO vm_indice
	...
	LET r_r20.r20_cant_ent   = rm_venta[i].cant_ven - rm_venta[i].cant_dev  
...
FETCH q_r11 INTO r_r11.*
	...
	UPDATE rept011 SET 
		...
		r11_stock_act = r11_stock_act + rm_venta[i].cant_dev,
     		r11_ing_dia   = rm_venta[i].cant_dev
...
FETCH q_r19 
    	UPDATE rept019 SET 
		r19_tipo_dev = rm_r19.r19_cod_tran,
		r19_num_dev  = rm_r19.r19_num_tran
	WHERE CURRENT OF q_r19
CLOSE q_r19

FOREACH q_upd INTO r_r20.*
	FOR i = 1 TO vm_indice
...
IF vm_num_rows = 0 THEN
	CALL fgl_winmessage(vg_producto,
		'No existen devoluciones para esta requisición.',
		'info')
	EXIT PROGRAM
END IF
