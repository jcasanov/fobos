------------------------------------------------------------------------------
-- Titulo           : vehp301.4gl - Consulta Estadísticas por Vendedores
-- Elaboracion      : 14-Sep-2001
-- Autor            : YEC
-- Formato Ejecucion: fglrun vehp301.4gl base_datos modulo compañía
...
OPEN FORM f_cons FROM '../forms/vehf301_1'
DISPLAY FORM f_cons
DISPLAY 'V e n d e d o r' TO tit_vend
LET vm_max_rows = 200
CALL carga_colores()
WHILE TRUE
	CALL lee_parametros()
...
END IF
LET query = 'SELECT v40_vendedor, v40_mes, SUM(v40_valor), SUM(v40_uni_venta) ',
		' FROM veht040 ',
...
LET vm_num_vend = 0
WHILE TRUE
	FETCH q_men INTO cod_vend, mes, valor, cant
	...
	IF status = NOTFOUND THEN
		...
		CALL fl_lee_vendedor_veh(vg_codcia, cod_vend)
			RETURNING r.*
		LET query = "INSERT INTO temp_acum VALUES('", cod_vend, "', '",
			     r.v01_nombres CLIPPED, "'", expr_ins CLIPPED, ')'
	...
	ELSE
		...
		EXECUTE up_temp USING valor, cant, cod_vend 
	END IF
END WHILE
LET int_flag = 0
...
FUNCTION muestra_movimientos_vendedor(vendedor, mes_ini, mes_fin, out1, out2, 
			 out3, out4, out5, out6, out7)
DEFINE vendedor		LIKE veht001.v01_vendedor
DEFINE mes_ini, mes_fin	SMALLINT
DEFINE out1, out2, out3	LIKE veht001.v01_vendedor
DEFINE out4, out5, out6	LIKE veht001.v01_vendedor
DEFINE out7		LIKE veht001.v01_vendedor
...
LET max_rows = 1000
OPEN WINDOW w_mov AT 4,5 WITH FORM "../forms/vehf301_2"
...
IF vendedor <> 0 THEN
	CALL fl_lee_vendedor_veh(vg_codcia, vendedor) RETURNING r_ven.*
	DISPLAY BY NAME vendedor
	DISPLAY r_ven.v01_nombres TO tit_ven
ELSE
	DISPLAY 'OTROS' TO tit_ven
END IF
LET int_flag = 0
...
FOREACH q_cab INTO r_cab.*
	...
	END IF
	IF vendedor = 0 THEN
		IF r_cab.v30_vendedor = out1 OR
		   r_cab.v30_vendedor = out2 OR
		   r_cab.v30_vendedor = out3 OR
		   r_cab.v30_vendedor = out4 OR
		   r_cab.v30_vendedor = out5 OR
		   r_cab.v30_vendedor = out6 OR
		   r_cab.v30_vendedor = out7 THEN
			CONTINUE FOREACH
		END IF
	ELSE	
		IF vendedor <> r_cab.v30_vendedor THEN
			CONTINUE FOREACH
		END IF
	END IF
...
DEFINE vendedor		LIKE veht001.v01_vendedor
DEFINE nombres		LIKE veht001.v01_nombres
...
DEFINE r_obj ARRAY[8] OF RECORD
	...
	etiqueta	LIKE veht001.v01_nombres,
	valor		DECIMAL(14,2),
	id_obj_arc	SMALLINT,
	id_obj_rec	SMALLINT
	END RECORD
...
ELSE
	LET titulo = 'UNIDADES VENDIDAS POR VENDEDOR DURANTE MESES SELECCIONADOS DEL ANO: ' || mes_c
END IF
--
...
IF tot_valor IS NULL THEN
	RETURN
END IF
LET query = 'SELECT te_vendedor, te_nombre, ', expr_suma CLIPPED,
		' FROM temp_acum ',
		' ORDER BY 3 DESC'
