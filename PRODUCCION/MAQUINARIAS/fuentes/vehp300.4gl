------------------------------------------------------------------------------
-- Titulo           : vehp300.4gl - Consulta Estadísticas por Bodegas
-- Elaboracion      : 12-Sep-2001
-- Autor            : YEC
-- Formato Ejecucion: fglrun vehp300.4gl base_datos modulo compañía
...
DEFINE rm_cons ARRAY[200] OF RECORD
	name_bod	LIKE veht002.v02_nombre,
...
OPEN FORM f_cons FROM '../forms/vehf300_1'
DISPLAY FORM f_cons
...
INPUT BY NAME rm_par.* WITHOUT DEFAULTS
	...
	ON KEY(F2)
		...
		IF infield(modelo) THEN
			CALL fl_ayuda_modelos_veh(vg_codcia) RETURNING mod_aux, lin_aux
	...
	AFTER FIELD modelo
		IF rm_par.modelo IS NOT NULL THEN
			CALL fl_lee_modelo_veh(vg_codcia, rm_par.modelo) RETURNING r_mod.*
			IF r_mod.v20_modelo IS NULL THEN
...
DEFINE cod_bod		LIKE veht002.v02_bodega
...
END IF
IF rm_par.modelo IS NOT NULL THEN
	LET expr2 = " v40_modelo = '", rm_par.modelo CLIPPED, "' "
END IF
LET query = 'SELECT v40_bodega, v40_mes, SUM(v40_valor), SUM(v40_uni_venta) ',
		' FROM veht040 ',
...
LET vm_num_bod = 0
WHILE TRUE
	FETCH q_men INTO cod_bod, mes, valor, cant
	...
	IF status = NOTFOUND THEN
		...
		CALL fl_lee_bodega_veh(vg_codcia, cod_bod)
			RETURNING r.*
		LET query = "INSERT INTO temp_acum VALUES('", cod_bod, "', '",
			     r.v02_nombre CLIPPED, "'", expr_ins CLIPPED, ')'
	...
	ELSE
		...
		EXECUTE up_temp USING valor, cant, cod_bod 
	END IF
END WHILE
LET int_flag = 0
...
DEFINE bod_aux		LIKE veht002.v02_nombre
DEFINE resp		CHAR(3)
...
WHILE TRUE
	...
	DISPLAY ARRAY rm_cons TO rm_cons.*
		...
		ON KEY(F6)
			LET i = arr_curr()
			CALL muestra_movimientos_bodega(rm_bod[i], rm_meses[vm_ind_ini], rm_meses[vm_ind_ini], null, null, null, null, null, null, null)
		ON KEY(F7)
			IF vm_pantallas > 1 THEN
...
FUNCTION muestra_movimientos_bodega(bodega, mes_ini, mes_fin, out1, out2, 
			 out3, out4, out5, out6, out7)
DEFINE bodega		LIKE veht002.v02_bodega
DEFINE mes_ini, mes_fin	SMALLINT
DEFINE out1, out2, out3	LIKE veht002.v02_bodega
DEFINE out4, out5, out6	LIKE veht002.v02_bodega
DEFINE out7		LIKE veht002.v02_bodega
...
DEFINE r_mode ARRAY[1000] OF CHAR(20)
...
LET max_rows = 1000
OPEN WINDOW w_mov AT 4,5 WITH FORM "../forms/vehf300_2"
...
DISPLAY BY NAME fec_ini, fec_fin, rm_par.moneda,
	...
	rm_par.tit_lin, rm_par.modelo
IF bodega <> '0' THEN
	CALL fl_lee_bodega_veh(vg_codcia, bodega) RETURNING r_bod.*
	DISPLAY BY NAME bodega
	DISPLAY r_bod.v02_nombre TO tit_bod
ELSE
	DISPLAY 'OTROS' TO tit_bod
END IF
LET int_flag = 0
...
FOREACH q_cab INTO r_cab.*
	...
	END IF
	IF bodega = '0' THEN
		IF r_cab.v30_bodega_ori = out1 OR
		   r_cab.v30_bodega_ori = out2 OR
		   r_cab.v30_bodega_ori = out3 OR
		   r_cab.v30_bodega_ori = out4 OR
		   r_cab.v30_bodega_ori = out5 OR
		   r_cab.v30_bodega_ori = out6 OR
		   r_cab.v30_bodega_ori = out7 THEN
			CONTINUE FOREACH
		END IF
	ELSE	
		IF r_cab.v30_bodega_ori <> bodega THEN
			CONTINUE FOREACH
		END IF
	END IF
	...
	FOREACH q_det INTO r_det.*
		...
		SELECT v20_linea INTO linea FROM veht020
			WHERE v20_compania = vg_codcia AND 
			      v20_modelo = r_veh.v22_modelo
		IF rm_par.linea IS NOT NULL AND linea <> rm_par.linea THEN
			CONTINUE FOREACH
		END IF
		IF rm_par.modelo IS NOT NULL AND 
			rm_par.modelo <> r_veh.v22_modelo THEN
			CONTINUE FOREACH
		END IF
		...
		LET r_mode[num_rows]		= r_veh.v22_modelo
		IF num_rows = max_rows THEN
			EXIT FOREACH
		END IF
	END FOREACH
...
WHILE TRUE
	...
	DISPLAY ARRAY r_mov TO r_mov.*
		...
		BEFORE ROW
			...
			CALL fl_lee_cod_vehiculo_veh(vg_codcia, vg_codloc, r_det.v31_codigo_veh)
				RETURNING r_veh.*
			MESSAGE i, ' de ', num_rows, '       ', r_mode[i]
		ON KEY(INTERRUPT)
			EXIT DISPLAY
		ON KEY(F5)
			LET i = arr_curr()
...
DEFINE bodega		LIKE veht002.v02_bodega
DEFINE nombre		LIKE veht002.v02_nombre
...
DEFINE r_obj ARRAY[8] OF RECORD
	bodega		LIKE veht002.v02_bodega,
	etiqueta	LIKE veht002.v02_nombre,
	valor		DECIMAL(14,2),
	id_obj_arc	SMALLINT,
	id_obj_rec	SMALLINT
	END RECORD
...
IF vm_cant_val = 'V' THEN
	LET titulo = 'VALORES VENDIDOS POR BODEGA DURANTE MESES SELECCIONADOS DEL ANO: ' || mes_c
ELSE
	LET titulo = 'UNIDADES VENDIDAS POR BODEGA DURANTE MESES SELECCIONADOS DEL ANO: ' || mes_c
END IF
--
...
LET max_elementos = vm_num_bod 
IF vm_num_bod > limite THEN
...
IF tot_valor IS NULL THEN
	RETURN
END IF
LET query = 'SELECT te_bodega, te_nombre, ', expr_suma CLIPPED,
		' FROM temp_acum ',
		' ORDER BY 3 DESC'
...
LET i = drawtext(030,10,'Haga click sobre una bodega para ver detalles')
...
END FOR
FOREACH q_gr1 INTO bodega, nombre, valor
	LET r_obj[indice].valor  = valor
	LET r_obj[indice].bodega = bodega
	IF indice = max_elementos THEN
		...
		ELSE
			LET r_obj[indice].etiqueta = nombre
		END IF
	ELSE
...
INPUT BY NAME tecla
	...
	ON KEY(F31,F32,F33,F34,F35,F36,F37,F38)
		...
		IF i = 8 THEN
			CALL muestra_movimientos_bodega('0',
				    rm_meses[1], rm_meses[vm_num_meses],
		                    r_obj[1].bodega, r_obj[2].bodega, 
				    r_obj[3].bodega, r_obj[4].bodega, 
				    r_obj[5].bodega, r_obj[6].bodega,
		     		    r_obj[7].bodega)
		ELSE
			CALL muestra_movimientos_bodega(r_obj[i].bodega,
				    rm_meses[1], rm_meses[vm_num_meses],
...
CREATE TEMP TABLE temp_bar
	(te_bodega	CHAR(2),
	 te_nombre	VARCHAR(20),
	 te_valor	DECIMAL(14,2),
         te_indice	SMALLINT)
LET i = 1
LET val_x = 0
LET tot_valor = 0
FOREACH q_gr2 INTO bodega, nombre, valor
	LET tot_valor = tot_valor + valor
	IF i <= limite - 1 THEN
		INSERT INTO temp_bar VALUES (bodega, nombre, valor, i)
		LET i = i + 1
	ELSE
...
FOREACH q_bar INTO bodega, nombre, valor
	...
	LET r_obj[indice + 1].etiqueta = nombre
        ...
        LET i = DrawText(key_y + 40, start_key_text, nombre)
