...
DECLARE q_drest CURSOR FOR 
	SELECT DATE(r20_fecing), r20_linea, r20_rotacion,
...
FOREACH q_drest INTO r.r60_fecha, r.r60_linea, r.r60_rotacion, 
		     r.r60_precio, r.r60_costo
	IF rt.g21_tipo = 'I' THEN
		LET r.r60_bodega = rc.r19_bodega_dest
		LET r.r60_precio = r.r60_precio * -1
		LET r.r60_costo  = r.r60_costo  * -1
	ELSE
		LET r.r60_bodega = rc.r19_bodega_ori
	END IF
	LET i = i + 1
	WHENEVER ERROR CONTINUE
...
COMMIT WORK
CALL fgl_winmessage(vg_producto, 'Mayorización terminó correctamente', 'exclamation')
RETURN 1
...
IF r_b00.b00_anopro < ano AND r.b10_tipo_cta <> 'R' THEN
	...
	SELECT b11_compania, b11_cuenta, b11_moneda, 2000,
		...
		GROUP BY 1,2,3,4
display r_b00.b00_anopro, ' - ', ano_ant
...
SELECT SUM(r11_stock_act) INTO stock FROM rept011 
	WHERE r11_compania = cod_cia AND r11_item = item
...
IF (LENGTH(cedruc) <> lim) AND (LENGTH(cedruc) <> 13) THEN
	CALL fgl_winmessage(vg_producto, 'El número de digitos de cédula/ruc es incorrecto.', 'exclamation')
	RETURN 0
END IF
...
FUNCTION fl_lee_modelo_maq(cod_cia, linea, modelo)
DEFINE cod_cia		LIKE maqt010.m10_compania
DEFINE linea		LIKE maqt010.m10_linea
...
FUNCTION fl_lee_canton(provincia, canton)
DEFINE provincia	LIKE maqt002.m02_provincia
DEFINE canton		LIKE maqt002.m02_canton
DEFINE r		RECORD LIKE maqt002.*

INITIALIZE r.* TO NULL
SELECT * INTO r.* FROM maqt002 
 WHERE m02_provincia = provincia 
   AND m02_canton    = canton
