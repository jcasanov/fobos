...
DEFINE rm_r29		RECORD LIKE rept029.*
...
IF rm_crep.r19_compania IS NULL THEN
	CALL fgl_winmessage(vg_producto, 'No existe transacción en módulo Repuestos: ' || cod_tran || ' ' || num_tran, 'stop')
	RETURN
END IF
...
CASE rm_crep.r19_cod_tran 
	...
	WHEN 'IM'
		...
		LET vm_indice = vm_indice + 1
	WHEN 'IX'
		CALL fl_contabiliza_recepcion_compra(45)
		LET vm_indice = vm_indice + 1
	WHEN 'IC'
		CALL fl_contabiliza_recepcion_compra(45)
		LET vm_indice = vm_indice + 1
	WHEN 'RQ'
		CALL fl_contabiliza_requisiciones_taller(42)
		LET vm_indice = vm_indice + 1
	WHEN 'DR'
		CALL fl_contabiliza_requisiciones_taller(42)
		LET vm_indice = vm_indice + 1
	...
	WHEN 'CL'
		...
		LET vm_indice = vm_indice + 1
	WHEN 'DC'
		CALL fl_contabiliza_dev_compras_locales(20)
		LET vm_indice = vm_indice + 1
	WHEN 'TR'
...
IF rm_auxv.b40_compania IS NULL THEN
        CALL fgl_winmessage(vg_producto, 'No hay configuración de auxiliares contables para bodega/grupo línea: ' || rm_crep.r19_bodega_dest || '/' || r_lr.r03_grupo_linea || ' en transacción: ' || r_drep.r20_cod_tran || ' ' || r_drep.r20_num_tran, 'stop')
        RETURN 0
END IF
LET cuenta = rm_auxv.b40_inventario
CALL fl_lee_auxiliares_ventas(r_drep.r20_compania, r1_r02.r02_localidad,
        vm_modulo, rm_crep.r19_bodega_ori, r_lr.r03_grupo_linea)
        RETURNING rm_auxv.*
IF rm_auxv.b40_compania IS NULL THEN
        CALL fgl_winmessage(vg_producto, 'No hay configuración de auxiliares contables para bodega/grupo línea: ' || rm_crep.r19_bodega_ori || '/' || r_lr.r03_grupo_linea || ' en transacción: ' || r_drep.r20_cod_tran || ' ' || r_drep.r20_num_tran, 'stop')
        RETURN 0
END IF
IF cuenta = rm_auxv.b40_inventario THEN
	RETURN 0
END IF

CASE vg_codloc
	-- Se esta contabilizando el envio por TR
	WHEN r1_r02.r02_localidad
		CALL fl_genera_detalle_comprobante(vm_tipo_comp, subtipo, rm_auxv.b40_transito, 'D',
        ...
        tot_costo, glosa, rm_crep.r19_cod_tran, rm_crep.r19_num_tran)
	-- Se esta contabilizando la recepcion por TR
	WHEN r2_r02.r02_localidad
		CALL fl_genera_detalle_comprobante(vm_tipo_comp, subtipo, cuenta, 'D',
        tot_costo, glosa, rm_crep.r19_cod_tran, rm_crep.r19_num_tran)
		CALL fl_genera_detalle_comprobante(vm_tipo_comp, subtipo, rm_auxv.b40_transito,'H',
        tot_costo, glosa, rm_crep.r19_cod_tran, rm_crep.r19_num_tran)
END CASE
RETURN 1
...
IF status = NOTFOUND THEN
	-- Esto es al contado, aun asi todo se genera como un documento
	-- a credito con fecha de vcto el mismo dia de fecha emision
	LET r_fpc.r25_valor_ant  = 0	
	LET r_fpc.r25_valor_cred = rm_crep.r19_tot_neto	
ELSE
...
IF status = NOTFOUND THEN
	CALL fgl_winmessage(vg_producto, 'Factura no tiene detalle: ' || rm_crep.r19_cod_tran || ' ' || rm_crep.r19_num_tran, 'stop')
	EXIT PROGRAM
END IF
CALL fl_lee_linea_rep(r_drep.r20_compania, r_drep.r20_linea)
	RETURNING r_lr.*
IF r_lr.r03_compania IS NULL THEN
	CALL fgl_winmessage(vg_producto, 'No existe línea de venta: ' || r_drep.r20_linea || ' en transacción: ' || r_drep.r20_cod_tran || ' ' || r_drep.r20_num_tran, 'stop')
	EXIT PROGRAM
END IF
CALL fl_lee_auxiliares_caja(rm_crep.r19_compania, rm_crep.r19_localidad,
			    vm_modulo, r_lr.r03_grupo_linea, rm_crep.r19_codcli)
	RETURNING rm_auxc.*
...
IF status = NOTFOUND THEN
	CALL fgl_winmessage(vg_producto, 'No existe registro de caja en transacción: ' || r_drep.r20_cod_tran || ' ' || r_drep.r20_num_tran, 'stop')
	EXIT PROGRAM
END IF
...
{OjO 28/Abr/2009
 Chequear: Esto es esta definitivamente mal
...
IF (dif_abs * 100) / rm_crep.r19_tot_neto > 2 THEN  -- Diferencia > 2%
	CALL fgl_winmessage(vg_producto, 'No cuadran valores de factura: ' || r_drep.r20_cod_tran || ' ' || r_drep.r20_num_tran, 'stop')
	EXIT PROGRAM
END IF
LET rm_crep.r19_tot_neto = rm_crep.r19_tot_neto - dif
{*
 * Esto deberia ser un foreach de la rept100 
 *}
...
SELECT r19_bodega_ori, r20_linea, SUM(r20_cant_ent * r20_costo)      
  FROM rept019, rept020
 WHERE r19_compania   = rm_crep.r19_compania  
   AND r19_localidad  = rm_crep.r19_localidad 
   AND r19_cod_tran   = rm_crep.r19_cod_tran 
   AND r19_num_tran   = rm_crep.r19_num_tran
   AND r20_compania   = r19_compania
   AND r20_localidad  = r19_localidad
   AND r20_cod_tran   = r19_cod_tran
   AND r20_num_tran   = r19_num_tran
 GROUP BY r19_bodega_ori, r20_linea

FOREACH q_bod INTO bodega, linea, costo 
	CALL fl_retorna_precision_valor(rg_gen.g00_moneda_base, costo)
		RETURNING costo
	CALL fl_lee_linea_rep(rm_crep.r19_compania, linea)
		RETURNING r_lr.*
	CALL fl_lee_auxiliares_ventas(rm_crep.r19_compania, 
		rm_crep.r19_localidad, vm_modulo, bodega, r_lr.r03_grupo_linea)
		RETURNING rm_auxv.*
	IF rm_auxv.b40_compania IS NULL THEN
		CALL fgl_winmessage(vg_producto, 'No hay configuración de auxiliares contables para bodega/grupo línea: ' || bodega || '/' || r_lr.r03_grupo_linea || ' en transacción: ' || rm_crep.r19_cod_tran || ' ' || rm_crep.r19_num_tran, 'stop')
		EXIT PROGRAM
	END IF
	LET glosa = rm_crep.r19_nomcli[1,25], ' ', rm_crep.r19_cod_tran, '-',
		    rm_crep.r19_num_tran USING '<<<<<<<<<<<<<<<'
	...
	END IF
	CALL fl_genera_detalle_comprobante(vm_tipo_comp, subtipo, 
			rm_auxv.b40_costo_venta, tipo_mov_1,
        		costo, glosa, rm_crep.r19_cod_tran, 
			rm_crep.r19_num_tran)
	CALL fl_genera_detalle_comprobante(vm_tipo_comp, subtipo, 
			rm_auxv.b40_inventario, tipo_mov_2, costo, glosa, 
			rm_crep.r19_cod_tran, rm_crep.r19_num_tran)
END FOREACH
...
DEFINE auxcon		LIKE rept016.r16_aux_cont
DEFINE costo_recepcion	LIKE ctbt013.b13_valor_base
DEFINE costo			LIKE ctbt013.b13_valor_base
...
CALL fl_lee_linea_rep(r_drep.r20_compania, r_drep.r20_linea)
	RETURNING r_lr.*
--OJOTE AQUI SE DEBE LEER EL AUXILIAR DE LA CABECERA DEL PEDIDO
...
END IF
      ...
      DECLARE q_pedido CURSOR FOR
              ...
              OPEN q_pedido
              FETCH q_pedido INTO rm_r29.*
              IF STATUS <> NOTFOUND THEN
                       LET pedido = rm_r29.r29_pedido
               END IF
      CLOSE q_pedido
      FREE  q_pedido
      INITIALIZE rm_r16.* TO NULL
      DECLARE q_cabped CURSOR FOR
              SELECT * FROM rept016
               WHERE r16_compania = vg_codcia
                 AND r16_localidad= vg_codloc
                 AND r16_pedido   = rm_r29.r29_pedido
              OPEN q_cabped
              FETCH q_cabped INTO rm_r16.*
              IF STATUS <> NOTFOUND THEN
                       LET auxcon =  rm_r16.r16_aux_cont
	      ELSE
        		CALL fgl_winmessage(vg_producto, 'No hay configuración de auxiliares con tables para bodega/grupo línea: ' || rm_crep.r19_bodega_ori || '/' || r_lr.r03_grupo_linea || ' en transacción: ' || r_drep.r20_cod_tran || ' ' || r_drep.r20_num_tran, 'stop')
        		EXIT PROGRAM
              END IF
      	CLOSE q_cabped
      	FREE  q_cabped
-- HASTA AQUI
	...
	-- Costo recepcion, lo que debe salir de la cuenta puente
DECLARE q_b13_imp CURSOR FOR 
	SELECT r40_compania, r40_tipo_comp, r40_num_comp
	  FROM rept117, rept040
	 WHERE r117_compania   = rm_crep.r19_compania
	   AND r117_localidad  = rm_crep.r19_localidad
	   AND r117_cod_tran   = 'IX'
	   AND r117_numliq     = rm_crep.r19_numliq
	   AND r40_compania    = r117_compania
	   AND r40_localidad   = r117_localidad
	   AND r40_cod_tran    = r117_cod_tran 
	   AND r40_num_tran    = r117_num_tran 
	 GROUP BY r40_compania, r40_tipo_comp, r40_num_comp

LET costo_recepcion = 0
INITIALIZE r_r40.* TO NULL
FOREACH q_b13_imp INTO r_r40.r40_compania, r_r40.r40_tipo_comp, 
					   r_r40.r40_num_comp
	SELECT SUM(b13_valor_base) 
	  INTO costo
	  FROM ctbt013
	 WHERE b13_compania    = r_r40.r40_compania 
	   AND b13_tipo_comp   = r_r40.r40_tipo_comp
	   AND b13_num_comp    = r_r40.r40_num_comp
	   AND b13_cuenta      = rm_auxv.b40_transito

	IF costo IS NOT NULL THEN
		LET costo = costo * (-1)
		LET costo_recepcion = costo_recepcion + costo
	END IF
END FOREACH

-- El valor en la cuenta puente siempre es negativo
{
IF costo_recepcion IS NULL THEN
	LET costo_recepcion = 0
END IF
LET costo_recepcion = costo_recepcion * (-1)
}
LET glosa = rm_crep.r19_cod_tran, ' - ', rm_crep.r19_num_tran
	    USING '<<<<<<<<<<<<<<<'
CALL fl_genera_detalle_comprobante(vm_tipo_comp, subtipo, rm_auxv.b40_transito, 'D', costo_recepcion, glosa, rm_crep.r19_cod_tran, rm_crep.r19_num_tran)
CALL fl_genera_detalle_comprobante(vm_tipo_comp, subtipo, rm_auxv.b40_inventario, 'D', tot_costo - costo_recepcion, glosa, rm_crep.r19_cod_tran, rm_crep.r19_num_tran)
CALL fl_genera_detalle_comprobante(vm_tipo_comp, subtipo, rm_r16.r16_aux_cont,
...
IF r_fact.r19_compania IS NULL THEN
	CALL fgl_winmessage(vg_producto, 'No existe factura anulada : ' || rm_crep.r19_cod_tran || ' ' || rm_crep.r19_num_tran, 'stop')
	RETURN
END IF
...
FOREACH cu_repcont INTO r_r40.*
	...
	IF r_b12.b12_compania IS NULL THEN
		CALL fgl_winmessage(vg_producto, 'No existe en ctbt012 comprobante de la rept040.', 'STOP')
		EXIT PROGRAM
	END IF
...
CALL fl_lee_cabecera_transaccion_rep(rm_crep.r19_compania,rm_crep.r19_localidad,
	...
	RETURNING r_fact.*
IF r_fact.r19_compania IS NULL THEN
	CALL fgl_winmessage(vg_producto, 'No existe factura devuelta : ' || rm_crep.r19_cod_tran || ' ' || rm_crep.r19_num_tran, 'stop')
	RETURN
END IF
...
FETCH q_drep INTO r_drep.*
IF status = NOTFOUND THEN
	CALL fgl_winmessage(vg_producto, 'Dev. Fact. no tiene detalle: ' || rm_crep.r19_cod_tran || ' ' || rm_crep.r19_num_tran, 'stop')
	EXIT PROGRAM
END IF
CALL fl_lee_linea_rep(r_drep.r20_compania, r_drep.r20_linea)
	RETURNING r_lr.*
...
IF r_t23.t23_compania IS NULL THEN
	CALL fgl_winmessage(vg_producto, 'No existe O.T.: ' || 
...
IF r_b43.b43_compania IS NULL THEN
	CALL fgl_winmessage(vg_producto, 'No hay configuración de auxiliares contables en Taller para grupo-línea: ' || r_t01.t01_grupo_linea || 
...
IF tot_costo < 0 OR rm_crep.r19_cod_tran = 'A-' THEN
	...
	END IF
END IF
CALL fl_genera_detalle_comprobante(vm_tipo_comp, subtipo, rm_auxv.b40_inventario, signo1,
        tot_costo, glosa, rm_crep.r19_cod_tran, rm_crep.r19_num_tran)
CALL fl_genera_detalle_comprobante(vm_tipo_comp, subtipo, rm_auxv.b40_ajustes,
        signo2, tot_costo, glosa, rm_crep.r19_cod_tran, rm_crep.r19_num_tran)

END FUNCTION



FUNCTION fl_contabiliza_recepcion_compra(subtipo)
DEFINE subtipo		LIKE ctbt012.b12_subtipo
DEFINE tot_costo	DECIMAL(14,2)
DEFINE glosa       	LIKE ctbt013.b13_glosa
DEFINE r_drep		RECORD LIKE rept020.*
...
CASE rm_crep.r19_cod_tran 
	WHEN 'IC'
		...
		IF rm_c10.c10_compania IS NULL THEN 
			CALL fgl_winmessage(vg_producto, 'No existe O/C: ' || rm_crep.r19_cod_tran || ' ' || rm_crep.r19_num_tran, 'stop')
			EXIT PROGRAM
		END IF
		INITIALIZE r_c13.* TO NULL
		DECLARE q_clok1 CURSOR FOR
			SELECT * FROM ordt013
				WHERE c13_compania  = rm_crep.r19_compania   AND 
				      c13_localidad = rm_crep.r19_localidad  AND 
				      c13_numero_oc = rm_crep.r19_oc_interna 
				ORDER BY c13_num_recep DESC
		OPEN q_clok1 
		FETCH q_clok1 INTO r_c13.*
		IF status = NOTFOUND THEN
			CALL fgl_winmessage(vg_producto, 'No hay recepción en ordt013: ' || rm_crep.r19_cod_tran || ' ' || rm_crep.r19_num_tran, 'stop')
			EXIT PROGRAM
		END IF
		CLOSE q_clok1
		FREE  q_clok1
	WHEN 'IX'
		...
		DECLARE q_pedidos CURSOR FOR
			SELECT * FROM rept117
			 WHERE r117_compania  = rm_crep.r19_compania
			   AND r117_localidad = rm_crep.r19_localidad
			   AND r117_cod_tran  = rm_crep.r19_cod_tran 
			   AND r117_num_tran  = rm_crep.r19_num_tran 
		OPEN  q_pedidos
		FETCH q_pedidos INTO r_r117.*
		IF status = NOTFOUND THEN
			CALL fgl_winmessage(vg_producto, 'No hay recepción en rept117: ' || rm_crep.r19_cod_tran || ' ' || rm_crep.r19_num_tran, 'stop')
			EXIT PROGRAM
		END IF
		CLOSE q_pedidos 
		FREE  q_pedidos
...
LET rm_crep.r19_tot_costo = rm_crep.r19_tot_costo * rm_crep.r19_paridad
CALL fl_retorna_precision_valor(rg_gen.g00_moneda_base, rm_crep.r19_tot_costo)
	RETURNING rm_crep.r19_tot_costo

OPEN q_drep
FETCH q_drep INTO r_drep.*
IF status = NOTFOUND THEN
	CALL fgl_winmessage(vg_producto, 'Recepcion de compra no tiene detalle: ' || rm_crep.r19_cod_tran || ' ' || rm_crep.r19_num_tran, 'stop')
	EXIT PROGRAM
END IF
CLOSE q_drep
LET tot_costo = 0
FOREACH q_drep INTO r_drep.*
	LET tot_costo = tot_costo + (r_drep.r20_cant_ven * r_drep.r20_costo)
END FOREACH
display tot_costo, rm_crep.r19_tot_costo
CALL fl_retorna_precision_valor(rg_gen.g00_moneda_base, tot_costo)
	RETURNING tot_costo
CALL fl_lee_linea_rep(r_drep.r20_compania, r_drep.r20_linea)
	RETURNING r_lr.*
...
CALL fl_genera_detalle_comprobante(vm_tipo_comp, subtipo, 
	rm_auxv.b40_inventario, 'D', rm_crep.r19_tot_costo, glosa, 
	rm_crep.r19_cod_tran, rm_crep.r19_num_tran)
CALL fl_genera_detalle_comprobante(vm_tipo_comp, subtipo, 
	rm_auxv.b40_transito, 'H', rm_crep.r19_tot_costo, glosa, 
	rm_crep.r19_cod_tran, rm_crep.r19_num_tran)
...
FOREACH qu_drt INTO r_p28.*
	CALL fl_lee_tipo_retencion(rm_crep.r19_compania, r_p28.p28_codigo_sri, 
							   r_p28.p28_tipo_ret, r_p28.p28_porcentaje)
	RETURNING r_c02.*
	IF r_c02.c02_compania IS NULL THEN
		CALL fgl_winmessage(vg_producto, 'No existe tipo retención en ordt002: ' || rm_crep.r19_cod_tran || ' ' || rm_crep.r19_num_tran, 'stop')
		EXIT PROGRAM
	END IF
...
CALL fl_retorna_precision_valor(rg_gen.g00_moneda_base, rm_crep.r19_tot_neto)
	RETURNING rm_crep.r19_tot_neto
LET rm_crep.r19_tot_bruto = rm_crep.r19_tot_bruto - rm_crep.r19_tot_dscto
LET val_iva = rm_crep.r19_tot_neto - rm_crep.r19_tot_bruto
...
FETCH q_drep INTO r_drep.*
IF status = NOTFOUND THEN
	CALL fgl_winmessage(vg_producto, 'Compra local no tiene detalle: ' || rm_crep.r19_cod_tran || ' ' || rm_crep.r19_num_tran, 'stop')
	EXIT PROGRAM
END IF
CALL fl_lee_linea_rep(r_drep.r20_compania, r_drep.r20_linea)
	RETURNING r_lr.*
...
IF r_p02.p02_compania IS NULL THEN
	CALL fgl_winmessage(vg_producto, 'No existe proveedor: ' || rm_crep.r19_codcli || ' en transacción: ' || r_drep.r20_cod_tran || ' ' || r_drep.r20_num_tran, 'stop')
	EXIT PROGRAM
END IF

-- Costo recepcion, lo que debe salir de la cuenta puente
SELECT SUM(b13_valor_base) 
  INTO costo_recepcion
  FROM rept019, rept040, ctbt013
 WHERE r19_compania   = rm_crep.r19_compania
   AND r19_localidad  = rm_crep.r19_localidad
   AND r19_cod_tran   = 'IC'
   AND r19_oc_interna = rm_crep.r19_oc_interna
...
CALL fl_genera_detalle_comprobante(vm_tipo_comp, subtipo, rm_auxv.b40_transito, 'D',
        costo_recepcion, glosa, rm_crep.r19_cod_tran, rm_crep.r19_num_tran)

-- Hay una diferencia entre el costo de lo recepcionado y el costo de ingreso?
-- va al inventario
CALL fl_genera_detalle_comprobante(vm_tipo_comp, subtipo, rm_auxv.b40_inventario, 'D',
        rm_crep.r19_tot_bruto - costo_recepcion, glosa, rm_crep.r19_cod_tran, rm_crep.r19_num_tran)
...
IF r_comp.r19_compania IS NULL THEN
	CALL fgl_winmessage(vg_producto, 'No existe compra local devuelta : ' || rm_crep.r19_cod_tran || ' ' || rm_crep.r19_num_tran, 'stop')
	RETURN
END IF
	...
	FOREACH q_dmast INTO tipo_comp, num_comp, subtipo, cod_tran, num_tran, 
		...
		CASE rm_crep.r19_cod_tran
			...
			WHEN 'IC'
					...
					LET r_dcomp.b13_numero_oc = rm_crep.r19_oc_interna
			WHEN 'CL'
    				LET r_dcomp.b13_codprov = rm_c10.c10_codprov
					LET r_dcomp.b13_numero_oc = rm_crep.r19_oc_interna
			WHEN 'DC'
...
FUNCTION fl_lee_auxiliares_caja(cod_cia, cod_loc, modulo, grupo_linea, cliente)
DEFINE cod_cia		LIKE ctbt040.b40_compania
DEFINE cod_loc		LIKE ctbt040.b40_localidad
DEFINE modulo		LIKE ctbt040.b40_modulo
DEFINE grupo_linea	LIKE gent020.g20_grupo_linea
DEFINE cliente		LIKE ctbt100.b100_codcli
...
SELECT * INTO r_b100.* FROM ctbt100 
	WHERE b100_compania    = cod_cia AND 
	      b100_localidad   = cod_loc AND 
	      b100_modulo      = modulo  AND
	      b100_grupo_linea = grupo_linea AND
	      b100_codcli      = cliente    

IF r_b100.b100_cxc_mb IS NOT NULL THEN
	LET r.b41_cxc_mb = r_b100.b100_cxc_mb
END IF

IF r_b100.b100_cxc_me IS NOT NULL THEN
	LET r.b41_cxc_me = r_b100.b100_cxc_me
END IF

IF r_b100.b100_ant_mb IS NOT NULL THEN
	LET r.b41_ant_mb = r_b100.b100_ant_mb
END IF

IF r_b100.b100_ant_me IS NOT NULL THEN
	LET r.b41_ant_me = r_b100.b100_ant_me
END IF
