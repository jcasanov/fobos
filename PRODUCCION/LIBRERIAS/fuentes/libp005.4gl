...
IF rm_orden.t23_compania IS NULL THEN
	CALL fgl_winmessage(vg_producto, 'No existe orden: ' || orden, 'stop')
	RETURN
END IF
IF rm_orden.t23_estado <> 'F' AND rm_orden.t23_estado <> 'D' THEN
	CALL fgl_winmessage(vg_producto, 'Orden no está facturada/devuelta', 'stop')
	RETURN
END IF
IF DATE(rm_orden.t23_fec_factura) <= rm_ctb.b00_fecha_cm THEN
	CALL fgl_winmessage(vg_producto, 'La fecha de la orden corresponde a un mes contable ya cerrado: ' || orden, 'stop')
	RETURN
END IF
...
IF status = NOTFOUND THEN
	CALL fgl_winmessage(vg_producto, 'No existe registro de caja de factura: ' || rm_orden.t23_num_factura, 'stop')
	EXIT PROGRAM
END IF
...
CALL fl_lee_auxiliares_caja(rm_orden.t23_compania, rm_orden.t23_localidad,
			    vm_modulo, r_t01.t01_grupo_linea, rm_orden.t23_cod_cliente)
	RETURNING rm_auxc.*
...
IF rm_auxv.b43_compania IS NULL THEN
	CALL fgl_winmessage(vg_producto, 'No hay configuración de auxiliares contables taller para grupo línea: ' || r_t01.t01_grupo_linea || ' en factura: ' || rm_orden.t23_num_factura, 'stop')
	EXIT PROGRAM
END IF
{OjO esto ya no va
...
IF tot_cap IS NULL THEN
	-- Igual, esto es contado y debe crearse como una cta x cobrar de
	-- ejecucion inmediata
	LET tot_cap = rm_orden.t23_tot_neto
	LET tot_int = 0
END IF
LET tot_comp = tot_efe + tot_tar + tot_ret + r_t25.t25_valor_ant + tot_cap
IF rm_orden.t23_tot_neto <> tot_comp THEN
	CALL fgl_winmessage(vg_producto, 'No cuadran valores de factura: ' || rm_orden.t23_num_factura, 'stop')
	EXIT PROGRAM
END IF
...
FOREACH cu_chipi INTO r_t50.*
	...
	IF r_b12.b12_compania IS NULL THEN
		CALL fgl_winmessage(vg_producto, 'No existe en ctbt012 comprobante de la talt050.', 'STOP')
		EXIT PROGRAM
	END IF
...
IF cuenta IS NULL THEN
	CALL fgl_winmessage(vg_producto, 'Cuenta nula. Revisar taller.', 'stop')
	EXIT PROGRAM
END IF		
IF tipo_mov = 'H' THEN
	LET valor = valor * -1
