...
FUNCTION fl_ayuda_bodegas_rep(cod_cia, cod_loc, indicador) ## indicador T ó F 
...
LET expr_bodf = ' 1 = 1 '
IF indicador ='F' THEN
	LET expr_bodf = ' r02_factura = "S" '
END IF
LET expr_loc = ' 1 = 1 '
IF cod_loc IS NOT NULL THEN
	LET expr_loc = ' r02_localidad = ', cod_loc
END IF
LET query = 'SELECT r02_codigo, r02_nombre FROM rept002 ',
			' WHERE r02_compania = ', cod_cia,
			'   AND r02_estado = "A" ',
       		'   AND ', expr_bodf CLIPPED, 
       		'   AND ', expr_loc CLIPPED, 
       		' ORDER BY 1 '

PREPARE cons_bod FROM query
DECLARE q_bode CURSOR FOR cons_bod
...
FUNCTION fl_ayuda_comisionistas(cod_cia)
DEFINE rh_comi ARRAY[100] OF RECORD
...
DEFINE filas_pant       SMALLINT        ## No. elementos de cada pantalla
DEFINE j	SMALLINT
DEFINE cod_cia 		LIKE cmst002.c02_compania
                                                                                
LET filas_max  = 100
OPEN WINDOW wh_comi AT 06,43
 	WITH FORM '../../../PRODUCCION/LIBRERIAS/forms/ayuf128'
        ATTRIBUTE(FORM LINE FIRST, COMMENT LINE LAST -1, MESSAGE LINE LAST,
                   BORDER)
LET filas_pant = fgl_scr_size('rh_comi')
LET int_flag = 0
MESSAGE 'Seleccionando datos..' 
DECLARE q_comi CURSOR FOR
        SELECT c02_codigo, c02_nombres FROM cmst002
		WHERE c02_compania = cod_cia
		  AND c02_estado = 'A'
        ORDER BY 2
LET i = 1
FOREACH q_comi INTO rh_comi[i].*
        LET i = i + 1
        IF i > filas_max THEN
                EXIT FOREACH
                                                                                        END IF
...
IF i = 0 THEN
        CALL fl_mensaje_consulta_sin_registros()
        CLOSE WINDOW wh_comi
        INITIALIZE rh_comi[1].* TO NULL
        RETURN rh_comi[1].c02_codigo, rh_comi[1].c02_nombres
END IF
CALL set_count(i)
LET int_flag = 0
DISPLAY ARRAY rh_comi TO rh_comi.*
        ON KEY(RETURN)
                EXIT DISPLAY
	BEFORE ROW
		LET j = arr_curr()
...
END IF
LET  i = arr_curr()
RETURN rh_comi[i].c02_codigo, rh_comi[i].c02_nombres
...
DECLARE q_hoc CURSOR FOR
	SELECT c01_tipo_orden, c01_nombre FROM ordt001 
	 WHERE c01_estado = 'A' ORDER BY 1
LET i = 1
...
---------
{
        ...
        LET query = "SELECT r10_codigo, r10_nombre, r10_linea, r10_precio_mb, r11_bodega, r11_stock_act, 0  FROM rept010, rept011 ",
                                " WHERE r10_compania = ", cod_cia, " AND ",
                                " r10_compania = r11_compania",    " AND ",
                                " r10_codigo = r11_item",          " AND ",
                                " r11_bodega = '", bodega,"'",     " AND ",
				...
				" WHERE r03_compania = ", cod_cia, " AND ",
                                " r03_grupo_linea = '", grupo,"')", " AND ",
                                 criterio CLIPPED
---------
	CALL fl_lee_bodega_rep(cod_cia, bodega) RETURNING r_r02.*
	PREPARE reppre FROM query
	DECLARE q_reppre CURSOR FOR reppre
	LET i = 1
	FOREACH q_reppre INTO rh_reppre[i].*
		LET rh_reppre[i].stock_disp = fl_lee_stock_disponible_rep(cod_cia, 
											r_r02.r02_localidad,
											rh_reppre[i].r10_codigo,
											'R')
		IF rh_reppre[i].stock_disp < 0 THEN
			LET rh_reppre[i].stock_disp = 0 
		END IF
		LET i = i + 1
		IF i > filas_max THEN
			EXIT FOREACH
...
FUNCTION fl_ayuda_doc_deudores_cob(cod_cia, cod_loc, cod_area, cod_cli, cod_doc, con_saldo)
...
END IF
IF con_saldo THEN
	LET expr_saldo = " z20_saldo_cap + z20_saldo_int > 0 "
ELSE
	LET expr_saldo = " 1 = 1 "
END IF
...
LET query =   "	SELECT 	z01_nomcli, z20_tipo_doc, 
		...
		" AND ", expr_cliente CLIPPED, 
		" AND ", expr_tipo CLIPPED, 
		" AND ", expr_area CLIPPED, 
		" AND ", expr_saldo CLIPPED, 
        	' ORDER BY 2 '
...
DEFINE rh_ret ARRAY[50] OF RECORD
	c02_codigo_sri		LIKE ordt002.c02_codigo_sri,
...
DECLARE q_ret CURSOR FOR
	SELECT c02_codigo_sri, c02_tipo_ret, c02_porcentaje, c02_nombre 
		FROM ordt002 
		WHERE c02_compania = cod_cia
		  AND c02_estado = 'A'
		ORDER BY 1
...
LET filas_max  = 300
OPEN WINDOW wh_modmaq AT 06,10
...
LET filas_max  = 250
OPEN WINDOW wh AT 06,20 WITH FORM '../../../PRODUCCION/LIBRERIAS/forms/ayuf402'
        ATTRIBUTE(FORM LINE FIRST, COMMENT LINE LAST -1, MESSAGE LINE LAST,
                   BORDER)
...
FUNCTION fl_ayuda_lineas_maq(codcia)
DEFINE rh_linmaq ARRAY[30] OF RECORD
        m05_linea               LIKE maqt005.m05_linea,
        m05_nombre              LIKE maqt005.m05_nombre
        END RECORD
DEFINE i                SMALLINT
DEFINE codcia           SMALLINT
DEFINE filas_max        SMALLINT        ## No. elementos del arreglo
DEFINE filas_pant       SMALLINT        ## No. elementos de cada pantalla
DEFINE j        SMALLINT
                                                                                
LET filas_max  = 30
OPEN WINDOW wh AT 06,39 WITH FORM '../../../PRODUCCION/LIBRERIAS/forms/ayuf403'
        ATTRIBUTE(FORM LINE FIRST, COMMENT LINE LAST -1, MESSAGE LINE LAST,
                   BORDER)
LET filas_pant = fgl_scr_size('rh_linmaq')
LET int_flag = 0
MESSAGE 'Seleccionando datos..'
DECLARE qh_linmaq CURSOR FOR
        SELECT m05_linea, m05_nombre FROM maqt005
	 WHERE m05_compania = codcia
         ORDER BY 1
LET i = 1
FOREACH qh_linmaq INTO rh_linmaq[i].*
        LET i = i + 1
        IF i > filas_max THEN
                EXIT FOREACH
        END IF
...
IF i = 0 THEN
        CALL fl_mensaje_consulta_sin_registros()
        CLOSE WINDOW wh
        INITIALIZE rh_linmaq[1].* TO NULL
        RETURN rh_linmaq[1].*
END IF
CALL set_count(i)
LET int_flag = 0
DISPLAY ARRAY rh_linmaq TO rh_linmaq.*
        ON KEY(RETURN)
                EXIT DISPLAY
        BEFORE ROW
                LET j = arr_curr()
...
FUNCTION fl_ayuda_factor_importacion_stock_rep(codcia)
DEFINE	codcia			INTEGER
DEFINE rh_impp ARRAY[100] OF RECORD
        r114_codigo			LIKE rept114.r114_codigo,
        r114_descripcion	LIKE rept114.r114_descripcion
END RECORD
DEFINE i                SMALLINT
DEFINE filas_max        SMALLINT        ## No. elementos del arreglo
DEFINE filas_pant       SMALLINT        ## No. elementos de cada pantalla
...
LET filas_max  = 100
OPEN WINDOW wh_impp AT 06,44
   WITH FORM '../../../PRODUCCION/LIBRERIAS/forms/ayuf404'
        ATTRIBUTE(FORM LINE FIRST, COMMENT LINE LAST -1, MESSAGE LINE LAST,
                   BORDER)
LET filas_pant = fgl_scr_size('rh_impp')
LET int_flag = 0
MESSAGE 'Seleccionando datos..' 
DECLARE q_impp CURSOR FOR
        SELECT r114_codigo, r114_descripcion FROM rept114
		 WHERE r114_compania = codcia
		   AND r114_flag_ident = 'IMP'
        ORDER BY 1
LET i = 1
FOREACH q_impp INTO rh_impp[i].*
        LET i = i + 1
        IF i > filas_max THEN
                EXIT FOREACH
        END IF
...
IF i = 0 THEN
        CALL fl_mensaje_consulta_sin_registros()
        CLOSE WINDOW wh_impp
        INITIALIZE rh_impp[1].* TO NULL
        RETURN rh_impp[1].r114_codigo, rh_impp[i].r114_descripcion
END IF
CALL set_count(i)
LET int_flag = 0
DISPLAY ARRAY rh_impp TO rh_impp.*
        ON KEY(RETURN)
                EXIT DISPLAY
	BEFORE ROW
		LET j = arr_curr()
...
IF int_flag THEN
        INITIALIZE rh_impp[1].* TO NULL
        RETURN rh_impp[1].r114_codigo, rh_impp[1].r114_descripcion
END IF
LET  i = arr_curr()
RETURN rh_impp[i].r114_codigo, rh_impp[i].r114_descripcion
                                                                                
END FUNCTION



FUNCTION fl_ayuda_categoria_clientes_cms(codcia, estado)
DEFINE rh_categ ARRAY[30] OF RECORD
        c04_codigo              LIKE cmst004.c04_codigo,
        c04_descripcion         LIKE cmst004.c04_descripcion
        END RECORD
DEFINE i                SMALLINT
DEFINE codcia           SMALLINT
...
LET filas_max  = 30
OPEN WINDOW wh AT 06,39 WITH FORM '../../../PRODUCCION/LIBRERIAS/forms/ayuf405'
        ATTRIBUTE(FORM LINE FIRST, COMMENT LINE LAST -1, MESSAGE LINE LAST,
                   BORDER)
LET filas_pant = fgl_scr_size('rh_categ')
LET int_flag = 0
MESSAGE 'Seleccionando datos..'
LET query = 'SELECT c04_codigo, c04_descripcion FROM cmst004 ',
	    ' WHERE c04_compania = ', codcia 
IF estado IS NOT NULL THEN
	LET query = query, ' AND c04_estado = "', estado, '"'
END IF
LET query = query, ' ORDER BY 1 '
PREPARE prepare_categ FROM query
DECLARE qh_categ CURSOR FOR prepare_categ
LET i = 1
FOREACH qh_categ INTO rh_categ[i].*
        LET i = i + 1
        IF i > filas_max THEN
                EXIT FOREACH
        END IF
...
IF i = 0 THEN
        CALL fl_mensaje_consulta_sin_registros()
        CLOSE WINDOW wh
        INITIALIZE rh_categ[1].* TO NULL
        RETURN rh_categ[1].*
END IF
CALL set_count(i)
LET int_flag = 0
DISPLAY ARRAY rh_categ TO rh_categ.*
        ON KEY(RETURN)
                EXIT DISPLAY
        BEFORE ROW
                LET j = arr_curr()
