...
SELECT UNIQUE r20_item item_tr
	...
	WHERE r19_compania    = codcia
	  ...
	  AND r20_item         IN (SELECT item
					FROM ite_cos_rea
					WHERE compania  = r20_compania
					  AND localidad = r20_localidad)
	INTO TEMP tmp_ite
...
DISPLAY ' '
SELECT UNIQUE r19_compania cia, r19_localidad loc, r19_cod_tran tp,
	r19_num_tran num_t
	...
	WHERE r19_compania   = codcia
	  ...
	  AND r20_cod_tran   = r19_cod_tran
	  AND r20_num_tran   = r19_num_tran
	  AND r20_item       = (SELECT item_tr FROM tmp_ite
...
INSERT INTO rept020_res
	...
	SELECT a.r20_compania, a.r20_localidad, a.r20_cod_tran, a.r20_num_tran,
		...
		WHERE r19_compania    = codcia
		  ...
		  AND a.r20_item     IN (SELECT item
					FROM ite_cos_rea
					WHERE compania  = a.r20_compania
					  AND localidad = a.r20_localidad)
IF STATUS < 0 THEN
