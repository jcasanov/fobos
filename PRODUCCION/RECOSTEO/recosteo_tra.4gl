...
MAIN
	...
	IF num_args() <> 4 AND num_args() <> 6 THEN
		...
		DISPLAY 'SON: BASE1 SERVIDOR_BASE1 COMPAÑIA LOCALIDAD 1/0 LG/LQ'
		EXIT PROGRAM
	END IF
	LET db1     = arg_val(1)
	LET serv1   = arg_val(2)
...
SELECT UNIQUE r20_compania, r20_localidad, r20_cod_tran, r20_num_tran
	FROM rept020
	WHERE r20_compania     = codcia1
	  AND r20_localidad    = codloc1
	  AND r20_item         IN (SELECT UNIQUE item
					FROM ite_cos_rea
					WHERE compania  = r20_compania)
...
DECLARE q_r19 CURSOR WITH HOLD FOR
	SELECT * FROM rept019
		WHERE r19_compania     = codcia1
		  AND r19_localidad    = codloc1
		  AND DATE(r19_fecing) BETWEEN MDY(01, 01, 2009)
					   --AND MDY(12, 31, 2009)
					   AND TODAY
		  AND EXISTS (SELECT 1 FROM tmp_r20
...
FOREACH q_r19 INTO r_r19.*
	IF NOT procesar_transf_ent(base1, r_r19.*) THEN
		LET l = l + 1
		CONTINUE FOREACH
	END IF
	IF r_r19.r19_cod_tran = 'TR' THEN
		...
		IF r_r19.r19_localidad <> r_r02.r02_localidad THEN
			--CALL actualizar_costo_ventas_ajustes_transf(base1, r_r19.*, 0)
			-- 1:
		...
		END IF
	END IF
	CALL procesar_costo_transacciones(base1, r_r19.*)
	LET j = j + 1
END FOREACH
DROP TABLE tmp_r20
DISPLAY ' '
...
FUNCTION procesar_transf_ent(base, r_r19)
DEFINE base		CHAR(40)
DEFINE r_r19		RECORD LIKE rept019.*
...
FUNCTION retorna_base2(localidad)
DEFINE localidad	LIKE rept002.r02_localidad
DEFINE base2		CHAR(40)

CASE localidad
	WHEN 1 LET base2 = base_gye CLIPPED, '@', serv2 CLIPPED, ':'
	WHEN 2 LET base2 = 'acero_gc@', serv2 CLIPPED, ':'
	WHEN 3 LET base2 = 'acero_qm@', serv3 CLIPPED, ':'
	WHEN 4 LET base2 = 'acero_qs@', serv4 CLIPPED, ':'
	WHEN 5 LET base2 = 'acero_qm@', serv3 CLIPPED, ':'
	OTHERWISE LET base2 = NULL
END CASE
RETURN base2 CLIPPED

END FUNCTION



FUNCTION procesar_transf_sal(base, loc_bod, r_r19, unavez)
DEFINE base		CHAR(40)
...
CALL retorna_base2(loc_bod) RETURNING base2
...
DROP TABLE t1
IF unavez THEN
	LET fecha = DATE(r_r90.r90_fecing)
...
IF r_tr_s.compania IS NULL THEN
	IF r_r90.r90_localidad <> 1 AND r_r90.r90_localidad <> 3 AND
	   YEAR(r_r90.r90_fecing) <= 2008
	THEN
		RETURN 1
	ELSE
		RETURN 0
...
FUNCTION registrar_transferencia_salida(loc_bod, r_r19)
DEFINE loc_bod		LIKE rept002.r02_localidad
DEFINE r_r19		RECORD LIKE rept019.*
DEFINE r_r90		RECORD LIKE rept090.*
...
SELECT * INTO r_r90.* FROM t1
DROP TABLE t1
LET fecha = DATE(r_r90.r90_fecing_fin)
LET lin   = r_r90.r90_codtra_fin, '-', r_r90.r90_numtra_fin USING "######&"
IF r_r90.r90_codtra_fin IS NULL THEN
	LET lin   = 'PENDIENTE'
END IF
DISPLAY '  TR-Sal LOC: ', r_r19.r19_localidad USING "&&",
		' TRAN: (', r_r19.r19_cod_tran, ')-',
		r_r19.r19_num_tran USING "######&", '. Proce.: ',
		r_r90.r90_locali_fin USING "&&", ' ', lin CLIPPED,
		'  Fec: ', fecha USING "dd-mm-yyyy"
	DISPLAY ' '
IF r_r90.r90_compania IS NULL THEN
	RETURN
...
LET query = 'INSERT INTO trans_salida ',
		'SELECT r90_compania, r90_locali_fin, r90_codtra_fin, ',
			'r90_numtra_fin, r92_item, r90_localidad, ',
			'r90_cod_tran, r90_num_tran, r92_item, "FOBOS", ',
			'CURRENT ',
			' FROM ', base2 CLIPPED, 'rept090, ',
				base2 CLIPPED, 'rept092 ',
			' WHERE r90_compania  = ', r_r19.r19_compania,
			'   AND r90_localidad = ', r_r19.r19_localidad,
			'   AND r90_cod_tran  = "', r_r19.r19_cod_tran, '"',
			'   AND r90_num_tran  = ', r_r19.r19_num_tran,
			'   AND r92_compania  = r90_compania ',
			'   AND r92_localidad = r90_localidad ',
			'   AND r92_cod_tran  = r90_cod_tran ',
			'   AND r92_num_tran  = r90_num_tran ',
			'   AND NOT EXISTS ',
				'(SELECT 1 FROM trans_salida ',
					'WHERE compania    = r90_compania ',
					'  AND local_ent   = r90_locali_fin ',
					'  AND codtran_ent = r90_codtra_fin ',
					'  AND numtran_ent = r90_numtra_fin ',
					'  AND item_ent    = r92_item) '
...
FUNCTION registrar_transferencia_llegada(r_r19)
DEFINE r_r19		RECORD LIKE rept019.*
DEFINE r_r90		RECORD LIKE rept090.*
...
SELECT * INTO r_r90.*
	...
	WHERE r90_compania   = r_r19.r19_compania
	  ...
	  AND r90_codtra_fin = r_r19.r19_cod_tran
	  AND r90_numtra_fin = r_r19.r19_num_tran
CALL retorna_base2(r_r90.r90_localidad) RETURNING base2
IF r_r90.r90_compania IS NULL THEN
	RETURN
END IF
LET query = 'INSERT INTO trans_salida ',
...
LET query = 'SELECT b.* ',
		' FROM ', base CLIPPED, ':rept020 b',
		' WHERE b.ROWID    = (SELECT MAX(a.ROWID) ',
					'FROM rept020 a ',
					'WHERE a.ROWID    < ', posi,
					'  AND a.r20_item = b.r20_item) ',
...
FUNCTION procesar_costo_transacciones(base, r_r19)
DEFINE base		CHAR(40)
DEFINE r_r19		RECORD LIKE rept019.*

CASE r_r19.r19_cod_tran
	WHEN 'A+'
		CALL actualizar_costo_ventas_ajustes_transf(base, r_r19.*, 0)
	WHEN 'A-'
		CALL actualizar_costo_ventas_ajustes_transf(base, r_r19.*, 0)
	WHEN 'AC'
		CALL actualizar_costo_compras_ajucos_imp(base, r_r19.*)
	WHEN 'AF'
		CALL actualizar_costo_dev_ventas(base, r_r19.*)
	WHEN 'CL'
		CALL actualizar_costo_compras_ajucos_imp(base, r_r19.*)
	WHEN 'DC'
		CALL actualizar_costo_dev_compras(base, r_r19.*)
	WHEN 'DF'
		CALL actualizar_costo_dev_ventas(base, r_r19.*)
	WHEN 'FA'
		CALL actualizar_costo_ventas_ajustes_transf(base, r_r19.*, 0)
...
FUNCTION actualizar_costo_ventas_ajustes_transf(base, r_tr_ori, flag)
DEFINE base		CHAR(40)
...
BEGIN WORK
WHENEVER ERROR CONTINUE
CALL query_detalle_trans(base, r_tr_ori.*, 0) RETURNING query
PREPARE tr_vta FROM query
DECLARE q_tr_vta CURSOR WITH HOLD FOR tr_vta
OPEN q_tr_vta
FETCH q_tr_vta INTO r_r19.*, r_r20.*, posi
CLOSE q_tr_vta
--IF flag AND sicomito_r19(base, r_r19.*) THEN		OJO ANTES
IF sicomito_r19(base, r_r19.*) THEN
	WHENEVER ERROR STOP
	ROLLBACK WORK
	RETURN
END IF
...
FOREACH q_tr_vta INTO r_r19.*, r_r20.*, posi
	IF unavez THEN
		CALL lee_bodega(base, r_tr_ori.r19_compania,
				r_tr_ori.r19_bodega_ori)
			RETURNING r_r02_o.*
		CALL lee_bodega(base, r_tr_ori.r19_compania,
				r_tr_ori.r19_bodega_dest)
			RETURNING r_r02_d.*
		IF (r_r19.r19_localidad = r_r02_o.r02_localidad) AND
		   (r_r19.r19_localidad = r_r02_d.r02_localidad)
		THEN
			DISPLAY '         LOC: ',r_r19.r19_localidad USING "&&",
				' TRAN: (', r_r19.r19_cod_tran, ')-',
				r_r19.r19_num_tran USING "######&",
				'  Fec: ', r_r19.r19_fecing
		END IF
		LET unavez              = 0
	...
	IF r_r20_ant.r20_costo IS NULL THEN
		LET r_r20_ant.r20_costo = r_r10.r10_costult_mb
	END IF
	LET r_r20.r20_costant_mb = r_r20_ant.r20_costo
	--}
	IF flag AND r_r19.r19_cod_tran = 'TR' THEN
		CALL obtener_transf_otra_loc(r_r20.*) RETURNING r_r20_ant.*
		LET r_r20.r20_costant_mb = r_r20_ant.r20_costant_mb
		LET r_r20.r20_costnue_mb = r_r20_ant.r20_costnue_mb
		LET r_r20.r20_costo      = r_r20_ant.r20_costo
	ELSE
...
END IF
CALL actualizar_rept019(base, r_r19.*, 1) RETURNING r_r19.*
WHENEVER ERROR STOP
COMMIT WORK
...
FUNCTION actualizar_costo_dev_ventas(base, r_tr_ori)
DEFINE base		CHAR(40)
DEFINE r_tr_ori		RECORD LIKE rept019.*
DEFINE r_g21		RECORD LIKE gent021.*
...
FOREACH q_tr_dev_vta INTO r_r19.*, r_r20.*, posi
	...
	IF YEAR(r_r20_fact.r20_fecing) > 2008 THEN
	...
	ELSE
		CALL obtener_item(base, r_r20.r20_compania, r_r20.r20_item)
			RETURNING r_r10_2.*
		LET r_r20.r20_costant_mb = r_r10_2.r10_costult_mb
		LET r_r20.r20_costnue_mb = r_r10_2.r10_costo_mb
		LET r_r20.r20_costo      = r_r10_2.r10_costo_mb
	END IF
...
FUNCTION actualizar_costo_compras_ajucos_imp(base, r_tr_ori)
DEFINE base		CHAR(40)
DEFINE r_tr_ori		RECORD LIKE rept019.*
DEFINE r_g21		RECORD LIKE gent021.*
DEFINE r_r19		RECORD LIKE rept019.*
...
FUNCTION actualizar_costo_dev_compras(base, r_tr_ori)
DEFINE base		CHAR(40)
DEFINE r_tr_ori		RECORD LIKE rept019.*
DEFINE r_g21		RECORD LIKE gent021.*
DEFINE r_r19		RECORD LIKE rept019.*
...
FUNCTION obtener_transf_otra_loc(r_r20)
DEFINE r_r20		RECORD LIKE rept020.*
DEFINE r_r20_ant	RECORD LIKE rept020.*
DEFINE r_r90		RECORD LIKE rept090.*
DEFINE query		CHAR(600)

INITIALIZE r_r90.*, r_r20_ant.* TO NULL
SELECT * INTO r_r90.*
	FROM rept090
	WHERE r90_compania   = r_r20.r20_compania
	  AND r90_locali_fin = r_r20.r20_localidad
	  AND r90_codtra_fin = r_r20.r20_cod_tran
	  AND r90_numtra_fin = r_r20.r20_num_tran
LET query = 'SELECT * FROM ', retorna_base2(r_r90.r90_localidad) CLIPPED,
				'rept020 ',
		'WHERE r20_compania  = ', r_r90.r90_compania,
		'  AND r20_localidad = ', r_r90.r90_localidad,
		'  AND r20_cod_tran  = "', r_r90.r90_cod_tran, '"',
		'  AND r20_num_tran  = ', r_r90.r90_num_tran,
		'  AND r20_bodega    = "', r_r20.r20_bodega, '"',
		'  AND r20_item      = "', r_r20.r20_item CLIPPED, '"'
PREPARE cons_tr_ant FROM query
DECLARE q_cons_tr_ant CURSOR FOR cons_tr_ant
OPEN q_cons_tr_ant
FETCH q_cons_tr_ant INTO r_r20_ant.*
CLOSE q_cons_tr_ant
FREE q_cons_tr_ant
RETURN r_r20_ant.*
...
--IF visualizar THEN
	DISPLAY '    El costo total en rept019 no puede ser CERO. ',
		r_r19.r19_cod_tran, '-',
		r_r19.r19_num_tran USING "<<<<<<&",
		' costo ', r_r19.r19_tot_costo USING "###,##&.##"
	DISPLAY ' '
--END IF
WHENEVER ERROR STOP
ROLLBACK WORK
...
FUNCTION query_detalle_trans(base, r_tr_ori, flag)
DEFINE base		CHAR(40)
DEFINE r_tr_ori		RECORD LIKE rept019.*
DEFINE flag		SMALLINT
...
LET codloc = r_tr_ori.r19_localidad
CASE r_tr_ori.r19_localidad
	WHEN 2 LET codloc2 = 1
	WHEN 4 LET codloc2 = 4
	WHEN 5 LET codloc2 = 3
END CASE
LET expr_fro = NULL
...
LET query = 'SELECT a.*, b.*, b.ROWID ',
		...
		'   AND b.r20_item      = (SELECT UNIQUE item ',
					'FROM ite_cos_rea ',
					'WHERE compania  = b.r20_compania ',
					'  AND item      = b.r20_item) '
{
					'  AND localidad IN (', codloc, ', ',
								codloc2, ')) '
}
...
LET query = 'UPDATE ', base CLIPPED, ':rept019 ',
		...
		' SET r19_tot_costo = ',
				'(SELECT NVL(SUM(r20_cant_ven * r20_costo), 0)',
					' FROM ', base CLIPPED, ':rept020 ',
					'WHERE r20_compania  = r19_compania ',
				  	'  AND r20_localidad = r19_localidad ',
				  	'  AND r20_cod_tran  = r19_cod_tran ',
				  	'  AND r20_num_tran  = r19_num_tran) ',
...
END IF
LET query = 'UPDATE ', base CLIPPED, ':rept019 ',
		' SET r19_tot_neto = r19_tot_costo ',
		' WHERE r19_compania  = ', r_r19.r19_compania,
	  	'   AND r19_localidad = ', r_r19.r19_localidad,
	  	'   AND r19_cod_tran  = "', r_r19.r19_cod_tran, '"',
	  	'   AND r19_num_tran  = ', r_r19.r19_num_tran,
		'   AND r19_cod_tran  IN ("TR", "A+", "A-", "AC") '
PREPARE up_r19_2 FROM query
EXECUTE up_r19_2
IF STATUS < 0 THEN
	DISPLAY '  Ha ocurrido un error al actualizar el registro: ',
		base CLIPPED, ' ', r_r19.r19_localidad USING "<<&", ' ',
...
LET query = 'SELECT * FROM ', base CLIPPED, ':rept019 ',
		' WHERE r19_compania  = ', r_r19.r19_compania,
	  	...
	  	'   AND r19_num_tran  = ', r_r19.r19_num_tran,
		' INTO TEMP t1 '
PREPARE sel_r19_t1 FROM query
EXECUTE sel_r19_t1
SELECT * INTO r_r19.* FROM t1
DROP TABLE t1
RETURN r_r19.*
...
FUNCTION actualizar_diario_contable_trans(base, r_r19)
DEFINE base		CHAR(40)
DEFINE r_r19		RECORD LIKE rept019.*
...
END IF
CASE r_r19.r19_localidad
	WHEN 2 LET base2 = base_gye CLIPPED, '@', serv2 CLIPPED
	WHEN 4 LET base2 = 'acero_qm@', serv3 CLIPPED
	OTHERWISE LET base2 = base
END CASE
LET query = 'SELECT a.* ',
		' FROM ', base2 CLIPPED, ':rept040, ',
			base2 CLIPPED, ':ctbt013_res a ',
...
IF STATUS = NOTFOUND THEN
	...
	DISPLAY '    Transacción no tiene diario en: ',
		base2 CLIPPED, ' ', r_r19.r19_cod_tran, '-',
		r_r19.r19_num_tran USING "<<<<<<<<&", '.'
	DISPLAY ' '
	--}
	--WHENEVER ERROR STOP
	...
	RETURN
END IF
IF r_b13_res.b13_valor_base = 0 AND r_r19.r19_cod_tran = 'TR' THEN
	--WHENEVER ERROR STOP
	--ROLLBACK WORK
	RETURN
END IF
CALL mayoriza_comprobante(base2, r_b13_res.b13_compania,r_b13_res.b13_tipo_comp,
			  r_b13_res.b13_num_comp, 'D')
SET LOCK MODE TO WAIT 20
BEGIN WORK
WHENEVER ERROR CONTINUE
--CALL obtener_costo_total(base2, r_r19.*) RETURNING tot_costo
LET tot_costo = r_r19.r19_tot_costo
FOREACH q_b13 INTO r_b13_res.*
	CALL actualizar_ctbt013(base2, r_b13_res.*, tot_costo)
END FOREACH
LET query = 'SELECT NVL(SUM(b13_valor_base), 0) valor_base ',
		' FROM ', base2 CLIPPED, ':ctbt013 ',
...
IF valor <> 0 THEN
	DISPLAY '    Error al actualizar el diario en ',
		base2 CLIPPED, ' ', r_b13_res.b13_tipo_comp, '-',
...
FUNCTION obtener_costo_total(base, r_r19)
DEFINE base		CHAR(40)
DEFINE r_r19		RECORD LIKE rept019.*
DEFINE r_r20		RECORD LIKE rept020.*
DEFINE posi		INTEGER
...
END IF
CALL query_detalle_trans(base, r_r19.*, 0) RETURNING query
