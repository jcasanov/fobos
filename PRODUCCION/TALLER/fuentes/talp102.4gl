...
DEFER INTERRUPT
CLEAR SCREEN
CALL startlog('../logs/talp102.error')
CALL fgl_init4js()
...
OPEN WINDOW w_sec AT 3,2 WITH 13 ROWS, 80 COLUMNS 
...
IF int_flag THEN
	...
	RETURN
END IF
LET query = 'SELECT *, ROWID FROM talt002 WHERE t02_compania = ',
...
INPUT BY NAME rm_sec.t02_nombre, rm_sec.t02_jefe  
	...
	AFTER INPUT
		...
		SELECT t02_seccion INTO serial
		 FROM talt002
      		 WHERE t02_compania  = vg_codcia 
		...
		IF status <> NOTFOUND THEN
		   ...
		   THEN
		   	CALL fgl_winmessage(vg_producto,'El nombre de la sección ya existe en la compañia en el registro de código  '|| serial,'exclamation')
