...
-- Autor            : NPC
-- Formato Ejecucion: fglrun talp206 base módulo compañía localidad
-- Ultima Correccion: 
-- Motivo Correccion: 
------------------------------------------------------------------------------
...
IF num_oc > 0 THEN
        CALL fgl_winmessage(vg_producto,'Existen órdenes de compras que no ' ||
					'se han cerrado.', 'stop')
	RETURN
END IF	
...
IF rm_ord.t23_cod_cliente IS NULL THEN
	LET int_flag = 0
	CALL fgl_winquestion(vg_producto,'Esta orden no tiene el código del cliente. Desea ingresarlo ?','No','Yes|No|Cancel','question',1)
		RETURNING resp
        WHENEVER ERROR STOP
	IF resp = 'Yes' THEN
        	LET int_flag = 1
	...
	IF rm_ord.t23_estado <> 'A' THEN
		CALL retorna_estado(rm_ord.t23_estado) RETURNING estado
       		IF rm_ord.t23_estado = 'C' THEN
       	 	...
       	 	ELSE
               	      	CALL fgl_winmessage(vg_producto,'Esta orden no está ACTIVA, sino ' || estado,'exclamation')
        		END IF
	ELSE
...
IF r_v00.v00_compania IS NULL THEN
	CALL fgl_winmessage(vg_producto, 
			    'No existe configuracion de vehículos.',
			    'stop')
	LET int_flag = 1
	RETURN	
END IF
...
------------------------
	...
	IF r_v30.v30_num_tran = -1 THEN
		CALL fgl_winmessage(vg_producto,
			'No se pudo generar el ajuste de costo al vehículo.',
			'exclamation')
		LET int_flag = 1
	        RETURN
	END IF
