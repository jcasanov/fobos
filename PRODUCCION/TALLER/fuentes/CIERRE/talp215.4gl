--------------------------------------------------------------------------------
-- Titulo           : talp215.4gl - Eliminacion Ordenes de Trabajo que no FA/DF
-- Elaboracion      : 24-Oct-2008
-- Autor            : NPC
-- Formato Ejecucion: fglrun talp215 base módulo compañía localidad
-- Ultima Correccion: 
-- Motivo Correccion: 
--------------------------------------------------------------------------------
...
DEFINE rm_det 		ARRAY [25000] OF RECORD
				fecha		DATE,
				...
				eliminar	CHAR(1)
			END RECORD
DEFINE rm_cli		ARRAY [25000] OF RECORD
...
DEFER INTERRUPT
CLEAR SCREEN
CALL startlog('../logs/talp215.err')
--#CALL fgl_init4js()
...
IF vg_gui = 0 THEN
	...
	LET num_cols = 78
END IF
OPEN WINDOW w_talp215 AT row_ini, 02 WITH num_rows ROWS, num_cols COLUMNS
	ATTRIBUTE(FORM LINE FIRST + 1, COMMENT LINE LAST, MENU LINE lin_menu,
		  MESSAGE LINE LAST, BORDER) 
IF vg_gui = 1 THEN
	OPEN FORM f_talf215_1 FROM "../forms/talf215_1"
ELSE
	OPEN FORM f_talf215_1 FROM "../forms/talf215_1c"
END IF
DISPLAY FORM f_talf215_1
...
WHILE TRUE
	...
	CALL borrar_detalle()
	CALL muestra_contadores_det(0)
	SELECT DATE(t23_fecing) fecha_tran, t23_orden ord_t,
		t23_tot_bruto valor_mo, t23_tot_bruto valor_fa,
		t23_tot_bruto valor_oc, t23_tot_bruto valor_tot, t23_estado est,
		t23_estado elimin, t23_cod_cliente codcli,
		t23_nom_cliente nomcli, t23_descripcion descripcion
		FROM talt023
	...
	WHILE NOT salir
		...
		LET total_ot = 0
		LET i        = 1
		FOREACH q_ordtal INTO rm_det[i].*, rm_cli[i].*
		...
		WHILE TRUE
			...
			INPUT ARRAY rm_det WITHOUT DEFAULTS FROM rm_det.*
				...
				ON KEY(F5)
					LET i = arr_curr()
					CALL fl_ver_orden_trabajo(
							rm_det[i].t23_orden,'O')
					LET int_flag = 0
				ON KEY(F6)
					LET i = arr_curr()
					IF rm_det[i].t23_val_mo_tal = 0 THEN
						CONTINUE INPUT
					END IF
					CALL fl_muestra_mano_obra_orden_trabajo(
							vg_codcia, vg_codloc,
							rm_det[i].t23_orden, 1)
					LET int_flag = 0
				ON KEY(F7)
					LET i = arr_curr()
					IF rm_det[i].tot_oc = 0 THEN
						CONTINUE INPUT
					END IF
				...
				ON KEY(F8)
					...
					IF rm_det[i].tot_fa = 0 THEN
						CONTINUE INPUT
					END IF
					CALL fl_control_prof_trans(vg_codcia,
							vg_codloc,
							rm_det[i].t23_orden)
					LET int_flag = 0
				ON KEY(F9)
					LET i = arr_curr()
					CALL actualizar_tmp('S', i)
					IF control_eliminacion() THEN
						LET int_flag = 1
						EXIT INPUT
					END IF
					LET int_flag = 0
				ON KEY(F10)
					CALL todas_niguna_ot('T')
					EXIT INPUT
				ON KEY(F11)
					CALL todas_niguna_ot('N')
					EXIT INPUT
		                ON KEY(F15)
	        	                LET col   = 1
					LET salir = 1
				...
				BEFORE ROW
					...
					DISPLAY BY NAME rm_cli[i].*
					IF rm_det[i].t23_val_mo_tal = 0 THEN
						CALL dialog.keysetlabel("F6","")
					ELSE
						CALL dialog.keysetlabel("F6","Mano de Obra") 
					END IF
					IF rm_det[i].tot_oc = 0 THEN
					...
					ELSE
						CALL dialog.keysetlabel("F8","Detalle Proforma") 
					END IF
				AFTER FIELD eliminar
					CALL actualizar_tmp(rm_det[i].eliminar,i)
				AFTER INPUT
					CONTINUE INPUT
			END INPUT
        		IF salir = 1 OR int_flag = 1 THEN
				IF int_flag = 0 THEN
					LET salir = 0
				END IF
	                	EXIT WHILE
...
LET query = "INSERT INTO tmp_det ",
		...
		" (SELECT NVL(SUM(r21_tot_bruto - r21_tot_dscto), 0) ",
			...
			"   AND r21_localidad = t23_localidad ",
			"   AND r21_num_ot    = t23_orden), ",
		" t23_estado, 'N', t23_cod_cliente, t23_nom_cliente, ",
		" t23_descripcion ",
		" FROM talt023 ",
...
FUNCTION actualizar_tmp(elim, i)
DEFINE elim		CHAR(1)
DEFINE i		SMALLINT

UPDATE tmp_det
	SET elimin  = elim
	WHERE ord_t  = rm_det[i].t23_orden
	  AND codcli = rm_cli[i].t23_cod_cliente
LET rm_det[i].eliminar = elim
...
--#DISPLAY "Ord Tra."		TO tit_col2
--#DISPLAY "Total MO"		TO tit_col3
--#DISPLAY "Total OC"		TO tit_col4
--#DISPLAY "Tot. Prof."		TO tit_col5
--#DISPLAY "Total OT"		TO tit_col6
...
SELECT COUNT(*) INTO cuantos FROM tmp_det WHERE elimin = 'S'
IF cuantos = 0 THEN
	CALL fl_mostrar_mensaje('Al menos marque una orden de trabajo para eliminarla.', 'exclamation')
	RETURN 0
END IF
IF cuantos > 1 THEN
	LET int_flag = 0
	CALL fl_hacer_pregunta('Desea ELIMINAR las ordenes de trabajo marcadas ?', 'No')
		RETURNING resp
	IF resp <> 'Yes' THEN
		RETURN 0
	END IF
...
FOR i = 1 TO vm_num_det
	IF rm_det[i].eliminar = 'N' THEN
		CONTINUE FOR
	END IF
	CALL eliminar_orden_trabajo(i, cuantos) RETURNING elimino
...
FUNCTION eliminar_orden_trabajo(pos, cuantos)
DEFINE pos		SMALLINT
DEFINE cuantos		INTEGER
DEFINE resp    		CHAR(6)
...
IF NOT (rm_t23.t23_estado <> 'F' AND rm_t23.t23_estado <> 'D') THEN
	...
	RETURN 0
END IF
IF rm_t23.t23_tot_neto <> 0 AND cuantos = 1 THEN
...
BEGIN WORK
	...
	DECLARE q_blo CURSOR FOR
		SELECT * FROM talt023
			WHERE t23_compania  = vg_codcia
			  AND t23_localidad = vg_codloc
			  AND t23_orden     = rm_det[pos].t23_orden
	      	FOR UPDATE
	OPEN q_blo
...
END IF
IF cuantos = 1 THEN
	LET mensaje = 'Orden de Trabajo ', rm_det[pos].t23_orden USING "<<<<<&",
			' ha sido ELIMINADA.'
...
FOR i = 1 TO vm_num_det
	CASE flag
		WHEN 'T'
			LET rm_det[i].eliminar = 'S'
		WHEN 'N'
			LET rm_det[i].eliminar = 'N'
	END CASE
	CALL actualizar_tmp(rm_det[i].eliminar, i)
END FOR
