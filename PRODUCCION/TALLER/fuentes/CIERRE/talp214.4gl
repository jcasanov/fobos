	...
	AFTER FIELD t60_fac_ant
		...
		IF rm_t60.t60_fac_ant IS NOT NULL THEN
			...
			IF r_t23.t23_cont_cred = 'R' THEN
				...
				SELECT NVL(SUM(z20_valor_cap + z20_valor_int),
					0),
					NVL(SUM(z20_saldo_cap + z20_saldo_int),
					0)
					INTO valor, deuda
					FROM cxct020
					WHERE z20_compania  = r_t23.t23_compania
					  ...
					  AND z20_num_tran  IN
						...
						WHERE r19_compania    =
						  ...
						  AND r19_ord_trabajo =
								r_t23.t23_orden)
				IF deuda <> valor THEN
					CALL fl_mostrar_mensaje('Esta factura no puede ser REFACTURADA, porque alguna de sus factura de INVENTARIO han sido parcial o totalmente canceladas.', 'exclamation')
					NEXT FIELD t60_fac_ant
				END IF
			END IF
