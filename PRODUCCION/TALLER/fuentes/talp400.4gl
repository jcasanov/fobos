...
DEFER INTERRUPT
CLEAR SCREEN
CALL startlog('../logs/talp400.error')
CALL fgl_init4js()
...
PAGE HEADER
	...
	PRINT COLUMN 1,   rg_cia.g01_razonsocial,
	      COLUMN 172, 'Página: ', PAGENO USING '&&&'
	PRINT COLUMN 1,   modulo CLIPPED,
	      COLUMN 79,  titulo CLIPPED,
	      COLUMN 176, UPSHIFT(vg_proceso) 
	PRINT COLUMN 77,  '** Moneda        : ', rm_t23.t23_moneda, ' ',
					 	 rm_g13.g13_nombre
	PRINT COLUMN 77,  '** Línea (Marca) : ', rm_t04.t04_linea, ' ',
						 r_t01.t01_nombre
	PRINT COLUMN 77,  '** Tipo de Orden : ', rm_t23.t23_tipo_ot, ' ',
			  			 r_t05.t05_nombre
	PRINT COLUMN 77,  '** Estado        : ', rm_t23.t23_estado, ' ',
						 nom_estado 
	PRINT COLUMN 77,  '** Fecha Inicial : ', fecha_desde USING 'dd-mm-yyyy'
	PRINT COLUMN 77,  '** Fecha Final   : ', fecha_hasta USING 'dd-mm-yyyy'
	PRINT COLUMN 77,  '** Tipo          : ';
	...
	PRINT COLUMN 1,   'Fecha',
	      ...
	      COLUMN 18,  '        Factura',
	      COLUMN 34,  'Cliente',
	      COLUMN 55,  'M.O. Interna',
	      COLUMN 68,  'M.O. Externa',
	      COLUMN 81,  'Rep.Externos',
	      COLUMN 94, 'Rep. Almacen',
	      COLUMN 107, '    Viaticos',
	      COLUMN 120, ' Suministros',
	      COLUMN 133, ' Total Bruto',
	      COLUMN 146, 'Total Dscto.',
	      COLUMN 159, 'Total Impto.',
	      COLUMN 172, '  Total Neto'
	PRINT '======================================================================================================================================================================================='
...
ON EVERY ROW
	...
	PRINT COLUMN 1,	  DATE(r_report.fecha)	USING 'dd-mm-yy',
	      ...
	      COLUMN 55,  r_report.mo 	 	USING '#,###,##&.##',
	      COLUMN 68,  r_report.mo_ci_ext	USING '#,###,##&.##',
	      COLUMN 81,  r_report.rep_ci_ext 	USING '#,###,##&.##',
	      COLUMN 94,  r_report.rep_alm	USING '#,###,##&.##',
	      COLUMN 107, r_report.viaticos	USING '#,###,##&.##',
	      COLUMN 120, r_report.suministros	USING '#,###,##&.##',
	      COLUMN 133, r_report.tot_bruto	USING '#,###,##&.##',
	      COLUMN 146, r_report.tot_dscto	USING '#,###,##&.##',
	      COLUMN 159, r_report.tot_impto	USING '#,###,##&.##',
	      COLUMN 172, r_report.tot_neto	USING '#,###,##&.##'
...
ON LAST ROW
	...
	PRINT COLUMN 42, 'TOTALES ==>  ',
	      COLUMN 55,  SUM(r_report.mo) 	 	USING '#,###,##&.##',
	      COLUMN 68,  SUM(r_report.mo_ci_ext)	USING '#,###,##&.##',
	      COLUMN 81,  SUM(r_report.rep_ci_ext) 	USING '#,###,##&.##',
	      COLUMN 94,  SUM(r_report.rep_alm)		USING '#,###,##&.##',
	      COLUMN 107, SUM(r_report.viaticos)	USING '#,###,##&.##',
	      COLUMN 120, SUM(r_report.suministros)	USING '#,###,##&.##',
	      COLUMN 133, SUM(r_report.tot_bruto) 	USING '#,###,##&.##',
	      COLUMN 146, SUM(r_report.tot_dscto) 	USING '#,###,##&.##',
	      COLUMN 159, SUM(r_report.tot_impto) 	USING '#,###,##&.##',
	      COLUMN 172, SUM(r_report.tot_neto)  	USING '#,###,##&.##'
