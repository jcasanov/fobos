...
IF r_j02.j02_codigo_caja IS NULL THEN
	CALL fl_mostrar_mensaje('No hay una caja asignada al usuario ' || vg_usuario || '.','exclamation')
	RETURN 0
END IF
...
END FOREACH
IF NOT salir THEN
	CALL fl_mostrar_mensaje('La caja del usuario ' || r_t23.t23_usuario CLIPPED || ' no esta aperturada.', 'exclamation')
END IF
RETURN salir
