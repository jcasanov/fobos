	...
	INPUT ARRAY r_detalle WITHOUT DEFAULTS FROM r_detalle.*
		...
		ON KEY(F2)          
			IF INFIELD(r22_bodega) THEN  
				CALL fl_ayuda_bodegas_rep(vg_codcia, vg_codloc, 'A', 'F', 'R', 'S')
			...
			END IF
				...
				CALL muestra_etiquetas_det(i, max_row, i)
                       		IF rm_r10.r10_estado = 'B' THEN           
                       			CALL fl_mostrar_mensaje('El Item esta con status bloqueado.','exclamation')           
                       			NEXT FIELD r22_item 
                       		END IF                     
...
END IF
COMMIT WORK
LET mensaje = 'Se generó ', preventas USING "<<<<<<&", ' preventas ',
