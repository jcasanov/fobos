...
IF resp = 'Yes' THEN
	LET param = vg_codloc, ' ', rm_ord.t23_orden, ' O '
...
IF r_g10.g10_codcobr IS NULL THEN
	...
	CALL fl_mostrar_mensaje('Tarjeta de crédito: ' ||
		r_g10.g10_nombre || ' no tiene código cobranzas asignado. '
		|| 'Por favor asígnelo en el módulo de parámetro GENERALES.',
		'stop')
	EXIT PROGRAM
...
DECLARE qu_profc CURSOR FOR SELECT r21_numprof FROM rept021
	WHERE r21_compania  = vg_codcia AND 
	      ...
	      r21_num_ot    = vm_orden AND 
	      r21_cod_tran  IS NULL
FOREACH qu_profc INTO numprof
	SELECT SUM(r22_cantidad) INTO cant FROM rept022
		WHERE r22_compania  = vg_codcia AND 
	              ...
	              r22_numprof   = numprof
	IF cant > 0 THEN 
		LET mensaje = 'La proforma: ', numprof, ' no está facturada.'
...
IF status <> NOTFOUND THEN
	LET mensaje = 'El item: ', item CLIPPED, 
		      ' queda con saldo negativo: ', 
		      cantidad USING '-----&', 
		      '. Revise las facturas y las transferencias ', 
		      'asociadas a la orden de trabajo.'
	ROLLBACK WORK
	EXIT PROGRAM
	CALL fl_mostrar_mensaje(mensaje, 'stop');
END IF		
