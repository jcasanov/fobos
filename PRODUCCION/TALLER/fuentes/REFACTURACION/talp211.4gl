...
DEFINE vm_nota_credito  LIKE cxpt021.p21_tipo_doc
...
LET int_flag = 0
INPUT BY NAME rm_t23.t23_num_factura
	WITHOUT DEFAULTS
	ON KEY(INTERRUPT)
...
END CASE
DISPLAY BY NAME r_tal.t28_num_dev, r_tal.t28_fec_anula,
		rm_t23.t23_num_factura, rm_t23.t23_nom_cliente,
		rm_t23.t23_fec_factura, rm_t23.t23_orden,
		rm_t23.t23_modelo
...
END IF
LET int_flag = 0
CALL fl_hacer_pregunta('Desea anular también las Recepciones por Ordenes de Compra ?.',	'Yes')
	RETURNING resp
IF resp <> 'Yes' THEN
...
SELECT * INTO r_c00.* FROM ordt000 WHERE c00_compania = vg_codcia
...
THEN
	LET mensaje = 'No se puede anular la recepción # ',
			...
			' por orden de compra # ',
			rm_c10.c10_numero_oc USING "<<<<<<<&",
			'. Revise la configuración de Compañías en el móduloi',
			' de COMPRAS.'
	CALL fl_mostrar_mensaje(mensaje CLIPPED, 'exclamation')
	RETURN 0
END IF
...
CALL fl_lee_comprobante_contable(codcia, tipo_comp, num_comp)
	RETURNING r_b12.*
