	...
	COMMAND KEY('C') 'Consultar'
		...
		IF vm_num_doc > 0 THEN
                ...
                ELSE
                	HIDE OPTION 'Detalle'
                END IF
		IF rm_par.localidad IS NULL AND rm_par.tipo_saldo = 'T' THEN
			SHOW OPTION 'Cobranza vs. Cont.'
                ELSE
			HIDE OPTION 'Cobranza vs. Cont.'
                END IF
...
LET query = 'INSERT INTO tempo_doc ',
		'SELECT z20_localidad, z20_areaneg, z20_tipo_doc, z20_num_doc,',
			...
			' THEN "Vencido" ELSE "Por Vencer" END, ',
...
LET expr2 = NULL
IF rm_par.fecha_emi IS NOT NULL THEN
	LET expr2 = '    OR fecha_emi < "', rm_par.fecha_emi, '"'
	INSERT INTO tmp_sal_ini2
...
LET query = 'SELECT NVL(SUM(z26_valor), 0) valor ',
		...
		'   AND z26_estado       = "A"',
		'   AND z26_fecha_cobro <= "', rm_par.fecha_cart, '"',
		' INTO TEMP t_suma '
...
WHILE TRUE
	...
	DISPLAY ARRAY rm_dcli TO rm_dcli.*
		...
		ON KEY(F5)
			LET i = arr_curr()
			SELECT cod_tran, num_tran
				INTO r_aux.cod_tran, r_aux.num_tran
				FROM tempo_doc 
				WHERE ROWID = rm_rowid[i]
			CALL ver_factura_devolucion(i, r_aux.cod_tran,
							r_aux.num_tran, 'D')
			LET int_flag = 0
		ON KEY(F6)
			LET i = arr_curr()
...
WHILE TRUE
	...
	DISPLAY ARRAY r_dda TO r_dda.*
		...
		ON KEY(F8)
			...
			END IF
			LET i = arr_curr()
			CALL ver_factura_devolucion(i, r_aux[i].cod_tran,
							r_aux[i].num_tran, 'F')
			LET int_flag = 0
		ON KEY(F15)
			LET col      = 1
...
FUNCTION ver_factura_devolucion(i, cod_tran, num_tran, tipo)
DEFINE i		INTEGER
...
CALL fl_lee_area_negocio(vg_codcia, rm_dcli[i].tit_area) RETURNING r_g03.*
CASE r_g03.g03_modulo
	WHEN 'RE'
		CALL fl_ver_transaccion_rep(vg_codcia, rm_dcli[i].tit_loc,
						cod_tran, num_tran)
	WHEN 'TA'
		LET prog = 'talp204 '
		LET expr = num_tran, ' F'
		IF tipo = 'F' THEN
			SELECT * INTO r_t28.*
				...
				WHERE t28_compania  = vg_codcia
				  AND t28_localidad = rm_dcli[i].tit_loc
