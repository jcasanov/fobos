...
LET query = 'INSERT INTO tempo_acum ',
		 'SELECT a.localidad, a.', campo CLIPPED, ', descri, 0, ',
			subquery_pven_venc('b.', campo, rm_par.rango1_i,
					rm_par.rango1_f), ', ',
			subquery_pven_venc('c.', campo, rm_par.rango2_i,
					rm_par.rango2_f), ', ',
			subquery_pven_venc('d.', campo, rm_par.rango3_i,
					rm_par.rango3_f), ', ',
			subquery_pven_venc('e.', campo, rm_par.rango4_i,
					rm_par.rango4_f), ', ',
			subquery_pven_venc('f.', campo, rm_par.rango5_i, 0),
...
LET expr_sql = NULL
LET expr_fec = '" - ', prefijo CLIPPED, 'fecha BETWEEN ', rango_i, ' AND ',
		rango_f
IF rango_f = 0 THEN
	LET expr_fec = '" - ', prefijo CLIPPED, 'fecha >= ', rango_i
END IF
...
LET subquery = 'NVL((SELECT NVL(SUM(', prefijo CLIPPED, 'valor), 0) ',
		...
		'   AND ', prefijo CLIPPED, 'fecha      < "', rm_par.fecha_cart,
							'"',
			expr_sql CLIPPED,
