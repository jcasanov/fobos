...
IF rm_b00.b00_compania IS NULL THEN
	--CALL fgl_winmessage(vg_producto,'No existe configuración para esta compañía en el módulo.','stop')
...
DEFINE secuencia	ARRAY[2000] OF SMALLINT
DEFINE resp		CHAR(6)
        ...
        LET INT_FLAG = 0
{
	...
	INPUT ARRAY rm_cuenta WITHOUT DEFAULTS FROM ra_cuenta.*
		...
		AFTER ROW
			IF secuencia[i] IS NOT NULL THEN
				LET secuencia[i] = 
...
IF i > vm_ind_cta THEN
	SELECT MAX(secuencia) INTO sec FROM tmp_detalle
	IF sec IS NULL THEN
		LET sec = 1
	ELSE
		LET sec = sec + 1
	END IF
	IF NOT rm_cuenta[i].cuenta IS NULL THEN
		INSERT INTO tmp_detalle VALUES(
       			sec, rm_cuenta[i].*,
                        0, rm_otros[i].*
		)
		LET vm_ind_cta = vm_ind_cta + 1
...
IF STATUS = NOTFOUND THEN
	--CALL fgl_winmessage(vg_producto,'No existe comprobante contable.','exclamation')
