select * from acero_qm:rept090
	where r90_compania         = 1
	  and r90_localidad        = 1
...
select count(*) tot_tr_uio from tmp_tr_uio;
drop table t2;
select r90_localidad lc, r90_cod_tran tp, r90_num_tran num, r90_locali_fin lcf,
	r90_numtra_fin numf, date(r90_fecing_fin) fecfin, r40_tipo_comp tpc,
	r40_num_comp num_c
	from t1, outer aceros:rept040
	where r40_compania  = r90_compania
...
drop table t2;
select nvl(round(sum(valor_ctb), 2), 0) tot_uio from tmp_tr_uio;
select nvl(round(sum(b.valor_ctb), 2), 0) tot_gye
	--from tmp_tr_uio b, outer tmp_tr_gye a
	from tmp_tr_uio a, outer tmp_tr_gye b
	where b.lc  = a.lc
	  and b.tp  = a.tp
	  and b.num = a.num;
