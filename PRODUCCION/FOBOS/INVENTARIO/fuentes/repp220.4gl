	...
	ON KEY(F2)
		...
		IF INFIELD(r21_codcli) THEN
			CALL fl_ayuda_cliente_localidad(vg_codcia,vg_codloc)
				RETURNING rm_z02.z02_codcli, rm_z01.z01_nomcli
...
IF num_args() <> 6 THEN
	...
	END IF
END IF
SELECT COUNT(r22_bodega)
	INTO cuantos
	FROM rept022, rept002
	WHERE r22_compania  = vg_codcia
	  AND r22_localidad = vg_codloc
	  AND r22_numprof   = rm_r21.r21_numprof
	  AND r02_compania  = r22_compania
	  AND r02_codigo    = r22_bodega
	  ...
	  AND r02_estado    = 'A'
IF cuantos > 0 THEN
	CALL fl_mostrar_mensaje('No se puede convertir en PRE-VENTA esta proforma debido a que tiene la bodega SIN STOCK.', 'exclamation')
	RETURN
END IF
...
FOREACH q_barc INTO r_r11.*
	...
	IF r_r02.r02_tipo = 'S' OR r_r02.r02_area = 'T' OR
	  (r_r02.r02_tipo_ident <> 'V' AND r_r02.r02_tipo_ident <> 'X' AND
	   r_r02.r02_tipo_ident <> 'E' AND r_r02.r02_tipo_ident <> 'P')
	THEN
		CONTINUE FOREACH
	END IF
	IF r_r02.r02_tipo_ident = 'P' AND flag = 'C' THEN
...
{--
IF vg_usuario = 'HSALAZAR' OR vg_usuario = 'JEFESIST' THEN
...
--}
	...
	--#CALL WinExec(comando) RETURNING err_flag
	--run "sleep 3"
	--LET comando = 'prueba.exe ', rm_r21.r21_numprof USING "<<<<<<&"
	--CALL WinExec(comando) RETURNING err_flag
--END IF
