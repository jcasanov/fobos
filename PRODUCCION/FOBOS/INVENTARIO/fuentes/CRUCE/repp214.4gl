...
DEFINE vm_fec_aut	LIKE ordt013.c13_fec_aut
...
CLOSE q_p01
FREE q_p01
IF rm_r19.r19_bodega_ori <> '17' THEN
...
DEFINE contador, lim	SMALLINT

DEFINE oc_ant		LIKE ordt010.c10_numero_oc

INITIALIZE vm_fec_aut, vm_num_aut, vm_serie_comp, vm_fecha_cadu TO NULL

LET INT_FLAG = 0
INPUT BY NAME rm_r19.r19_cod_tran, rm_r19.r19_oc_interna, vm_fec_aut,vm_num_aut,
	...
	ON KEY(INTERRUPT)
		IF NOT FIELD_TOUCHED(rm_r19.r19_cod_tran, rm_r19.r19_oc_interna,
				vm_fec_aut, vm_num_aut, vm_serie_comp,
				vm_fecha_cadu, rm_r19.r19_oc_externa,
				rm_r19.r19_fact_venta, rm_r19.r19_vendedor,
	...
	AFTER FIELD r19_bodega_ori
		IF rm_r19.r19_bodega_ori IS NOT NULL THEN
			...
			IF r_r02.r02_localidad <> vg_codloc THEN
				CALL fl_mostrar_mensaje('Debe escoger una bodega de esta localidad.', 'exclamation')
				CLEAR n_bodega
				NEXT FIELD r19_bodega_ori
			END IF
			DISPLAY r_r02.r02_nombre TO n_bodega
	...
	AFTER FIELD r19_oc_externa
		...
		IF rm_r19.r19_oc_externa[5, 7] <> vm_serie_comp[4, 6] THEN
			...
			NEXT FIELD r19_oc_externa
		END IF
		IF NOT fl_valida_numeros(rm_r19.r19_oc_externa[1, 3]) THEN
			NEXT FIELD r19_oc_externa
		END IF
		IF NOT fl_valida_numeros(rm_r19.r19_oc_externa[5, 7]) THEN
			NEXT FIELD r19_oc_externa
		END IF
		LET lim = LENGTH(rm_r19.r19_oc_externa)
		IF NOT fl_valida_numeros(rm_r19.r19_oc_externa[9, lim]) THEN
			NEXT FIELD r19_oc_externa
		END IF
...
SELECT c13_fec_aut, c13_num_aut, c13_serie_comp, c13_fecha_cadu
	INTO vm_fec_aut, vm_num_aut, vm_serie_comp, vm_fecha_cadu
	FROM ordt013
	WHERE c13_compania  = vg_codcia
	  AND c13_localidad = vg_codloc
	  AND c13_numero_oc = rm_r19.r19_oc_interna
...
DISPLAY BY NAME vm_fec_aut, vm_num_aut, vm_serie_comp, vm_fecha_cadu
...
IF r_c10.c10_numero_oc IS NULL THEN
	INITIALIZE rm_r19.r19_moneda,
		   ...
		   vm_fec_aut, vm_num_aut, vm_serie_comp, vm_fecha_cadu
	 	TO NULL
	CLEAR cod_proveedor, n_proveedor, n_moneda, vm_fec_aut, vm_num_aut,
		vm_serie_comp, c10_flete, c10_otros, vm_fecha_cadu
ELSE
...
LET r_c13.c13_fec_aut     = vm_fec_aut
...
LET mensaje = 'Transferencias por CRUCE de BODEGA "SIN STOCK" con la bodega ',
		rm_r19.r19_bodega_ori CLIPPED, ' generadas OK.'
...
SELECT r21_cod_tran, r21_num_tran
	FROM rept021
	WHERE r21_compania    = vg_codcia
	  AND r21_localidad   = vg_codloc
	  AND r21_trans_fact  = 'S'
	  AND r21_cod_tran   IS NOT NULL
	  AND NOT EXISTS
		(SELECT 1 FROM rept069
			WHERE r69_compania  = r21_compania
			  AND r69_localidad = r21_localidad
			  AND r69_cod_tran  = r21_cod_tran
			  AND r69_num_tran  = r21_num_tran)
	INTO TEMP tmp_r21
...
SELECT r20_cod_tran, r20_num_tran, r20_fecing fecha, r20_bodega, r20_item,
	...
	WHERE r20_compania   = vg_codcia
	  ...
	  AND NOT EXISTS
		(SELECT 1 FROM rept069
			WHERE r69_compania  = r20_compania
			  AND r69_localidad = r20_localidad
			  AND r69_cod_tran  = r20_cod_tran
			  AND r69_num_tran  = r20_num_tran)
	  AND NOT EXISTS
		(SELECT 1 FROM tmp_r21
			WHERE r21_cod_tran = r20_cod_tran
			  AND r21_num_tran = r20_num_tran)
	INTO TEMP t_r20
	  ...
	  AND NOT EXISTS
		(SELECT 1 FROM rept069
			WHERE r69_compania  = r19_compania
			  AND r69_localidad = r19_localidad
			  AND r69_cod_tran  = r19_cod_tran
			  AND r69_num_tran  = r19_num_tran)
	  AND NOT EXISTS
		(SELECT 1 FROM tmp_r21
			WHERE r21_cod_tran = r19_cod_tran
	...
	WHERE r19_compania   = vg_codcia
	  ...
	  AND NOT EXISTS
		(SELECT 1 FROM rept069
			WHERE r69_compania  = r19_compania
			  AND r69_localidad = r19_localidad
			  AND r69_cod_tran  = r19_tipo_dev
			  AND r69_num_tran  = r19_num_dev)
	  AND NOT EXISTS
		(SELECT 1 FROM tmp_r21
			WHERE r21_cod_tran = r19_tipo_dev
			  AND r21_num_tran = r19_num_dev)
	INTO TEMP t_r19
DROP TABLE tmp_r21
