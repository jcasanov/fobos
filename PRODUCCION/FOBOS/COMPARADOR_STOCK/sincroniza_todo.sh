# SINCRONIZA  EL STOCK ENTRE TODAS LAS LOCALIDADES
...
> $directorio/sincroniza_todo.log
for i in $stringdb;
do
        time sh sincroniza.sh $i > $directorio/$i.sincro.log
	cat $directorio/$i.sincro.log >> $directorio/sincroniza_todo.log
done

distintos=`sh suma_distintos.sh $directorio`
echo ""
echo "TOTAL DISTINTOS: "$distintos >> $directorio/compara_todo.log
echo "TOTAL DISTINTOS: "$distintos >> $directorio/sincroniza_todo.log

if [ $distintos != 0 ];
then
        echo "ENVIANDO MENSAJE A CELULAR .."
        echo "TOTAL DISTINTOS: "$distintos |nail -r "stock_$distintos@fobos.com" \
        -s "FOBOS: SINCRONIZA STOCK" \
	95115593@im.movistar.com.ec 99038655@im.movistar.com.ec

        echo "ENVIANDO MAIL CON LOG ADJUNTO .."
        echo "TOTAL DISTINTOS: "$distintos |nail -r "fobos@acerocomercial.com" \
        -s "FOBOS: SINCRONIZA  STOCK" -a $directorio/sincroniza_todo.log \
