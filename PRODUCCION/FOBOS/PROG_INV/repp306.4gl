...
LET query = 'SELECT DATE(r21_fecing) fecha_ini, r21_numprof, r21_nomcli, ',
	  ' r01_iniciales, (DATE(r21_fecing) + r21_dias_prof) fecha_max, ',
		  ' r21_tot_neto, r21_cod_tran ',
		  ' FROM rept021, rept001 ',
		  ' WHERE r21_compania  = ', vg_codcia, ' AND ',
		        ' r21_localidad = ', vg_codloc, ' AND ',
		        ' r21_moneda    = "', rm_par.r21_moneda, '" AND ',
		          expr_fecha CLIPPED, ' AND ',
		          expr_vend  CLIPPED, ' AND ', 
			  expr_sql   CLIPPED, ' AND ',
			  expr_fact  CLIPPED, ' AND ',
			' r01_compania  = r21_compania  AND ', 
			' r01_codigo    = r21_vendedor ', 
		  ' INTO TEMP temp_prof'
