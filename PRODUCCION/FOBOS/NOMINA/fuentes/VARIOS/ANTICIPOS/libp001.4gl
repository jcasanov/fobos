...
FUNCTION fl_ayuda_forma_pago(cod_cia, cont_cred, estado)
...
WHILE TRUE
	IF NOT primera THEN
		...
		IF int_flag THEN
			...
			RETURN rh_forpago[1].j01_codigo_pago,
				rh_forpago[1].j01_nombre,
...
IF proc = 'FR' THEN
	...
	LET valor_fec = valor_fec - 365
	IF valor_fec > 0 AND valor_fec < 365 THEN
...
LET query = 'SELECT n32_cod_liqrol, n32_fecha_ini, n32_fecha_fin, ',
		'NVL(CASE WHEN "', proc, '" <> "DC" THEN n32_tot_gan END, ',
			'NVL((SELECT n36_ganado_real ',
				'FROM rolt036 ',
				'WHERE n36_compania    = ', cod_cia,
				'  AND n36_proceso     = "', proc, '"',
				expr_sql1 CLIPPED,
				'  AND n36_cod_trab    = ', cod_trab, '), ',
				'NVL((SELECT (n03_valor / 360) * ',
		...
		'   AND n32_cod_trab    = ', cod_trab,
		'   AND n32_estado     <> "E" ',
		' ORDER BY n32_fecha_ini ASC '
PREPARE cons_liqgan FROM query
DECLARE q_liqgan CURSOR FOR cons_liqgan
LET num_row   = 1
LET tot_gan_g = 0
LET val_gan_g = 0
LET primera   = 1
FOREACH q_liqgan INTO rh_liqgan[num_row].*
	IF proc = 'FR' THEN
		IF DAY(fec_ini) <> 1 AND primera THEN
			...
			IF num_dias > 180 THEN
				IF num_dias < 300 THEN
					LET fec_ini = MDY(12, DAY(fec_ini),
							YEAR(fec_ini) - 1)
				ELSE
					LET fec_ini = MDY(01, 01, YEAR(fec_ini))
							- 1 UNITS DAY
				END IF
				LET num_dias = fec_ini - fecha_aux
				IF num_dias > 180 THEN
		...
		END IF
	END IF
	IF proc = 'DC' AND creo_tmp AND fecha_aux IS NOT NULL THEN
		IF rh_liqgan[num_row].n32_fecha_ini > fecha_aux THEN
			LET fact = aux_f
		END IF
	END IF
	LET rh_liqgan[num_row].valor_gan = rh_liqgan[num_row].n32_tot_gan / fact
	IF (proc = 'VA' OR proc = 'VP') AND dias_adi > 0 THEN
		LET rh_liqgan[num_row].valor_gan =rh_liqgan[num_row].valor_gan +
				((rh_liqgan[num_row].valor_gan / 15) * dias_adi)
	END IF
...
--#DISPLAY 'E'			TO tit_col5 
--#DISPLAY 'A'			TO tit_col6 
LET expr_tip = NULL
IF tipo_ret IS NOT NULL THEN
	LET expr_tip = '   AND c03_tipo_ret   = "', tipo_ret, '"'
END IF
LET expr_por = NULL
IF porc_ret IS NOT NULL THEN
...
WHILE TRUE
	...
	WHILE NOT salir
		LET query = 'SELECT c03_codigo_sri, c03_concepto_ret,',
				...
				' c03_estado, ',
				' CASE WHEN ', campo CLIPPED, ' IS NOT NULL ',
					' THEN "S" ',
					' ELSE "N" ',
				' END ',
			' FROM ordt003, OUTER ', tabla CLIPPED,
