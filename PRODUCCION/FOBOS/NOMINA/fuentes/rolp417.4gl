--------------------------------------------------------------------------------
-- Titulo           : rolp417.4gl - LISTADO CONTROL LIQUIDACION DECIMO CUARTO
-- Elaboracion      : 02-sep-2003
-- Autor            : JCM
-- Formato Ejecucion: fglrun rolp417 BD MODULO COMPANIA fecha_ini fecha_fin 
-- Ultima Correccion:
-- Motivo Correccion: 
--------------------------------------------------------------------------------
...
DEFER INTERRUPT
CLEAR SCREEN
CALL startlog('../logs/rolp417.err')
--#CALL fgl_init4js()
...
DECLARE q_decimos CURSOR FOR
	SELECT n36_cod_trab, n30_nombres, n36_valor_bruto,
...
--	print '&k2S' 		-- Letra condensada
        ...
        CALL fl_justifica_titulo('C', 'LISTADO DECIMO CUARTO SUELDO', 30)
                RETURNING titulo
	...
	FREE  q_est
        ...
        PRINT COLUMN 25, "** PERIODO: ", vm_fecha_ini, " - ", vm_fecha_fin,
...
ON EVERY ROW
	...
	PRINT COLUMN 04,  r_rol.cod_trab USING '####',
	      COLUMN 10,  r_rol.nom_trab CLIPPED,
