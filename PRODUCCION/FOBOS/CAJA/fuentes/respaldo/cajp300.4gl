...
WHILE TRUE
	...
	CALL lee_parametros() RETURNING expr_sql, expr_sql3, expr_sql4
	IF int_flag = 2 THEN
		CONTINUE WHILE
	END IF
	IF int_flag THEN
	...
	LET col          = 1
	WHILE TRUE
		IF expr_sql <> ' 1=1' OR expr_sql4 = ' 1=1' THEN
		LET query = 'SELECT DATE(j10_fecha_pro), j10_nomcli, ',
			...
			ELSE
				...
				LET expr  = NULL
			END IF
			IF expr_sql3 <> ' 1=1' AND expr_sql4 = ' 1=1' THEN
				LET expr  = NULL
			ELSE
				LET expr  = 'OUTER'
			END IF
			IF expr_sql4 <> ' 1=1' THEN
				LET lim = LENGTH(expr_sql4)
				LET pos = lim - 4
				IF expr_sql4[pos, lim] = "EC" THEN
					LET expr_sql4 = NULL
				END IF
			END IF
...
INITIALIZE cod_aux, expr_sql, expr_sql3, expr_sql4 TO NULL
LET int_flag = 0
...
END INPUT
IF int_flag THEN
	RETURN expr_sql, expr_sql3, expr_sql4
END IF
...
END IF
OPTIONS INPUT NO WRAP
CONSTRUCT BY NAME expr_sql4 ON j10_tipo_destino
	ON KEY(INTERRUPT)
		LET int_flag = 2
		EXIT CONSTRUCT
        ON KEY(F1,CONTROL-W)
