...
CREATE TEMP TABLE te_master
	(te_tipo_comp		CHAR(2),
	 ...
	 te_indice		SMALLINT)
DECLARE q_drep CURSOR FOR SELECT * FROM acero_qs:rept020
...
SELECT * INTO r_fpc.* FROM acero_qs:rept025
...
IF rm_auxv.b40_compania IS NULL THEN
	...
	EXIT PROGRAM
END IF
SELECT * INTO r_cj.* FROM acero_qs:cajt010
...
DECLARE q_dcaj CURSOR FOR SELECT * FROM acero_qs:cajt011
...
IF rm_crep.r19_cod_tran = 'IM' THEN
	...
	DECLARE qu_y2k CURSOR FOR 
		SELECT r29_pedido FROM acero_qs:rept029
...
INITIALIZE r.* TO NULL
SELECT * INTO r.* FROM acero_qs:rept019 
...
INITIALIZE r.* TO NULL
SELECT * INTO r.* FROM acero_qs:cajt002 
...
INITIALIZE r.* TO NULL
SELECT * INTO r.* FROM acero_qs:cxct022 
