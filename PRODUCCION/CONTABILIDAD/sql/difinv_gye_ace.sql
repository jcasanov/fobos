...
select r19_localidad loc, r19_cod_tran cod, r19_num_tran num,
	...
	into temp tmp_gye;
drop table t2;
select count(*) tot_reg from tmp_gye;
select nvl(round(sum(val_r19), 2), 0) t3_val_r19,
	nvl(round(sum(val_r20), 2), 0) t3_val_r20,
	nvl(round(sum(val_ctb), 2), 0) t3_val_ctb
	from tmp_gye;
select * from tmp_gye where val_r19 <> val_ctb into temp t4;
select count(*) tot_reg_dif_gye from t4;
...
select r19_localidad loc, r19_cod_tran cod, r19_num_tran num,
	...
	into temp tmp_uio;
drop table t2;
select a.*, b.val_ctb val_ctb_uio
	from tmp_gye a, tmp_uio b
	where a.loc2     = b.loc
	  and a.cod2     = b.cod
	  and a.num2     = b.num
