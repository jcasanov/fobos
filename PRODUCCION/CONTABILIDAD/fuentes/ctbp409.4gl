...
PAGE HEADER
	...
	PRINT COLUMN 01,  'CONTABILIDAD',
	      COLUMN 51,  'MOVIMIENTOS DE CUENTA POR FILTROS',
...
BEFORE GROUP OF r_rep.filtro
	...
	CASE r_rep.flag_filtro
		WHEN 'C'
			CALL fl_lee_cliente_general(r_rep.filtro)
				RETURNING r_z01.*
			LET registro = r_rep.filtro CLIPPED, '|',
					r_z01.z01_nomcli CLIPPED, '|',
					vm_cta_inicial CLIPPED, '|',
					nom_cta_ini CLIPPED, '|', '|', '|'
	...
	LET tot_val_db = 0
	LET tot_val_cr = 0
	LET registro   = registro CLIPPED, vm_fec_antes USING 'dd-mm-yyyy',
			'|', '|', '|', '|', vm_saldo USING '---,---,--&.--'
...
ON EVERY ROW
	...
	IF vm_archivo = 'Yes' THEN
		LET registro = '|', '|',
				...
				'|', r_rep.tipo_comp,
				'|', r_rep.num_comp,
				'|', r_rep.fec_proceso USING 'dd-mm-yyyy',
				'|', r_rep.glosa CLIPPED,
				'|', val_db USING '##,###,###,##&.##',
				'|', val_cr USING '##,###,###,##&.##',
				'|', vm_saldo USING '--,---,---,--&.##'
