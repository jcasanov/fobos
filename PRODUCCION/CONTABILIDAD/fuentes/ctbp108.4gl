------------------------------------------------------------------------------
-- Titulo           : ctbp108.4gl - Mantenimiento de Códigos de Filtros 
-- Elaboracion      : 27-sep-2001
-- Autor            : NPC
-- Formato Ejecucion: fglrun ctbp108 base módulo commpañía
-- Ultima Correccion: 
-- Motivo Correccion: 
------------------------------------------------------------------------------
...
CALL startlog('../logs/ctbp108.err')
--#CALL fgl_init4js()
	...
	AFTER FIELD b08_filtro
		IF rm_ctb.b08_filtro IS NOT NULL THEN
			...
			IF r_ctb_aux.b08_filtro IS NOT NULL THEN
				CALL fgl_winmessage(vg_producto,'Código de filtro de cuenta ya existe','exclamation')
