...
CALL fl_mostrar_mensaje('Clave digitada está errónea', 'exclamation')
...
END IF
IF num_sri[4,4] <> "-" THEN
	LET num_sri[5,15] = num_sri[4,15]
	LET num_sri[4,4] = '-'
END IF
IF num_sri[8,8] <> "-" THEN
	LET num_sri[9,15] = num_sri[8,15]
	LET num_sri[8,8] = '-'
END IF
...
IF num_sri[5,7] <> r_g37.g37_pref_pto_vta THEN
	...
	RETURN r_g37.*, num_sri, flag
END IF
IF LENGTH(num_sri[9,15]) <> LENGTH(sec_sri) THEN
	CALL fl_mostrar_mensaje('La secuencia del SRI ' || num_sri[9,15] || ' es incorrecta.','exclamation')
	LET flag = 0
	LET num_sri = r_g37.g37_pref_sucurs, '-', r_g37.g37_pref_pto_vta, '-',
			sec_sri
	RETURN r_g37.*, num_sri, flag
END IF
{--
IF num_sri[9,15] < sec_sri THEN
	CALL fl_mostrar_mensaje('La secuencia del SRI ' || num_sri[9,15] || ' ya existe.','exclamation')
	LET flag = 0
	LET num_sri = r_g37.g37_pref_sucurs, '-', r_g37.g37_pref_pto_vta, '-',
			sec_sri
	RETURN r_g37.*, num_sri, flag
...
INPUT BY NAME r_n30.n30_cod_trab, r_n30.n30_nombres, r_n30.n30_tipo_pago, 
	...
	ON KEY (F2)
		IF infield(n30_bco_empresa) THEN
                        CALL fl_ayuda_cuenta_banco(cod_cia)
		...
		SELECT g37_autorizacion
			...
			WHERE g37_compania   = codcia
			  ...
			  AND g37_tipo_doc   =
				(SELECT r38_tipo_doc
					FROM rept038
					WHERE r38_compania    = g37_compania
					  AND r38_localidad   = g37_localidad
	  		  ...
	  		  AND g37_fecha_exp >= DATE(TODAY)
	OPEN q_autoriz
	FETCH q_autoriz INTO r_r95.r95_autoriz_sri
	CLOSE q_autoriz
	FREE q_autoriz
END IF
...
INPUT BY NAME r_r95.r95_fecha_initras, r_r95.r95_num_sri, r_r95.r95_autoriz_sri,
		...
		r_r95.r95_persona_id, r_r95.r95_persona_dest,
...
IF cod_tran = 'TR' THEN
	...
	END FOREACH
	IF NOT resul THEN
		CALL fl_mostrar_mensaje('La localidad de la bodega de destino des esta transferencia, no tiene guía de remisión.', 'stop')
		RETURN 0
	END IF
END IF
SELECT * INTO r_r95.*
...
FUNCTION fl_lee_codigos_sri(codcia, tipo_ret, porc_ret, cod_sri)
...
DECLARE q_s25 CURSOR FOR
	SELECT * FROM srit025
		WHERE codcia   = s25_compania
		  AND tipo_ret = s25_tipo_ret
		  AND porc     = s25_porcentaje
		  AND cliprov  = s25_cliprov
OPEN q_s25
