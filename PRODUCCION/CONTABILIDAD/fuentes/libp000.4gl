...
FOREACH q_crot INTO r_cab.*
	IF rot.t23_estado = 'D' AND r_cab.r19_fecing > rfd.t28_fec_factura THEN
		CONTINUE FOREACH
	END IF
	CALL fl_lee_cod_transaccion(r_cab.r19_cod_tran)
		RETURNING r.*
...
IF STATUS < 0 THEN
	--ROLLBACK WORK
	CALL fl_mostrar_mensaje('Registro ' || r_doc.z20_tipo_doc || '-' || r_doc.z20_num_doc || ' esta bloqueado por ortro proceso.', 'exclamation')
	RETURN -1
END IF
CLOSE q_ddev
...
FUNCTION fl_lee_vacaciones(cod_cia, cod_trab, per_ini, per_fin, sec)
...
DEFINE sec		LIKE rolt039.n39_secuencia
