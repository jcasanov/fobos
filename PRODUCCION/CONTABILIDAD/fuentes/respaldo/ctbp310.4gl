...
LET query = 'SELECT b10_cuenta, b10_descripcion, b10_estado, b10_nivel, ',
		' (SELECT NVL(SUM(b13_valor_base), 0) ',
			...
			'   AND b13_fec_proceso BETWEEN "', fecha_ini,
						 '" AND "', fecha_fin, '"',
			'   AND b13_valor_base  >= 0) movi_neto_db, ',
		' (SELECT NVL(SUM(b13_valor_base), 0) ',
			' FROM ctbt012, ctbt013 ',
			' WHERE b12_compania     = b10_compania ',
			'   AND b12_estado      <> "E" ',
			...
			'   AND b13_fec_proceso BETWEEN "', fecha_ini,
						 '" AND "', fecha_fin, '"',
			'   AND b13_valor_base   < 0) movi_neto_cr ',
		' FROM ctbt010 ',
		' WHERE b10_compania  = ', vg_codcia, 
		expr_cta CLIPPED,
		'   AND b10_nivel     BETWEEN ', rm_par.nivel_ini,
...
DISPLAY BY NAME rm_par.cuenta_ini, rm_par.cuenta_fin,
		...
		rm_par.mes_fin, rm_par.n_mes_fin
...
ON LAST ROW
	PRINT COLUMN 071, '--------------------',
	      COLUMN 092, '--------------------'
	PRINT COLUMN 072, tot_mov_neto_db USING '((((,(((,(((,((&.##)',
