...
LET query = 'SELECT z50_areaneg, z02_zona_cobro, z50_codcli, z01_nomcli, ',
		  ...
		  ' z50_compania, z50_localidad, z50_cod_tran, z50_num_tran ',
...
PAGE HEADER
	...
	PRINT COLUMN 001, "AREA",
	      ...
	      COLUMN 080, "FECHA EMI.",
	      COLUMN 094, "FECHA VCTO.",
	      COLUMN 109, fl_justifica_titulo('D', "DIAS", 5),
	      COLUMN 116, fl_justifica_titulo('D', "SALDO", 16)
	PRINT COLUMN 001, "------------------------------------------------------------------------------------------------------------------------------------"
...
ON EVERY ROW
	...
	PRINT COLUMN 001, fl_justifica_titulo('D', areaneg, 3),
	      ...
	      COLUMN 021, nomcli[1,31],
	      COLUMN 054, tipo_doc, '-', 
	      		  fl_justifica_titulo('I', num_doc, 15) CLIPPED, '-', 
	      		  fl_justifica_titulo('I', dividendo, 3) USING "&&&",
	      COLUMN 080, fecha_emi USING "dd-mm-yyyy",
	      COLUMN 094, fecha_vcto USING "dd-mm-yyyy",
	      COLUMN 109, antiguedad USING "-,--&",
	      COLUMN 116, saldo USING "#,###,###,##&.##"

ON LAST ROW
	NEED 2 LINES
	PRINT COLUMN 116, "------------------"
	PRINT COLUMN 116, SUM(saldo) USING "#,###,###,##&.##";
	print ASCII escape;
