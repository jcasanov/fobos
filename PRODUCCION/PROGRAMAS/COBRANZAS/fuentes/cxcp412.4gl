...
LET int_flag = 0
INPUT BY NAME rm_par.moneda, rm_par.documento, rm_par.area, rm_par.cliente,
	rm_par.inicial, rm_par.final, rm_par.saldo, rm_par.origen_doc
	WITHOUT DEFAULTS
	ON KEY (INTERRUPT)
		LET int_flag = 1
		RETURN
...
PAGE HEADER
	...
	PRINT COLUMN 01,   'F. DCTO.',
	      ...
	      COLUMN 55,  ' SEC.',
	      COLUMN 61,  'No. SRI',    		
	      COLUMN 78,  'VALOR CAPITAL',
	      COLUMN 92,  'VALOR INTERES',
...
ON EVERY ROW
	...
	PRINT COLUMN 01,  fecha_doc	USING 'dd-mm-yyyy',
	      ...
	      COLUMN 28,  origen,
	      COLUMN 30,  area[1, 5],
	      COLUMN 36,  tipo_documento,
	      COLUMN 39,  num_documento,
	      COLUMN 55,  sec_documento USING '###&&',
	      COLUMN 61,  num_sri,
	      COLUMN 78,  valor_capital USING '##,###,##&.##',
	      COLUMN 92,  valor_interes USING '##,###,##&.##',
...
ON LAST ROW
	...
	PRINT COLUMN 78,  '-------------',
	      ...
	      COLUMN 106, '-------------',
	      COLUMN 120, '-------------'
	PRINT COLUMN 78,  SUM(valor_capital) USING '##,###,##&.##',
	      COLUMN 92,  SUM(valor_interes) USING '##,###,##&.##', 
